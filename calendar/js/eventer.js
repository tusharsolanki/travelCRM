...
function EventerEventsCalendar(tDIV) {
	...
	this.dateBoxWidth = 130;
	this.dateBoxHeight = 132;
	...
	function displayEventsData() {
		...
		$(self.cal + ' ul#month-dates-container > li').each(function(index, element) {
			...
			$(this).css({
				...
				'top'			:	(self.dateBoxHeight-(-6)) * row + 'px',
				'left'			:	(self.dateBoxWidth+self.dateBoxHSpace+2) * col +'px',
				'z-index'		:	index
			});
			...
			if ((index + 1) % 7 == 0) {
				...
				if (totalHeight > self.dateBoxHeight) {
					...
					$('li.date-box:gt(' + (rowID * 7 - 1) + ')').each(function(index, element) {
						$(this).css('top', parseInt($(this).css('top')) + (totalHeight - self.dateBoxHeight - 25));
					});
				}
				else {
					$('li.date-box-row-' + rowID).each(function(index, element) {
		...
		$(self.cal + ' .date-box-disabled').css('opacity', .20);
		
		$(self.cal + ' #calendar-nav').css('width', ((self.dateBoxWidth + self.dateBoxHSpace) * 7 - self.dateBoxHSpace) + 'px');
		$(self.cal + ' #week-day-names-container').css('width', ((self.dateBoxWidth + self.dateBoxHSpace) * 7 - self.dateBoxHSpace+15) + 'px');
	...
	function showEvent($targetDateBox) {
		...
		//alert($('#' + $($targetDateBox).attr('id') + ' ul.event-details-items li:eq(' + indexID + ') .event-item-details-scrollbar .info-panel iframe').attr('src'));
		var iframe_src = $('#' + $($targetDateBox).attr('id') + ' ul.event-details-items li:eq(' + indexID + ') .event-item-details-scrollbar .info-panel iframe').attr('src');
		$('#' + $($targetDateBox).attr('id') + ' ul.event-details-items li:eq(' + indexID + ') .event-item-details-scrollbar .info-panel').html("");
		$('#' + $($targetDateBox).attr('id') + ' ul.event-details-items li:eq(' + indexID + ') .event-item-details-scrollbar .info-panel').html("<iframe name=\'iframename\' width=\'950\' height=\'565\' src=\'"+iframe_src+"\'></iframe>");
	}
