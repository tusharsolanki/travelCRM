...
class phonecall_script_model extends CI_Model
{
    function __construct()
    {
        parent::__construct();
        $this->table_name = 'phone_call_script_master';
    ...
    */
	...
	public function getscriptpagingid($script_id='')
	{
		$this->db->select('*');
    ...
    */
	function get_new_template($parent_db='',$new_ar='')
	{
		if(!empty($new_ar))
		{
			$new_ar=" AND ph.id NOT IN (".$new_ar.")";
		}
		$now=date('Y-m-d H:i:s');
		$sql_ins = "INSERT INTO ".$this->table_name." (template_name, template_category, template_subcategory, template_subject, calling_script,superadmin_template_id, admin_publish_date, superadmin_publish_date,is_default,edit_flag, created_date, created_by, status) SELECT ph.template_name, ccat.id, ph.template_subcategory, ph.template_subject, ph.calling_script,ph.id, '".$now."' as admin_publish_date, ph.superadmin_publish_date,ph.is_default,'1' as edit_flag, ph.created_date,'1' as created_by,ph.status FROM ".$parent_db.".".$this->table_name." as ph LEFT JOIN ".$parent_db.".marketing_master_lib__category_master pcat ON ph.template_category  = pcat.id LEFT JOIN marketing_master_lib__category_master ccat ON ccat.superadmin_cat_id  = pcat.id  where ph.is_default  = '1' and ph.publish_flag = '1' ".$new_ar.";";
    ...
    */
	function get_update_template($parent_db='',$parent_id='',$next_temp,$db_name='')
	{
		...
		$sql_ins = "INSERT INTO ".$table_name." (template_name, template_category, template_subcategory, template_subject, calling_script,superadmin_template_id, admin_publish_date, superadmin_publish_date,is_default,edit_flag, created_date, created_by, status) SELECT '".$next_temp."' as template_name, ccat.id, ph.template_subcategory, ph.template_subject, ph.calling_script,ph.id, '".$now."' as admin_publish_date, ph.superadmin_publish_date,ph.is_default,'1' as edit_flag, ph.created_date,'1' as created_by,ph.status FROM ".$parent_db.".".$this->table_name." as ph LEFT JOIN ".$parent_db.".marketing_master_lib__category_master pcat ON ph.template_category  = pcat.id LEFT JOIN ";
		if(!empty($db_name))
		  	$sql_ins .= $db_name.".marketing_master_lib__category_master ccat ";
		else
			$sql_ins .= "marketing_master_lib__category_master ccat ";
		$sql_ins .= "ON ccat.superadmin_cat_id  = pcat.id where ph.id = ".$parent_id.";";
		//marketing_master_lib__category_master ccat ON ccat.superadmin_cat_id  = pcat.id  where ph.id = ".$parent_id.";";
		$query = $this->db->query($sql_ins);
		$lastId = mysql_insert_id();
		return $lastId;
		// return $query->result_array();
