...
class letter_library_model extends CI_Model
{
    function __construct()
    {
        parent::__construct();
        $this->table_name = 'letter_template_master';
 ...
 /*
    ...
    */
	function get_new_template($parent_db='',$new_ar='')
	{
		if(!empty($new_ar))
		{
			$new_ar=" AND let.id NOT IN (".$new_ar.")";
		}
		$now=date('Y-m-d H:i:s');
		$sql_ins = "INSERT INTO ".$this->table_name." (template_name, template_category, template_subcategory,size_w,size_h , template_subject,letter_content,superadmin_template_id, admin_publish_date, superadmin_publish_date,is_default,edit_flag, created_date, created_by, status) SELECT let.template_name, ccat.id, let.template_subcategory,let.size_w,let.size_h,let.template_subject, let.letter_content,let.id, '".$now."' as admin_publish_date, let.superadmin_publish_date,let.is_default,'1' as edit_flag, let.created_date,'1' as created_by,let.status FROM ".$parent_db.".".$this->table_name." as let LEFT JOIN ".$parent_db.".marketing_master_lib__category_master pcat ON let.template_category  = pcat.id LEFT JOIN marketing_master_lib__category_master ccat ON ccat.superadmin_cat_id  = pcat.id where let.is_default  = '1' and let.publish_flag = '1' ".$new_ar.";";
    ...
    */
	function get_update_template($parent_db='',$parent_id='',$next_temp,$db_name='')
	{
		...
		$sql_ins = "INSERT INTO ".$table_name." (template_name, template_category, template_subcategory,size_w,size_h , template_subject,letter_content,superadmin_template_id, admin_publish_date, superadmin_publish_date,is_default,edit_flag, created_date, created_by, status) SELECT '".$next_temp."' as template_name, ccat.id, let.template_subcategory,let.size_w,let.size_h,let.template_subject, let.letter_content,let.id, '".$now."' as admin_publish_date, let.superadmin_publish_date,let.is_default,'1' as edit_flag, let.created_date,'1' as created_by,let.status FROM ".$parent_db.".".$this->table_name." as let LEFT JOIN ".$parent_db.".marketing_master_lib__category_master pcat ON let.template_category  = pcat.id LEFT JOIN ";
		if(!empty($db_name))
		  	$sql_ins .= $db_name.".marketing_master_lib__category_master ccat ";
		else
			$sql_ins .= "marketing_master_lib__category_master ccat ";
		$sql_ins .= "ON ccat.superadmin_cat_id  = pcat.id where let.id = ".$parent_id.";";
		$query = $this->db->query($sql_ins);
		$lastId = mysql_insert_id();
		return $lastId;
		// return $query->result_array();
