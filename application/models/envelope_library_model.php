...
class envelope_library_model extends CI_Model
{
    function __construct()
    {
        parent::__construct();
        $this->table_name = 'envelope_template_master';
    ...
    */
    ...
    /*
    @Description: Function is for Insert Envelope Templets details by Admin
    @Author: Mit Makwana
    @Input: Envelope details for Insert into DB
    ...
    */
    ...
    function insert_record($data)
    {
	  ...
	  /*
    @Description: Function is for Insert Envelope details by Admin
    @Author: Mit Makwana
    @Input: Envelope details for Insert into DB
    @Output: - Insert record into DB
    @Date: 12-08-2014
    */
    ...
    @Author: Mit Makwana
    @Input: Envelope details for Update into DB
    ...
    */
    ...
    @Author: Mit Makwana
    @Input: - Envelope id which is delete by admin
    ...
    */
	...
	/*
		...
		@Date: 10-09-2014
    */
	public function getemailpagingid($envelope_id='')
	{
		$this->db->select('*');
    ...
    */
	function get_update_template($parent_db='',$parent_id='',$next_temp,$db_name='')
	{
		...
		$sql_ins = "INSERT INTO ".$table_name." (template_name, template_category, template_subcategory,template_type, template_size_id, size_w,size_h , envelope_content,superadmin_template_id, admin_publish_date, superadmin_publish_date,is_default,edit_flag, created_date, created_by, status) SELECT '".$next_temp."' as template_name, ccat.id, en.template_subcategory, en.template_type,en.template_size_id, en.size_w,en.size_h , en.envelope_content,en.id, '".$now."' as admin_publish_date, en.superadmin_publish_date,en.is_default,'1' as edit_flag, en.created_date,'1' as created_by,en.status FROM ".$parent_db.".".$this->table_name." as en LEFT JOIN ".$parent_db.".marketing_master_lib__category_master pcat ON en.template_category  = pcat.id LEFT JOIN ";
		if(!empty($db_name))
		  	$sql_ins .= $db_name.".marketing_master_lib__category_master ccat ";
		else
			$sql_ins .= "marketing_master_lib__category_master ccat ";
		$sql_ins .= "ON ccat.superadmin_cat_id  = pcat.id where en.id = ".$parent_id.";";
		//marketing_master_lib__category_master ccat ON ccat.superadmin_cat_id  = pcat.id where en.id = ".$parent_id.";";
		$query = $this->db->query($sql_ins);
		$lastId = mysql_insert_id();
		return $lastId;
		// return $query->result_array();
