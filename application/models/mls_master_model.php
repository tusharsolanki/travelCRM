<?php
class mls_master_model extends CI_Model
{
    function __construct()
    {
        parent::__construct();
        $this->table_name = 'mls_master';
        $this->table_name_mls_assign_data = 'mls_assign_data';
    ...
    */
	function insert_plan_type_parent($data)
    {
		...
		$idata['created_date'] = date('Y-m-d H:i:s');		
        $idata['status'] = '1';
       	if(!empty($data['mls_name']))
		{
			$lastId = array();
			$mls_name = $data['mls_name'];
			$count=0;
			foreach($mls_name as $row)
			{
				$idata['mls_name'] = $row;
				if(trim($idata['mls_name']) != "")
				{
					$result =  $this->db->insert($this->table_name,$idata);
					$lastId[$count]['mls_name'] = $idata['mls_name'];
    ...
    */
    ...
    /*
        @Description: Function for Insert Assign MLS transaction
    ...
    */
    ...
    public function insert_assign_mls($data='')
    {
        $this->db->insert($this->table_name_mls_assign_data,$data);
        return mysql_insert_id();
    }
    
    /*
        @Description: Function for Select Assign MLS
        @Author     : Sanjay Chabhadiya
        @Input      : 
        @Output     : 
        @Date       : 29-04-2015
    */
    ...
    public function select_assign_mls($data='')
    {
        if(!empty($data))
        {
            $this->db->select('*');
            $this->db->where($data);
            $result = $this->db->get($this->table_name_mls_assign_data);
            return $result->result_array();
        }
    }
    
    /*
        @Description: Function for Delete Assign MLS
        @Author     : Sanjay Chabhadiya
        @Input      : 
        @Output     : 
        @Date       : 29-04-2015
    */
    ...
    public function delete_assign_mls($data='')
    {
        if(!empty($data))
        {
            $this->db->where($data);
            $this->db->delete($this->table_name_mls_assign_data);
        }
    }
}