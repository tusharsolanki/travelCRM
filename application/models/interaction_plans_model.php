...
class interaction_plans_model extends CI_Model
{
    function __construct()
    {
        parent::__construct();
        $this->table_name = 'interaction_plan_master';
    ...
    */
	...
	public function select_records_plan_contact_trans($getfields='', $match_values = '', $condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$sort='',$db_name='')
    {
        $fields =  $getfields ? implode(',', $getfields) : '';
        $sql = 'SELECT ';
...
JOIN interaction_plan__status_master ipsm ON ipsm.id = ipm.plan_status ORDER BY ipm.created_date DESC");*/
	...
	public function select_usercontact_intraction_plan($contact_id='',$user_id='')
    {
		$this->db->select('DISTINCT ipct.interaction_plan_id',false);
		$this->db->from('interaction_plan_contacts_trans ipct');
		$this->db->where('ipct.contact_id',$contact_id);
  ...
  */
    ...
    */
	...
	function insert_contact_trans_record($data,$db_name='')
    {
        if(!empty($db_name))
	...
	function insert_contact_converaction_trans_record($data,$db_name='')
    {
        
		if(!empty($db_name))
		{$result =  $this->db->insert($db_name.'.'.$this->table_name_contact_conve_trans,$data);	}
		else
    ...
    */
	...
	public function update_adminuser_trans_record($data='',$parent_db_name='')
    {
        //pr($data);exit;
        $this->db->where('id',$data['id']);
		if(!empty($parent_db_name))
        	$query = $this->db->update($parent_db_name.".".$this->table_name4,$data);
		else
    ...
    */
	...
	public function update_interaction_plan($data)
	{
		$this->db->where('p_p_id',$data['p_p_id']);
		$this->db->where('created_by',$data['created_by']);
		$this->db->update($this->table_name,$data);
	}

	public function get_updated_premium_plans($db_name='',$where='')
	{
		$this->db->select('ipmp.*,ipat.id as adminuser_trans_id,ipat.user_id');
		$this->db->from($db_name.'.interaction_plan_adminuser_trans as ipat');
		$this->db->join($db_name.'.interaction_plan_master_premium as ipmp','ipmp.id = ipat.interaction_plan_id','left');
		if(!empty($where))
			$this->db->where($where);
		$query_FC = $this->db->get();
  		return $query_FC->result_array();
