...
class interaction_model extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->table_name = 'interaction_plan_interaction_master';
    ...
    */
    ...
    public function select_records($getfields='', $match_values = '', $condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$sort='',$where_cond='',$db_name='')
    {
...
JOIN interaction_plan__status_master ipsm ON ipsm.id = ipm.plan_status ORDER BY ipm.created_date DESC");
    ...
    */
    ...
    {  
		...
		if(!empty($where) && $match_values != null &&  $compare_type != null && is_array($where))
		{
			//pr($where);
			//pr($match_values);
	...
	function insert_contact_communication_record($data,$db_name='')
    {
		if(!empty($db_name))
			$result =  $this->db->insert($db_name.".".$this->table_name1,$data);
		else
	...
	function get_contact_interaction_task_date($interaction_plan_interaction_id,$contact_id='',$db_name='')
	{
		...
		if(!empty($db_name))
		{return $this->db->get($db_name.'.'.$this->table_name2)->row();}
		else
    ...
    */
	...
	public function fetch_contact_communication_record($id)
    {
        //pr($data);exit;
		if(!empty($id))
		{
			$this->db->where('id',$id);
			return $this->db->get($this->table_name2)->result_array();
		}
		else
			return array();
    }
    ...
    */
	...
	public function current_interaction_plan($plan_id,$assign_to='')
	{
...
WHERE ipccp.interaction_plan_id = '.$plan_id;
if(!empty($assign_to))
	$qry .= ' AND ipim.assign_to IN ('.$assign_to.')';
$qry .= ' AND cm.id IS NOT NULL GROUP BY ipccp.interaction_plan_interaction_id,ipccp.contact_id ORDER BY ipim.interaction_sequence_date ASC
';
//echo $qry;
    ...
    */
    ...
    public function select_records1($getfields='', $match_values = '',$match_values1 = '', $condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$sort='',$where_cond='',$or_clause='')
    {
    ...
    */
    ...
    public function select_records2($getfields='', $match_values = '', $condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$sort='',$where_cond='')
    {
    ...
    public function get_interaction_plan_interaction_trans_record($icdata='')
    {
        $this->db->select('id');
	...
	$this->db->where('interaction_plan_interaction_id',$icdata['interaction_plan_interaction_id']);
        $this->db->from('interaction_plan_contact_communication_plan');
        $result = $this->db->get()->result_array();
