				...
				<?php } ?>
                      ...
                      <div class="row">
                        <div class="col-xs-12 mrgb2">
                          ...
                          :</label>
                          <textarea id="txtarea_description"  placeholder="e.g. Description" name="txtarea_description" class="form-control parsley-validated"><?php if(!empty($editRecord[0]['description'])){ echo htmlentities($editRecord[0]['description']); }?>
</textarea>
                        </div>
                      </div>
                  ...
                  <div class="col-sm-12 text-center margin-top-10">
					...
					<input type="submit" class="btn btn-secondary" value="Save Communication" name="submitbtn" onclick="return checkcontactcount();" />
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-sm-12 add_new_contact_popup">
          <div class="table-responsive">
		  <?php $this->load->view('superadmin/default_interaction_plans/add_contact_popup_ajax');?>
		  </div>
        </div>
      </div>
...
$(document).ready(function(){
	...
	function contact_search()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>superadmin/default_interaction_plans/search_contact_ajax/",
			data: {
			result_type:'ajax',searchtext:$("#search_contact_popup_ajax").val(),contact_status:$('#slt_contact_status').val(),contact_source:$('#slt_contact_source').val(),contact_type:$('#contact_type').val()
		},
		beforeSend: function() {
	...
	function addcontactstointeractionplan()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>superadmin/default_interaction_plans/add_contacts_to_interaction_plan/",
			data: {
