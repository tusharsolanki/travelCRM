<?php 
    /*
        @Description: superadmin Tempalte Envelope
        @Author: Mit Makwana
        @Date: 12-08-2014
    */
          ...
          <thead>
           <tr role="row">
            ...
            <th width="10%" class="hidden-xs hidden-sm" data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><?=$this->lang->line('common_label_update_publish')?></th>
             ...
             <th width="13%" class="hidden-xs hidden-sm sorting_disabled actionbtn7" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade">
			 <?php echo $this->lang->line('common_label_action')?></th>
           </tr>
           </thead>
					...
					?>
						...
						?> > 
							...
							<?php
								...
								{ ?>
									<?=!empty($row['size_w'])?$row['size_w'].'"':'';?> <strong>X</strong> <?=!empty($row['size_h'])?$row['size_h'].'"':'';?></td>
							<?  }
							?>
...
<? } ?>
							...
							<td class="hidden-xs hidden-sm ">
								<a class="btn btn-xs btn-success" title="Copy Envelope" href="<?= $this->config->item('superadmin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy copyicon5"></i></a> &nbsp;        
								<a class="btn btn-xs btn-success" title="Edit Envelope" href="<?= $this->config->item('superadmin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
