          ...
          <thead>
           ...
           <?php /*?> <th class="sorting_disabled" role="columnheader" rowspan="1" colspan="1" aria-label="" width="5%">
            ...
            <th width="20%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if(isset($sortfield) && $sortfield == 'property_type'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('property_type','<?php echo $sorttypepass;?>')">Property Type</a></th>
             ...
             <th width="15%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if(isset($sortfield) && $sortfield == 'code'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('code','<?php echo $sorttypepass;?>')">Code</a></th>		
                    
            <th width="30%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'description'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('description','<?php echo $sorttypepass;?>')"><?=$this->lang->line('common_label_desc')?></a></th>
			

			<th width="15%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'value_code'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('value_code','<?php echo $sorttypepass;?>')">Value Code</a></th>
           ...
           <?php if(!empty($datalist) && count($datalist)>0){
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){ //pr($row);?>
						...
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							<?php /*?><td class="">
                              <div class="text-center">
                                  <input type="checkbox" class="mycheckbox_1" name="check_1[]" value="<?php echo  $row['id'] ?>">
							  </div>
                            </td><?php */?>
							<?php //pr($row);exit;?>
              ...
              <td class="hidden-xs hidden-sm"><?=!empty($row['code'])?$row['code']:'';?></td>
							<td class="hidden-xs hidden-sm"><?=!empty($row['description'])?ucfirst(strtolower($row['description'])):'';?></td>
              <td class="hidden-xs hidden-sm"><?=!empty($row['value_code'])?$row['value_code']:'';?></td>
          ...
          <div class="col-sm-6">
           <div class="dataTables_paginate paging_bootstrap float-right">
            ...
            <label>
             ...
             <option value="">MLS Amenity</option>
