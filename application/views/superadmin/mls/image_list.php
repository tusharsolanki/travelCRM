...
$property_id = $this->router->uri->segments[4];
?>
 <div id="content">
  <div id="content-header">
   <h1><?=$this->lang->line('property_image');?></h1>
  </div>
    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=$this->lang->line('property_image');?></h3>
      ...
      <div class="portlet-content">
       <div class="table_large-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
          ...
          <div class="col-sm-11">
           <div class="dataTables_filter" id="DataTables_Table_0_filter">
            ...
            <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?=!empty($uri_segment)?$uri_segment:'0'?>">
			 ...
			 <button class="btn btn-secondary howler" title="Search Image" data-type="danger" onclick="contact_search('changesearch');">Search</button>
			 <button class="btn btn-secondary howler" title="View All Admin" data-type="danger" onclick="clearfilter_contact();">View All</button>
            </label>
           </div>
          </div>
         </div>
         <div class="row dt-rt">
         <!-- <div class="col-sm-6">
           <button class="btn btn-danger howler" title="Delete Image" data-type="danger" onclick="deletepopup1('0');">Delete Image</button>
          </div>-->
          <div class="col-sm-6">
         <?php /*?> <? if(!empty($admin_list)) {?>
           <a title="Add Admin Rights" class="btn pull-right btn-secondary-green howler margin-left-5px" href="<?=base_url('superadmin/'.$viewname.'/admin_rights');?>">Add Admin Rights</a>
           <? } ?><?php */?>
         <?php /*?> <a title="Add Admin" class="btn pull-right btn-secondary-green howler " href="<?=base_url('superadmin/'.$viewname.'/add_record');?>">Add Admin</a><?php */?>
         
          </div>
         </div>
         <div id="common_div">
         <?=$this->load->view('superadmin/'.$viewname.'/image_ajax_list')?>
         </div>
        </div>
       </div>
      </div>
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>superadmin/mls/property_image_list/<?=$property_id?>/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('superadmin_base_url').$viewname.'/ajax_delete_all1';?>",
			dataType: 'json',
			//async: false,
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>superadmin/mls/property_image_list/<?=$property_id?>/"+data,
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
