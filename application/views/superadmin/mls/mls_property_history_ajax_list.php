          ...
          <thead>
           ...
           <!-- <th class="sorting_disabled" role="columnheader" rowspan="1" colspan="1" aria-label="" width="5%">
            ...
            <th width="20%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if(isset($sortfield) && $sortfield == 'property_type'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('property_type','<?php echo $sorttypepass;?>')">Property Type</a></th>
             ...
             <th width="20%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if(isset($sortfield) && $sortfield == 'ml_number'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('ml_number','<?php echo $sorttypepass;?>')">ML Number</a></th>		
                    
            <th width="25%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'list_price'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('list_price','<?php echo $sorttypepass;?>')">List Price</a></th>
			

			<th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'change_date'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('change_date','<?php echo $sorttypepass;?>')">Change Date</a></th>
           ...
           <?php if(!empty($datalist) && count($datalist)>0){
              ...
              <td class="hidden-xs hidden-sm"><?=!empty($row['mls_name'])?$row['mls_name']:'';?></td>
							<td class="hidden-xs hidden-sm"><?=!empty($row['property_type'])?strtoupper($row['property_type']):'';?></td>
                            <td class="hidden-xs hidden-sm"><?=!empty($row['ml_number'])?$row['ml_number']:'';?></td>
							<td class="hidden-xs hidden-sm"><?=!empty($row['list_price'])?$row['list_price']:'';?></td>
                            <td class="hidden-xs hidden-sm"><?=!empty($row['change_date'])?ucfirst(strtolower($row['change_date'])):'';?></td>
          ...
          <div class="col-sm-6">
           <div class="dataTables_paginate paging_bootstrap float-right">
            ...
            <label>
             ...
             <option value="">MLS Property History</option>
