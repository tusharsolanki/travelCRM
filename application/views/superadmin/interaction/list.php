<?php 
    /*
        @Description: superadmin Action list
...
?>
 <div id="content">
  <div id="content-header">
   <h1><?=$this->lang->line('interaction_header');?></h1>
  </div>
    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=ucfirst($list[0]['plan_name'])?> : <?=$this->lang->line('interaction');?></h3>
      ...
      <div class="portlet-content">
       <div class="table-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          <div class="col-sm-6">
           <button class="btn btn-danger howler" data-type="danger" onclick="active_plan('0');" title="Add to Archive">Add to Archive List</button>
		   <!--<button class="btn btn-danger howler" data-type="danger" onclick="deletepopup1('0');">Delete Action</button>-->
          </div>
          <div class="col-sm-6">
		  
		   <a class="btn  pull-right btn-success howler margin-left-5px" href="<?=base_url('superadmin/'.$viewname."/".$plan_id.'/view_archive');?>" title="View Archive">View Archive</a>
           <?php if(isset($datalist[0]['plan_status']) && $datalist[0]['plan_status'] == 0){}else{ ?>
          <a class="btn  pull-right btn-secondary-green howler" href="<?=base_url('superadmin/'.$viewname.'/add_record/'.$plan_id);?>" title="Add Action">Add Action</a>
		  <?php } ?>
...
<script>
    ...
    {   
		if(id.length > 50)
			id = id.substr(0, 50)+'...';
		$.confirm({'title': 'CONFIRM','message': " <strong> Are you sure want to archive "+unescape(id)+" "+"<strong>?</strong>",'buttons': {'Yes': {'class': '',
		'action': function(){
	...
	function active_all_plans(name)
		{
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('superadmin_base_url').$viewname."/".$plan_id.'/ajax_Inactive_all';?>",
			dataType: 'json',
			//async: false,
			...
			success: function(data){
				...
				if(data.msg != '')
				{
					...
					$("#temp_dev").html(data.msg);
					$("#temp_dev").fadeOut(4000);
				}
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>superadmin/interaction/<?php echo $plan_id.'/'; ?>"+data.page,
					data: {
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search(allflag)
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>superadmin/interaction/<?php echo $this->router->uri->segments[3]; ?>/"+uri_segment,
				data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>superadmin/interaction/<?php echo $plan_id; ?>",
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
...
$('body').on('click','.view_contacts_btn',function(e){
			...
			planid = $(this).attr('data-group');
			contact_id = $(this).attr('data-id');
			//interactionid = $(this).attr('data-id');
			...
			data: {'contact_list':contact_id,'interaction_plan_id':planid},
			success: function(html){
			...
			/*$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('superadmin_base_url').'interaction/view_contacts_of_interaction_by_step';?>",
			data: {'interaction_plan':planid,'interaction':interactionid},
			success: function(html){
				//$(".view_contact_popup").html(html);	
			},
			error: function(jqXHR, textStatus, errorThrown) {
