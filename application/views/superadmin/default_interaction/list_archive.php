       ...
       <div class="table-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <div class="col-sm-6">
          <a class="btn  pull-right btn-success howler" href="<?=base_url('superadmin/'.$viewname."/".$plan_id);?>" title="Back to List">Back to List</a>
          </div>
         </div>
         <div id="common_div">
...
<script>
    ...
    {   
		if(id.length > 50)
			id = id.substr(0, 50)+'...';
		$.confirm({'title': 'CONFIRM','message': " <strong> Are you sure want to add "+unescape(id)+" to list "+"<strong>?</strong>",'buttons': {'Yes': {'class': '',
		'action': function(){
	...
	function active_all_plans(name)
		{
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('superadmin_base_url').$viewname."/".$plan_id.'/ajax_Active_all';?>",
			dataType: 'json',
			//async: false,
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>superadmin/<?=$viewname?>/<?php echo $plan_id; ?>/view_archive/"+data,
					data: {
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search(allflag)
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>superadmin/<?=$viewname?>/<?php echo $this->router->uri->segments[3]; ?>/view_archive/"+uri_segment,
				data: {
...
$('body').on('click','.view_contacts_btn',function(e){
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('superadmin_base_url').'interaction_plans/view_contacts_of_interaction_plan';?>",
