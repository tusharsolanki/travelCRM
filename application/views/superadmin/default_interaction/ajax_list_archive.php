...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
            ...
            <th class="hidden-xs hidden-sm sorting_disabled text-center" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade" width="10%"><?php echo $this->lang->line('common_label_action')?></th>
          ...
          <?php if(!empty($datalist) && count($datalist)>0){
					...
					<?php if($row['start_type'] == '1'){
							...
							<td class=""><?php echo $row['admin_name'];?></td>
							<td class="text-center">
							<a data-toggle="modal" class="text_color_red text_size view_contacts_btn" href="#basicModal" data-id="<?=!empty($row['interaction_plan_id'])?$row['interaction_plan_id']:'';?>" title="Assigned Contacts">
							...
							<td class="hidden-xs hidden-sm text-center">
									<!--	<a class="btn btn-xs btn-success" href="<?= $this->config->item('superadmin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
										...
										<a class="btn btn-xs btn-success" href="javascript:void(0);" onclick="active_plan_single('<?php echo  $row['id'] ?>','<?php echo  rawurlencode(ucfirst(strtolower($row['description']))) ?>');" title="Un-Archive">Un-Archive</a> 
