          ...
          <thead>
           <tr role="row">
            ...
            <th width="29%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'email_id'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('email_id','<?php echo $sorttypepass;?>')"><?=$this->lang->line('common_label_email')?></a></th>
             ...
             <th width="24%" class="hidden-xs hidden-sm sorting_disabled text-center" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade"><?php echo $this->lang->line('common_label_action')?></th>
           </tr>
           </thead>
						...
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<td class="hidden-xs hidden-sm "><?=!empty($row['email_id'])?$row['email_id']:'';?></td>				
                      		<td class="hidden-xs hidden-sm text-center">
                                    <?php /*
							<a title="Edit Admin Rights" class="btn btn-xs btn-success" href="<?= $this->config->item('superadmin_base_url').$viewname; ?>/edit_right/<?= $row['id'] ?>"> Edit Rights</a> &nbsp;
                                      */ ?>
...
<? } ?>
							<a title="Edit Admin" class="btn btn-xs btn-success" href="<?= $this->config->item('superadmin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
                                                        <button class="btn btn-xs btn-primary" title="Delete Admin" onclick="deletepopup1('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['admin_name']))) ?>');"><i class="fa fa-times"></i></button> &nbsp;
                                                        <!--<a title="<?=$this->lang->line('mls_settings')?>" class="btn btn-xs btn-success" href="<?= $this->config->item('superadmin_base_url').$viewname; ?>/edit_mls_settings/<?= $row['id'] ?>"><?=$this->lang->line('mls_settings')?></a>-->
