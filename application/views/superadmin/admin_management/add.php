			   ...
			   ?>" data-required="true">
                    ...
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        ...
                        (Admin email id can not be edited.)
                        <?php } ?>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        <label for="text-input">
                          <?=$this->lang->line('common_label_password');?>
                          <span class="val">*</span></label>
                    ...
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        <label for="text-input">
                          <?=$this->lang->line('common_label_cpassword');?>
                          <span class="val">*</span></label>
                    ...
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        ...
                        <?=!empty($editRecord[0]['address'])?$editRecord[0]['address']:''?>
                        </textarea>
                      </div>
                    </div>
                    <div class="row">
                    ...
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        <label for="text-input"> No of Users Allowed </label>
                        <input type="text" placeholder="" name="number_of_users_allowed" id="number_of_users_allowed" class="form-control parsley-validated"  value="<?=!empty($editRecord[0]['number_of_users_allowed'])?$editRecord[0]['number_of_users_allowed']:''?>" onkeypress="return isNumberKey(event);" />
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        ...
                        <input id="user_license_no" name="user_license_no" class="form-control parsley-validated" type="text" value="<?php if(!empty($editRecord[0]['user_license_no'])){ echo htmlentities($editRecord[0]['user_license_no']); }?>" placeholder="License No">
                      </div>
                    </div>
                    <?php /* ?>
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        ...
                        <input id="mls_agent_id" name="mls_agent_id" class="form-control parsley-validated" type="text" value="<?php if(!empty($editRecord[0]['mls_user_id'])){ echo htmlentities($editRecord[0]['mls_user_id']); }?>" placeholder="<?=$this->lang->line('mls_agent_id');?>">
                      </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-12 form-group">
                            <label for="text-input"><?=$this->lang->line('mls_firm_id');?></label>
                            <input id="mls_firm_id" name="mls_firm_id" class="form-control parsley-validated" type="text" value="<?php if(!empty($editRecord[0]['mls_firm_id'])){ echo htmlentities($editRecord[0]['mls_firm_id']); }?>" placeholder="<?=$this->lang->line('mls_firm_id');?>" onkeypress="return isNumberKey(event)">
                        </div>
                    </div>
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        <label for="text-input">
                          ...
                          <select class="form-control parsley-validated" id="mls_id" name="mls_id[]" multiple="multiple">
                           <?php if(!empty($mlslistdata)){
                                    foreach($mlslistdata as $row){ ?>
                              <option value="<?=$row['id']?>" <?php if(!empty($assign_mls) && in_array($row['id'],$assign_mls)) { echo "selected=selected";}?> <?php if(!empty($child_assigned) && in_array($row['id'],$child_assigned)) { echo "disabled=disabled";}?> >
                                        <?=ucwords($row['mls_name'])?>
                                        </option>
                                    <?php } ?>
                            <?php } ?>
                           </select>
                           <?php foreach($child_assigned as $value) { ?>
                              <input type='hidden' name='mls_id[]' value=<?=$value?> />
                           <?php } ?>
                      </div>
                    </div>
                    <?php */ ?>
                  ...
                  <div class="col-sm-12 col-lg-7">
                    <div class="row">
                      <div class="col-sm-12 form-group">
                        <label for="text-input">Time Zone</label>
                        <select class="form-control parsley-validated" name='timezone' id='timezone' data-required="true">
                          <!--<option value="">Select Time Zone</option>-->
                          <?php if(!empty($timezone_list)){
                            if(!empty($editRecord[0]['timezone']))
                            {
                    		foreach($timezone_list as $key=>$row)
				{ ?>
                          <option value="<?=$row['zone']?>" <?php if(!empty($editRecord[0]['timezone']) && $editRecord[0]['timezone'] == $row['zone']) echo "selected=selected"; ?> >
                          <?=$row['zone'];?>
                          |
                          <?=$row['diff_from_GMT'];?>
                          </option>
                          <?php }
                            ...
                            else
                            {
                                foreach($timezone_list as $key=>$row)
				{ ?>
                          <option value="<?=$row['zone']?>" <?php if($row['zone'] == 'America/New_York') echo "selected=selected"; ?> >
                          <?=$row['zone'];?>
                          |
                          <?=$row['diff_from_GMT'];?>
                          </option>
...
function check_twilio_number(twilio_number)
{
	$.ajax({
		type: "POST",
		url: "<?php echo $this->config->item('superadmin_base_url').$viewname.'/check_twilio_number';?>",
		dataType: 'json',
		async: false,
...
$("select#mls_id").multiselect({
		header: "Select MLS",
		noneSelectedText: "Select MLS",
		selectedList: 0
}).multiselectfilter();
</script> 
