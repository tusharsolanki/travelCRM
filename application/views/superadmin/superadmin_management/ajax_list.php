...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
            ...
            <th width="28%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'email_id'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('email_id','<?php echo $sorttypepass;?>')"><?=$this->lang->line('common_label_email')?></a></th>
			
             <th width="21%" class="hidden-xs hidden-sm sorting_disabled" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade"><?php echo $this->lang->line('common_label_action')?></th> 
           </tr>
           </thead>
					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){
						...
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							?>
                            &nbsp;
							<a title="Edit Super Admin" class="btn btn-xs btn-success" href="<?= $this->config->item('superadmin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
							<?php if(!empty($row['id']) && $row['id'] != 1) { ?>
                            	<button class="btn btn-xs btn-primary" title="Delete Super Admin" onclick="deletepopup1('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['admin_name']))) ?>');"><i class="fa fa-times"></i></button>
                            <?php } ?>
          ...
          <div class="col-sm-6">
           <div class="dataTables_paginate paging_bootstrap float-right" >
            ...
            <label>
             ...
             <option value=""><?=$this->lang->line('label_superadmin_per_page');?></option>
