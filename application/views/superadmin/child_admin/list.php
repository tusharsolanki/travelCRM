<?php 
/*
	@Description: Child Admin Website
	@Author: Ami Bhatti
	@Date: 08-10-14
*/
...
?>
 <div id="content">
  <div id="content-header">
   <h1><?=$this->lang->line('child_admin_header');?></h1>
  </div>
    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=$this->lang->line('child_admin_header');?></h3>
      </div>
      <div class="portlet-content">
       <div class="table_large-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
          ...
          <div class="col-sm-11">
           <div class="dataTables_filter" id="DataTables_Table_0_filter">
            <label>
			 ...
			 <button class="btn btn-secondary howler" title="Search Domain" data-type="danger" onclick="contact_search('changesearch');">Search</button>
			 <button class="btn btn-secondary howler" title="View All Domain" data-type="danger" onclick="clearfilter_contact();">View All</button>
            </label>
           </div>
          </div>
         </div>
         <div class="row dt-rt">
          <div class="col-sm-6">
           <button class="btn btn-danger howler" title="Delete Domain" data-type="danger" onclick="deletepopup1('0');"><?=$this->lang->line('child_admin_delete_head');?></button>
          </div>
          <div class="col-sm-6">
          <a title="<?=$this->lang->line('child_admin_add_head');?>" class="btn pull-right btn-secondary-green howler" href="<?=base_url('superadmin/'.$viewname.'/add_record');?>"><?=$this->lang->line('child_admin_add_head');?></a>
          </div>
         </div>
         <div id="common_div">
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>superadmin/<?=$viewname?>/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>superadmin/<?=$viewname?>/"+data,
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
	...
	function deletepopup1(id,name,domain)
	{      
			var boxes = $('input[name="check[]"]:checked');
			...
			else
			{
				...
				var msg = 'Are you sure want to delete website - '+name+' ('+domain+') It will delete all the content and data related to the website';
			}
		...
		if(status == 0)
				{
						...
						msg = 'Are you sure want to suspend domain ';
				}else
				{
						path = "<?= $this->config->item('superadmin_base_url').$viewname; ?>/publish_record/"+id;
						msg = 'Are you sure want to publish domain ';
				}
