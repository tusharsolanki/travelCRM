...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
  ...
  <tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> id="view_archive_<?php echo  $row['id'] ?>" >
      ...
      <? } ?>
      <td class="hidden-xs hidden-sm "><?=!empty($row['save_type_name'])?ucfirst($row['save_type_name']):'';?></td>
      ...
      <? if(in_array('letter_delete',$this->modules_unique_name) || in_array('envelope_delete',$this->modules_unique_name) || in_array('label_delete',$this->modules_unique_name)){ ?>
      <td class="hidden-xs hidden-sm text-center"><!--<a title="View Communication" class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url'); ?>interaction/<?= $row['id'] ?>"><i class="fa fa-search"></i></a> &nbsp; -->
