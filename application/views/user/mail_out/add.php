          ...
          <div class="portlet-content">
            ...
            <div class="col-sm-12">
              <div class="tab-content" id="myTab1Content">
                ...
                <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('user_base_url').'mail_out/mail_out_print'?>"  data-validate="parsley" novalidate>
...
/*							   
					...
					<div class="row">
                      ...
                      <div class="col-sm-12 added_contacts_list"><div class="row">
						<?php $this->load->view('user/mail_out/selected_contact_ajax')?>
                      </div></div>
                    </div>
                  </div>
                  <div class="row">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-sm-12 add_new_contact_popup">
          <div class="">
		  <?php $this->load->view('user/mail_out/add_contact_popup_ajax');?>
		  </div>
        </div>
      </div>
...
function loadData(loadType,loadId){
  $.ajax({
	...
	function contact_search()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/mail_out/search_contact_ajax/",
			data: {
			result_type:'ajax',searchtext:$("#search_contact_popup_ajax").val(),sortfield:$("#sort_by").val(),contact_status:$('#slt_contact_status').val(),contact_source:$('#slt_contact_source').val(),contact_type:$('#contact_type').val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val(),perpage:$("#perpage").val(),search_tag:$("#search_tag").val()
		},
		beforeSend: function() {
	...
	function addcontactstointeractionplan()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/mail_out/add_contacts_to_mail_out/",
			data: {
...
$('#template').change(function(){
	$.ajax({  
		type: "post",
		dataType: 'json',
		url:"<?=$this->config->item('user_base_url')?>mail_out/ajax_templatename/",
