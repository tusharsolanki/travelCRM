			...
			<div class="table_large-responsive">
				<div class="col-sm-12">
					...
					<li><input type="submit" class="btn btn-secondary" value="Print" onClick="Popup()" name="print" /></li><li>
					   <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('user_base_url').'mail_out/generate_pdf'?>" novalidate>
					...
					<li> <a class="btn btn-primary" href="<?php  echo $this->config->item('user_base_url').'mail_out';?>">Cancel</a></li>
					</ul>
				</div>
...
function selecttemplate()
{
    ...
    function Popup() 
    {
		...
		$.ajax({  
				type: "post",
				url:'<?=base_url('user/'.$viewname.'/insert_data')?>',
