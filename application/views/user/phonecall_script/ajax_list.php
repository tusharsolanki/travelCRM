...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
  <thead>
    <tr role="row">
    <?php if(!empty($this->modules_unique_name) && in_array('phone_call_scripts_delete',$this->modules_unique_name)){?>
      ...
      <th width="30%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'template_subject'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('template_subject','<?php echo $sorttypepass;?>')">
        <?=$this->lang->line('tasksubject_label_name')?>
        </a></th>
     <? if(in_array('phone_call_scripts_add',$this->modules_unique_name) || in_array('phone_call_scripts_edit',$this->modules_unique_name) || in_array('phone_call_scripts_delete',$this->modules_unique_name)){ ?>
      <th width="13%" class="hidden-xs hidden-sm sorting_disabled actionbtn" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade"><?php echo $this->lang->line('common_label_action')?></th>
      <? } ?>
      <th>&nbsp;</th>
    </tr>
  </thead>
  <tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <?
						...
						$content1 = !empty($row['email_message'])?$row['email_message']:'';
    ...
    <tr <? if($i%2==1){ ?>class="bgtitle" <? }
     ...
     <?php if(!empty($this->modules_unique_name) && in_array('phone_call_scripts_delete',$this->modules_unique_name)){?>
      <td class="hidden-xs hidden-sm "><?php if(!empty($this->modules_unique_name) && in_array('phone_call_scripts_add',$this->modules_unique_name)){?>
        ...
        <?php if(!empty($this->modules_unique_name) && in_array('phone_call_scripts_edit',$this->modules_unique_name)){?>
      ...
      <td>
		  ...
		  <?=!empty($row['calling_script'])?$row['calling_script']:'';?>
        </div>
        </td>
    </tr>
    <?php } } else {?>
