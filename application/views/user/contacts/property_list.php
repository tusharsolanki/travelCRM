					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<td class="hidden-xs hidden-sm "><a target="_blank" class="" title="Edit Record" href="<?= $this->config->item('user_base_url').'listing_manager'; ?>/edit_record/<?= $row['id'] ?>">
							<?=!empty($row['address'])?ucwords($row['address']):'';?></a></td>
							<td class="hidden-xs hidden-sm "><a target="_blank" class="" title="Edit Record" href="<?= $this->config->item('user_base_url').'listing_manager'; ?>/edit_record/<?= $row['id'] ?>">
							<?=!empty($row['city'])?ucwords($row['city']):'';?></a></td>
                            ...
                            <td class="hidden-xs hidden-sm "><a target="_blank" class="" title="Edit Record" href="<?= $this->config->item('user_base_url').'listing_manager'; ?>/edit_record/<?= $row['id'] ?>"><?=!empty($row['my_price'])?$row['my_price']:'';?></a>
