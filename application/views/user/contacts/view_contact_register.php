<?php if(isset($sortby) && $sortby == 'asc'){ $sorttypepass = 'desc';}else{$sorttypepass = 'asc';}?>
 <table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                        ...
                                        $i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                                          foreach($editRecord as $er){?>
                                            <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
                                                <td class="hidden-xs hidden-sm "><?php if(!empty($er['contact_name'])){ echo ucfirst(strtolower($er['contact_name'])); }else{ echo "-"; } //if(!empty($er['first_name'])){ echo $er['first_name']; }else{ echo "-"; }?><?php //if(!empty($er['middle_name'])){ echo $er['middle_name']; }else{ echo "-"; }?> <?php //if(!empty($er['last_name'])){ echo $er['last_name']; }else{ echo "-"; }?></td>
                              ...
                              <?php } } else {?>
                              <tr>
                                <td colspan="5" align="center"><?=$this->lang->line('user_general_noreocrds')?></td>
                              </tr>
	...
	function delete_all(id)
		{
			...
			success: function(data){
				$.ajax({
					...
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext_cr").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
