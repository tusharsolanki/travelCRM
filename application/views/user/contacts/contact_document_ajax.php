   ...
   <?php
		...
		$i=1;
			...
			<tr <? if($i%2==1){ ?>class="bgtitle delete_document_trans_record<?= $row['id'] ?>" <? }?> > 
				...
				<a href="javascript:void(0);" class="btn btn-xs btn-primary" title="Delete" onclick="ajaxdeletetransdata('delete_document_trans_record','<?= $row['id'] ?>');"><i class="fa fa-times"></i></a></td>
				<?php } ?>
			</tr>
	<?php } }else{?>
			<tr>
