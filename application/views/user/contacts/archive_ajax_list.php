<?php 
    /*
        @Description: Usert contact list
        @Author: Niral Patel
        @Date: 07-05-14
    */
					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							<td class="">
                              <div class="text-center">
							   <? if(!empty($row['created_by']) && ((is_array($user_session['agent_id_array']) && in_array($row['created_by'],$this->user_session['agent_id_array'])) || $row['created_by'] == $row['assign_to'])){?>
