					  ...
					  ?>
						<tr class="<? if($i%2==1){ echo 'bgtitle'; }?> <?php if($row['is_sent_to_all'] == '0') {  echo 'new_red_class'; } ?>" > 
							...
							if($row['is_draft'] == '0' && strtotime(date('Y-m-d H:i:s')) > strtotime($datetime)) { ?>
									<a class="" href="<?= $this->config->item('user_base_url').$viewname; ?>/view_data/<?= $row['id'] ?>" title="View" >
                                    <?
                            ...
                            <td class="hidden-xs hidden-sm ">
								<?php 
								if($row['is_draft'] == '0' && strtotime(date('Y-m-d H:i:s')) > strtotime($datetime)) { ?>
									<a class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url').$viewname; ?>/view_data/<?= $row['id'] ?>" title="View" > <i class="fa fa-envelope"></i></a>
								<?php } ?>
                           ...
                           <? if(!empty($chk3))
							...
							<a class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>" title="Copy"><i class="fa fa-copy copyicon4"></i></a> &nbsp; <? }?>
                             <? } if(!empty($chk1))
							{?>
                                        <?php if(($row['is_draft'] == 1) || ($row['is_draft'] == 0 && strtotime($datetime) >= strtotime(date('Y-m-d H:i:s')))) { ?>
										<a class="btn btn-xs btn-success copyicon5" href="<?= $this->config->item('user_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>" title="Edit Social Campaign"><i class="fa fa-pencil"></i></a> &nbsp; 
                                        <?php } } ?>
										<? if(!empty($chk2))
							{?>
