					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr class="<? if($i%2==1) echo 'bgtitle '; if(strtotime($row['task_date']) < strtotime($dt)) echo 'new_red_class'; ?>" > 
                            ...
                            <?php
							if(isset($row['is_subscribe']) && $row['is_subscribe'] == '1'){?>
                           ...
                           <?php
                            ...
                            <!--<input type="button" class="btn btn-xs btn-success" onclick="resend_mail('<?=$row['id']?>','<?=$row['contact_id']?>')" value="Send" title="Send">-->
                            <?php }?>&nbsp;
										<button class="btn btn-xs btn-primary" title="Delete Record" onclick="deletepopup1('<?php echo  $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['contact_name']))) ?>');"><i class="fa fa-times"></i></button>
							</td>
                          </tr>
                          <div id="temp_desc_<?php echo  $row['id'] ?>" style="display:none;"><?=!empty($row['email_message'])?ucfirst($row['email_message']):''?></div>
          <?php } ?>
