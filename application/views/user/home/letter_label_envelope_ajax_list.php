...
?>
 ...
 <form class="form parsley-form" enctype="multipart/form-data" name="letter_label_envelope" id="letter_label_envelope" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('user_base_url').'dashboard/mail_out' ?>"  data-validate="parsley" novalidate target="_blank">
...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
            ...
            <th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'task_date'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version">Template</th>
					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){
						...
						<tr class="<? if($i%2==1) echo 'bgtitle '; if(strtotime($row['task_date']) < strtotime($dt)) echo 'new_red_class'; ?>" > 
							<td class="">
                              <div class="text-center">
                                  <input type="checkbox" class="mycheckbox_1" name="check_1[]" value="<?=$row['interaction_master_interaction_id']?>">
							  </div>
                            </td>
                            ...
                            <a class="view_form_btn" data-toggle="modal" title="Template"  href="#template_details" data-id="<?php echo  $row['id'] ?>">
							 ...
							 $category_name ='';
							  ...
							  elseif(!empty($row['interaction_type']) && $row['interaction_type'] == 5)
							  {
								...
								echo $category_name = $row['letter_category'];
							  }
							?>
                            </a>
                            </td>
							...
							?>
                            	...
                            	<input type="submit" class="btn btn-xs btn-success" value="Complete" name="mail_out" title="Complete" onclick="finaldata(<?=$row['interaction_master_interaction_id']?>)" />
                            </td>
							<td class="hidden-xs hidden-sm text-center">
