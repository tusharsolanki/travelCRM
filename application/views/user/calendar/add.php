...
<?php
	...
	$user_session_array_array = $this->session->userdata('user_session_array_user');
	
	 $type=$user_session_array_array['type'];
	 $access=$user_session_array_array['access'];
	 $user_type = $this->router->uri->segments[1]; 
  ...
  <div id="content-container">
   <div class="">
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i>Calendar - My Appointment    <label><input type="radio" name="is_public" id="is_public" checked="checked" value="0" onclick="gridcontainer_refresh();" /> My Calendar &nbsp;&nbsp;  </label> <label><input type="radio" name="is_public" id="is_public" value="1" onclick="gridcontainer_refresh();" /> Public Calendar</label></h3>
      </div>
      <!-- /.portlet-header -->
    ...
    $(document).ready(function() {
      ...
      var DATA_FEED_URL = "<?=base_url('user/calendar')?>";
      var op = {
        view: view,
      ...
      function Edit(data)
      {
        ...
        /*var eurl="<?=$this->config->item('wdcalendar')?>edit.php?agentid=<?=!empty($agentid)?$agentid:''?>&agentname=<?=!empty($agentname)?$agentname:''?>&id={0}&start={2}&end={3}&isallday={4}&title={1}";*/
				var eurl='<?=base_url()?>user/calendar/edit_calender/?id={0}&start={2}&end={3}&isallday={4}&title={1}&type='+type+'&access='+acc;
        if(data)
        {
      ...
      $("#faddbtn").click(function(e) {
        ...
        var url ="<?=base_url('user/calendar/edit_calender')?>";
	...
	<input type="hidden" id="baseurl" value="<?=base_url($user_type).'/'?>" />
      ...
      <?php if(!empty($this->modules_unique_name) && in_array('calendar_add',$this->modules_unique_name)){?>
       ...
       <? } ?> 
		...
		<div id="showreflashbtn" class="fbutton pull-right">
            ...
            <?php /*<a href="https://www.google.com/accounts/AuthSubRequest?next=<?=base_url('user/calendar/google_connection')?>
&scope=https://www.google.com/calendar/feeds/&secure=&session=1">Fetch events from Google Calendar</a>*/?>
                    <a href="<?=base_url('user/calendar/google_connection')?>">Fetch events from Google Calendar</a>
			<? } ?>
			</div>
        </div>
