...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
  <thead>
    <tr role="row">
      ...
      <th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'platform'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('platform','<?php echo $sorttypepass;?>')">
        <?=$this->lang->line('platform_label_name')?>
        </a></th>
       <? if(in_array('social_media_posts_add',$this->modules_unique_name) || in_array('social_media_posts_edit',$this->modules_unique_name) || in_array('social_media_posts_delete',$this->modules_unique_name)){ ?>
  ...
  <tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
      ...
      <? if(in_array('social_media_posts_add',$this->modules_unique_name) || in_array('social_media_posts_edit',$this->modules_unique_name) || in_array('social_media_posts_delete',$this->modules_unique_name)){ ?>
      <td class="hidden-xs hidden-sm"><?php if(!empty($this->modules_unique_name) && in_array('social_media_posts_add',$this->modules_unique_name)){?>
        <a title="Copy Record" class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy copyicon5"></i></a> &nbsp;
        <? } ?>
		<?php if(!empty($this->modules_unique_name) && in_array('social_media_posts_edit',$this->modules_unique_name)){?>
         <a title="Edit Record" class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp;
        <? } ?>
        <?php if(!empty($this->modules_unique_name) && in_array('social_media_posts_delete',$this->modules_unique_name)){?>
