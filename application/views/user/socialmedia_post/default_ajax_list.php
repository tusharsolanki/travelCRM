...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
            ...
            <th width="20%" class="hidden-xs hidden-sm <?php if(isset($default_sortfield) && $default_sortfield == 'template_subject'){if($default_sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="default_applysortfilte_contact('template_subject','<?php echo $sorttypepass;?>')"><?=$this->lang->line('tasksubject_label_name')?></a></th>

			<th width="20%" class="hidden-xs hidden-sm <?php if(isset($default_sortfield) && $default_sortfield == 'platform'){if($default_sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="default_applysortfilte_contact('platform','<?php echo $sorttypepass;?>')"><?=$this->lang->line('platform_label_name')?></a></th>
			<th>&nbsp;</th>
           ...
           <?php if(!empty($default_datalist) && count($default_datalist)>0){
					...
					?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
                            ...
                            <td class="hidden-xs hidden-sm " id="temp_platform_<?php echo  $row['id'] ?>"><?=!empty($row['platform'])?ucfirst(strtolower($row['platform'])):'';?>
                            </td>
							 <td>
                             <div id="temp_desc_<?php echo  $row['id'] ?>" style="display:none;">
									<?=!empty($row['post_content'])?$row['post_content']:'';?>
								</div>
                             </td>
						</tr>
          <?php } } else {?>
