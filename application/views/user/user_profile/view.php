        ...
        <div class="portlet">
          <div class="portlet-header">
            <h3><i class="fa fa-tasks"></i>
			 ...
			 <?php /*?> <span class="pull-right"><a title="Back" class="btn btn-secondary" href="<?php echo $this->config->item('user_base_url')?><?php echo $viewname.'/'.'livewire_configuration';?>"><?php echo $this->lang->line('common_back_title')?></a> </span> 
              <?php */?>
               <span class="pull-right"><a title="Back" class="btn btn-secondary" href="<?php echo $this->config->item('user_base_url')?><?php echo $viewname.'/edit_record';?>"><?php echo $this->lang->line('common_label_edit_profile')?></a> </span> 
                ...
                <div class="col-lg-7 col-xs-12">
                 <div class="add_emailtype">
                    ...
                    <div class="col-sm-5">
                      ...
                      <label for="text-input">
                      	<?php foreach($user_type as $row){
								if($row['id'] == $editRecord[0]['user_type']){ ?>
                  ...
                  <div class="add_address_div add_emailtype">
                    ...
                    <div class="delete_address_trans_record<?=$rowtrans['id']?> padding-top-10 clear autooverflow">
                      ...
                      <div class="col-sm-3 columns">
                        ...
                        <?php if(!empty($rowtrans['address_type']) && $rowtrans['address_type'] == $row['id']){ echo $row['name']; }else{ echo '-';}?>
                        <?php } ?>
                        <?php } ?>
                      </div>
                  ...
                  <div class="add_emailtype1">
                    ...
                    <div class="col-sm-5">
                    <label for="text-input">
                                <?=!empty($user_info[0]['twilio_number'])?$user_info[0]['twilio_number']:'-'?>
                    </label>
                    </div>
                    <div class="col-sm-5">
                      <label for="text-input">
                    ...
                    <div class="col-sm-5">
                      <label for="text-input">
                        <?=!empty($user_info[0]['twilio_account_sid'])?$user_info[0]['twilio_account_sid']:'-'?>
                      </label>
                    </div>
                    ...
                    <div class="col-sm-5">
                    <label for="text-input">
                                <?=!empty($user_info[0]['twilio_auth_token'])?$user_info[0]['twilio_auth_token']:'-'?>
                    </label>
                    </div>
                    <div class="col-sm-5">
                      <label for="text-input">
                    ...
                    <div class="col-sm-5">
                    <label for="text-input">
                                <?=!empty($user_info[0]['phone'])?$user_info[0]['phone']:'-'?>
                    </label>
                    </div>
                  </div>
                  <?php */ ?>
                    ...
                    <div>
			 		...
			 		foreach($website_trans_data as $rowtrans){ ?>
                        <div class="delete_website_trans_record<?=$rowtrans['id']?> padding-top-10 clear autooverflow">
                          <div class="col-sm-3">
                            <?php if(!empty($rowtrans['website_type'])){ echo $rowtrans['website_type']; }else{ echo "-";}?>
                          </div>
			 		...
			 		foreach($profile_trans_data as $rowtrans){ ?>
                        <div class="delete_social_trans_record<?=$rowtrans['id']?> padding-top-10 clear autooverflow">
                          <div class="col-sm-3">
                            ...
                            <?php if(!empty($rowtrans['profile_type']) && $rowtrans['profile_type'] == $row['id']){ echo $row['name']; }else{ echo '-';}?>
                            <?php } ?>
                            <?php } ?>
                          </div>
                          <div class="col-sm-5 form-group">
              ...
              <p> <span class="txt">&nbsp;</span>
                <?php 	
						  if(!empty($user_info[0]['brokerage_pic']) && file_exists($this->config->item('broker_big_img_path').$user_info[0]['brokerage_pic'])){
							?>
                <img  width="100" height="100" id="uploadPreview1" src="<?=$this->config->item('broker_upload_img_small')?>/<?=(!empty($user_info[0]['brokerage_pic'])?$user_info[0]['brokerage_pic']:'');?>"/>
                <? } else{
				if(!empty($user_info[0]['brokerage_pic']) && file_exists($this->config->item('broker_small_img_path').$user_info[0]['brokerage_pic'])){
				?>
                <img  width="100" height="100" id="uploadPreview1" src="<?=$this->config->item('broker_upload_img_big')?>/<?=(!empty($user_info[0]['brokerage_pic'])?$user_info[0]['brokerage_pic']:'');?>" />
                <?
				}else{
				?>
                <img id="uploadPreview1" class="noimage" src="<?=base_url('images/no_image.jpg')?>"  width="100" />
