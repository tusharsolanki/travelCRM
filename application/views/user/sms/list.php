       ...
       <div class="table_large-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <div class="col-sm-11">
           <div class="dataTables_filter" id="DataTables_Table_0_filter">
            <label>
			 <a href="<?=base_url('user/'.$viewname.'/queued_list');?>" class="btn btn-secondary howler" title="Queued SMS" > Queued SMS </a>
			  <a href="<?=base_url('user/'.$viewname.'/all_sent_sms');?>" class="btn btn-secondary howler" title="Sent SMS" > Sent SMS </a>
            </label>
           </div>
          </div>
          <div class="col-sm-12">
         ...
         <div class="row dt-rt">
          ...
          <a class="btn  pull-right btn-secondary-green howler" href="<?=base_url('user/'.$viewname.'/add_record');?>" title="<?=$this->lang->line('label_add_sms_campaign');?>"><?=$this->lang->line('label_add_sms_campaign');?></a>
            <? } ?>
          </div>
         </div>
         <div id="common_div">
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/sms/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/sms/",
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
