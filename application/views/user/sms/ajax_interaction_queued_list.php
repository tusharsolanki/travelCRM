					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($interaction_plan as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
                            ...
                            <a href="<?php echo $this->config->item('user_base_url').$viewname?>/interaction_plan_queued_list/<?=$row['id']?>">
							<?=!empty($row['plan_name'])?htmlentities(ucfirst(strtolower($row['plan_name']))):'';?>  </a>
							</td>			
                          </tr>
          <?php } } else {?>
		  <tr>
