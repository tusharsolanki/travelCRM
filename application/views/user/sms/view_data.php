        ...
        <div class="tab-content" id="myTab1Content">
         ...
         <div class="tab-pane fade in active" id="home">
          ...
          <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('user_base_url')?><?php echo $path?>" novalidate>
		   ...
		   <div class="row">
             ...
             <label for="text-input"><?=!empty($datalist[0]['first_name'])?ucfirst(strtolower($datalist[0]['first_name'])):''?> <?=!empty($datalist[0]['last_name'])?ucfirst(strtolower($datalist[0]['last_name'])):''?><?php if(!empty($datalist[0]['phone_no'])) echo '('.$datalist[0]['phone_no'].')';?></label>
             </div>
            </div>
            <div class="row">
             <div class="col-sm-12">
         ...
         <div class="row">
          ...
          <div class="form-group">
                  <label for="select-multi-input">
                 	SMS Message : 
	  				...
	  				<?=!empty($datalist[0]['sms_message'])?ucfirst(strtolower($datalist[0]['sms_message'])):'';?>
                </div>
               </div>
          <div class="col-sm-12 pull-left text-center margin-top-10">
			...
			{
			?>
				<a class="btn btn-primary" onclick="history.go(-1)" href="<?php echo $this->config->item('user_base_url').$viewname."/all_sent_sms/".$pagingid; ?>" title="Back">Close</a>
			<?php } else { ?>
			<a class="btn btn-primary" onclick="history.go(-1)" href="<?php echo $this->config->item('user_base_url').$viewname."/sent_sms/".$this->uri->segment(4)."/".$pagingid;?>" title="Back">Close</a>
			<?php } ?>
         </div>
          </form>
