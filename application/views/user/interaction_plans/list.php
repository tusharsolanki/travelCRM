    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       ...
       &nbsp;&nbsp;
	   <?php if(!empty($user_session['user_type'])){ ?>
       <a class="btn  pull-right btn-success howler mrg33" title="Assign Communication" href="<?=base_url('user/'.$viewname.'/assign_plan');?>">Assigned Communication</a>
       <? } ?>
      </div>
      <!-- /.portlet-header -->
       ...
       <div class="">
		...
		<?php if(!empty($msg)){?>
           ...
           <div class="dataTables_filter" id="DataTables_Table_0_filter">
            ...
            <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?=!empty($uri_segment)?$uri_segment:'0'?>">
             <input class="" type="text" name="searchtext" id="searchtext" aria-controls="DataTables_Table_0" placeholder="Search..." value="<?=!empty($searchtext)?htmlentities($searchtext):''?>">
                        <button class="btn btn-success howler" data-type="btn-success" onclick="contact_search('changesearch');" title="Search">Search</button>
			<button class="btn btn-success howler" data-type="btn-success" onclick="clearfilter_contact();" title="View All" title="View All">View All</button>
		</label>
           </div>
          </div>
         </div>
         <div class="row dt-rt">
		  ...
		  <?php if(!empty($this->modules_unique_name) && in_array('communications_delete',$this->modules_unique_name)){?>
		  <a title="View Archive" class="btn  pull-right btn-success howler margin-left-5px" href="<?=base_url('user/'.$viewname.'/view_archive');?>">View Archive</a>
           <? } ?>
           <?php if(!empty($this->modules_unique_name) && in_array('communications_add',$this->modules_unique_name)){?>
          <a title="Add Communication" class="btn  pull-right btn-secondary-green howler" href="<?=base_url('user/'.$viewname.'/add_record');?>">Add Communication</a>
           <? } ?>   
          </div>
         </div>
...
<script>
    ...
    {   
			if(id.length > 50)
			{
				...
				var msg = 'Are you sure want to archive "'+id +'"';
			}
			else
			{
				var msg = 'Are you sure want to archive "'+unescape(id) +'"';
			}
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search(allflag)
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>user/interaction_plans/"+uri_segment,
				data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/interaction_plans/",
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
