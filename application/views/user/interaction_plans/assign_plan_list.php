...
?>
 <div id="content">
  <div id="content-header">
   <h1><?=$this->lang->line('assign_interaction_plans_header');?></h1>
    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=$this->lang->line('assign_interaction_plans');?></h3>
      </div>
      <!-- /.portlet-header -->
		...
		<?php if(!empty($msg)){?>
          ...
          <div class="col-sm-12">
           ...
           <input type="text" name="searchtext" id="searchtext" aria-controls="DataTables_Table_0" placeholder="Search..." />
			<button class="btn btn-success howler" data-type="danger" onclick="contact_search();" title="Search" title="Search">Search</button>
         ...
         <!--<div class="row dt-rt">
		  ...
		  <a title="Add Communication" class="btn  pull-right btn-success howler margin-left-5px" href="<?=base_url('user/'.$viewname.'/add_record');?>">Add Communication</a>
		  <a title="View Archive" class="btn  pull-right btn-success howler" href="<?=base_url('user/'.$viewname.'/view_archive');?>">View Archive</a>
          
          </div>
         </div>-->
         <div id="common_div" class="table_large-responsive">
         <?=$this->load->view('user/'.$viewname.'/assign_plan_ajax_list')?>
         </div>
        </div>
       </div>
       <!-- /.table-responsive --> 
	...
	function active_all_plans(name)
		{
			...
			success: function(data){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/interaction_plans/assign_plan/"+data,
					data: {
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search()
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>user/interaction_plans/assign_plan/",
				data: {
				result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
			},
			beforeSend: function() {
