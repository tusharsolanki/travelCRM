                  ...
                  <div class="row dt-rb form">
                    ...
                    <div class="col-sm-6">
                      ...
                      <div class="col-sm-12 added_contacts_list table-responsive">
						...
						<?php $this->load->view('user/interaction_plans/selected_contact_ajax')?>
...
$(document).ready(function(){
	...
	function contact_search()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/interaction_plans/search_contact_ajax/",
			data: {
			result_type:'ajax',searchtext:$("#search_contact_popup_ajax").val(),contact_status:$('#slt_contact_status').val(),contact_source:$('#slt_contact_source').val(),contact_type:$('#contact_type').val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val(),perpage:$("#perpage").val(),search_tag:$("#search_tag").val()
		},
		beforeSend: function() {
	...
	function addcontactstointeractionplan()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/interaction_plans/add_contacts_to_interaction_plan/",
			data: {
...
<script type="text/javascript">
	...
	$('body').on('click','.pause_interaction_plan',function(e){
		...
		'action': function(){
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('user_base_url').$viewname.'/pause_interaction_plan';?>",
	...
	$.confirm({
		...
		'action': function(){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo $this->config->item('user_base_url').$viewname.'/stop_interaction_plan';?>",
				data: {'interaction_plan':planid},
				success: function(html){
					//$(".view_contact_popup").html(html);	
					window.location.reload();
	...
	$('body').on('click','.play_interaction_plan',function(e){
		...
		'action': function(){
			...
			else
			{
				$.ajax({
				type: "POST",
				url: "<?php echo $this->config->item('user_base_url').$viewname.'/play_interaction_plan';?>",
				data: {'interaction_plan':planid},
				success: function(html){
					//$(".view_contact_popup").html(html);	
					//contact_search();
