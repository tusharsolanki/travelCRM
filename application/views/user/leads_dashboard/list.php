     ...
     <div class="portlet">
      ...
      <div class="portlet-content">
       <div class="">
          ...
          <div class="col-sm-11">
           <div class="dataTables_filter" id="DataTables_Table_0_filter">
            <label>
                ...
                <input type="text" name="searchtext" id="searchtext" aria-controls="DataTables_Table_0" placeholder="Search..." title="Search Text" value="<?=!empty($searchtext)?htmlentities($searchtext):''?>">
        ...
        <div class="col-sm-6">
             <?php /*<a class="btn  pull-right btn-success howler margin-left-5px" title="Lead Distribution" href="<?=base_url('user/user_rr_weightage');?>">Lead Distribution</a> */ ?>
             ...
             <a class="btn  pull-right btn-success howler" title="<?php echo $this->lang->line('leads_dashboard_assigncomm_plan');?>" href="<?=base_url('user/joomla_assign/joomla_assign_home');?>"><?php echo $this->lang->line('leads_dashboard_assigncomm_plan');?></a>
             <? } } ?>
        </div>
...
$(function() {
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/leads_dashboard/"+uri_segment,
			data: {
			result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val(),allflag:allflag
                        ,registration_source:$("#sel_reg_source").val(),lead_type:$("#sel_lead_type").val()
		},
		beforeSend: function() {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/leads_dashboard/"+data,
					data: {
	...
	{      
	   ...
	   'action': function(){
        ...
        function change_contact_type(contact_type,contact_id)
        {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo $this->config->item('user_base_url').$viewname.'/change_contact_type';?>",
                dataType: 'json',
                async: false,
		 ...
		 function change_contact_category(contact_category,contact_id)
        {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo $this->config->item('user_base_url').$viewname.'/change_contact_category';?>",
                dataType: 'json',
                async: false,
...
$('body').on('click','.view_contacts_btn',function(e){
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('user_base_url').$viewname.'/view_contact_interaction_plan_list';?>",
			data: {'contact_id':id},
			success: function(html){
        ...
        function setcontact_session(contact_id,view_id)
        {
            $.ajax({
                ...
                success: function(html){
                    //window.location.href = "<?=base_url()?>user/contacts/view_record/"+contact_id;
                    window.location.href = "<?=base_url()?>user/leads_dashboard/view_record/"+contact_id;
                },
                error: function(jqXHR, textStatus, errorThrown) {
                        console.log(textStatus, errorThrown);
                        //$(".view_contact_popup").html('Something went wrong.');
