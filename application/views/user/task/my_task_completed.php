    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?php echo "Completed Task List";?></h3>
       <span class="float-right margin-top--15"><a href="<?php echo $this->config->item('user_base_url').$viewname.'/my_task';?>" class="btn btn-secondary" title="Back">Back</a> </span>
      </div>
      <!-- /.portlet-header -->
      ...
      <div class="portlet-content">
       <div class="table_large-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          <div id="common_div">
         		<?=$this->load->view('user/'.$viewname.'/mytask_completed_ajax_list')?>
         </div>
         </div>
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/task/my_completed_task/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/my_completed_task/",
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
