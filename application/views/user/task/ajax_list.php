...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
  <thead>
    <tr role="row">
      <?php if(!empty($this->modules_unique_name) && in_array('tasks_delete',$this->modules_unique_name)){?>
      ...
      <th width="15%" class="hidden-xs hidden-sm text-center"><?=$this->lang->line('completed_name')?>
        </a></th>
      <? if(in_array('tasks_add',$this->modules_unique_name) || in_array('tasks_edit',$this->modules_unique_name) || in_array('tasks_delete',$this->modules_unique_name)){ ?>
      <th width="11%" class="hidden-xs hidden-sm sorting_disabled text-center action" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade"><?php echo $this->lang->line('common_label_action')?></th>
      <? } ?>
    </tr>
  </thead>
  <tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
      ...
      <td class="hidden-xs hidden-sm text-center"><?php if($this->uri->segment(3) == 'completed_task') echo "Completed"; else {  ?>
        <input <?php if($row['is_completed'] == 1){?> checked="checked" <?php } ?> type="checkbox" id="iscompleted<?=$i;?>" value="<?php echo $row['id'];?>" name="iscompleted" onclick="iscompleted(this.value);" />
        <?php } 
                            $task_status = 1;
     ...
     <? if(in_array('tasks_add',$this->modules_unique_name) || in_array('tasks_edit',$this->modules_unique_name) || in_array('tasks_delete',$this->modules_unique_name)){ ?>
      <td class="hidden-xs hidden-sm text-center">
        ...
        <? } ?>
        <?php if(!empty($this->modules_unique_name) && in_array('tasks_edit',$this->modules_unique_name)){?>
      ...
      <div id="DataTables_Table_0_length" class="dataTables_length row pagignation_margin_right">
        <label>
          <select name="DataTables_Table_0_length" size="1" aria-controls="DataTables_Table_0" onchange="changepages();" id="perpage">
            <option value="">
            <?=$this->lang->line('label_tasks_per_page');?>
            </option>
