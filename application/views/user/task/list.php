    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=$this->lang->line('task_list_head');?></h3>
       ...
       <?php if(!empty($user_session['user_type']) && $user_session['user_type'] == '3'){ ?>
       <a class="btn  pull-right btn-success howler mrg36" title="My Task" href="<?=base_url('user/'.$viewname.'/my_task');?>">Assigned Task</a>
       <? } ?>
      </div>
      <!-- /.portlet-header -->
       ...
       <div class="table_large-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <div class="col-sm-6">
           <!--<a class="btn btn-secondary pull-right btn-success howler margin-left-5px" title="Completed Task" href="<?=base_url('user/'.$viewname.'/completed_task');?>">Completed Task</a>-->
           <?php if(!empty($this->modules_unique_name) && in_array('tasks_add',$this->modules_unique_name)){?>
          <a class="btn  pull-right btn-secondary-green howler" title="Add Task" href="<?=base_url('user/'.$viewname.'/add_record');?>">Add Task</a>
          <? } ?>
          </div>
         </div>
         <div id="common_div">
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/task/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/task/",
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
