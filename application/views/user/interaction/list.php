...
?>
 <div id="content">
  <div id="content-header">
   <h1><?=htmlentities(ucfirst(strtolower($list[0]['plan_name'])))?> : <?=$this->lang->line('interaction_header');?></h1>
  </div>
    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=htmlentities(ucfirst(strtolower($list[0]['plan_name'])))?> : <?=$this->lang->line('interaction');?></h3>
        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <div class="col-sm-6">
		   ...
		   <a class="btn  pull-right btn-success howler margin-left-5px" href="<?=base_url('user/'.$viewname."/".$plan_id.'/view_archive');?>" title="View Archive">View Archive</a>
          <?php if(isset($datalist[0]['plan_status']) && $datalist[0]['plan_status'] == 0){}else{ ?>
          <a class="btn  pull-right btn-secondary-green howler" href="<?=base_url('user/'.$viewname.'/add_record/'.$plan_id);?>" title="Add Action">Add Action</a>
		  <?php } ?>
          </div>
         </div>
         <div id="common_div">
...
<script>
	...
	{      
			...
			if(boxes.length == '0')
			{
			$.confirm({'title': 'Alert','message': " <strong> Please select action"+"<strong></strong>",'buttons': {'ok'	: {'class'	: 'btn_center alert_ok'}}});
			return false;}
			else
			{
	...
	function active_all_plans(name)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/interaction/<?php echo $plan_id.'/'; ?>"+data.page,
					data: {
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search()
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>user/interaction/<?php echo $this->router->uri->segments[3]; ?>",
				data: {
				result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
			},
			beforeSend: function() {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/interaction/<?php echo $plan_id; ?>",
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
