					...
					$i = 1;
                      foreach($attachment as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							?>
							<?=!empty($row['attachment_name'])?ucfirst($row['attachment_name']):'';?> </td>
							<td class="hidden-xs hidden-sm text-center">
								<a href="#" class="btn btn-xs btn-primary" onclick="deletepopup1('<?php echo $row['id'] ?>','<?php echo $row['attachment_name'] ?>','<?=$filesize?>');"><i class="fa fa-times"></i>
...
function delete_all(id,name,filesize)
{
	$.ajax({
		type: "POST",
		url: "<?php echo $this->config->item('user_base_url').'emails/ajax_delete_attachment';?>",
		data: {'single_remove_id':id,'attachment_name':name},
		success: function(data){
			filesizeCount = filesizeCount - filesize;
			$.ajax({
				type: "POST",
				url: "<?php echo $this->config->item('user_base_url').$viewname?>/attachmentlist",
				data: {
