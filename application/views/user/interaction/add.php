        ...
        <div class="col-md-12">
          ...
          <div class="portlet">
			...
			<?php if(!empty($editRecord[0]['interaction_plan_id'])){?>
			  <span class="pull-right"><a title="Back" class="btn btn-secondary" href="<?php echo $this->config->item('user_base_url')?><?php echo $viewname.'/'.$editRecord[0]['interaction_plan_id'];?>"><?php echo $this->lang->line('common_back_title')?></a> </span>
			  <?php }else {?>
			  <span class="pull-right"><a title="Back" class="btn btn-secondary" href="<?php echo $this->config->item('user_base_url')?><?php echo $viewname.'/'.$inte_plan_id;?>"><?php echo $this->lang->line('common_back_title')?></a> </span> 			<?php } ?>
            </div>
            <!-- /.portlet-header -->
            
            <div class="portlet-content">
              <div class="table-responsive">
			   <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('user_base_url')?><?php echo $path?>" data-validate="parsley" novalidate onsubmit="return validation();" >
                      ...
                      <div class="text-center pull-right"> 
					  		<?php if(!empty($editRecord)){ ?>
								<?php if(!empty($previous_interaction)){ ?>
									<a href="<?= $this->config->item('user_base_url').$viewname; ?>/edit_record/<?=$previous_interaction?>"><i class="fa fa-backward btn btn-prev-next-avail arrow"></i></a>
								<?php }else{ ?>
									<a href="javascript:void(0);"><i class="fa fa-backward btn btn-tertiary arrow"></i></a>
								<?php } ?>
								
								<?php if(!empty($next_interaction)){ ?>
									<a href="<?= $this->config->item('user_base_url').$viewname; ?>/edit_record/<?=$next_interaction?>"><i class="fa fa-forward btn btn-prev-next-avail arrow"></i></a>
								<?php }else{ ?>
...
<option value="<?php echo $interaction_type[$i]['id'];?>" <?php if(!empty($editRecord[0]['interaction_type']) && $editRecord[0]['interaction_type'] == $interaction_type[$i]['id']){ echo "selected=selected"; } ?>><?php echo $interaction_type[$i]['name'];?></option>
                        ...
                        <div class="col-xs-12">
                          <fieldset class="edit_main_div">
                            ...
                            <div class="col-xs-12 mrgtop1 form-group">
                              ...
                              <textarea name="txtarea_description" id="txtarea_description" placeholder="e.g. Description" class="form-control" data-required="true"><?php if(!empty($editRecord[0]['description'])){ echo htmlentities($editRecord[0]['description']); }?></textarea>
                            </div>
                            <div class="col-xs-12">
                              <label for="validateSelect">Assigned To:</label>
                              <select  class="form-control parsley-validated" name="slt_assigned" id="slt_assigned">
                                <?php if(!empty($user_list)){
										foreach($user_list as $row){ 
										$email = !empty($row['email_id'])?' ('.$row['email_id'].')':'';
										?>
                                			<option value="<?=$row['id']?>" <?php if(!empty($editRecord[0]['assign_to']) && $editRecord[0]['assign_to'] == $row['id']){ echo ' selected=selected '; }?> ><?php if($row['admin_name']!='') { echo ucwords($row['admin_name'].$email);}else{ echo ucwords($row['admin_name'].$email);}?></option>
								<?php 	}
									} ?>
                              </select>
                            </div>
                            <div class="col-xs-12">
                              ...
                              <div class="row ">
                                <div class="col-sm-1">
                                  <label>
            <input type="radio" name="rad_start_type" <?php if(!empty($editRecord[0]['start_type']) && $editRecord[0]['start_type'] == '1'){ echo 'checked="checked"'; }?> value="1" checked="checked" id="rad_start_type">
                                  </label> 
                                </div>
      ...
      <input type="text" class="form-control" id="txt_interaction_stat_1" value="<?php if(isset($editRecord[0]['number_count']) && $editRecord[0]['start_type'] == '1'){ echo $editRecord[0]['number_count'];}?>" name="txt_interaction_stat_1" placeholder="Number" onkeypress="return isNumberKey(event);" >
                              ...
                              <div class="row">
                                <div class="col-sm-1">
                                  <label>
                                    <input type="radio" name="rad_start_type"  value="2" <?php if(!empty($editRecord[0]['start_type']) && $editRecord[0]['start_type'] == '2'){ echo 'checked="checked"'; }?> id="rad_start_type_2">
                                  </label>
                                </div>
                                <div class="col-sm-3">
                                  ...
                                  <select class="form-control parsley-validated" name="slt_interaction_stat_2" id="slt_interaction_stat_2">
                                    <option value="">Action</option>
                                     <?php for($i=0;$i < count($interaction_list);$i++)
											{
												if(!isset($editRecord[0]['id']) || (!empty($editRecord[0]['id']) && $interaction_list[$i]['id'] != $editRecord[0]['id'])){
										?>
                              ...
                              <div class="row">
                                <div class="col-sm-1">
                                  <label>
                                    <input type="radio" name="rad_start_type"  value="3" <?php if(!empty($editRecord[0]['start_type']) && $editRecord[0]['start_type'] == '3'){ echo 'checked="checked"'; }?> id="rad_start_type_3">
                                  </label>
                                </div>
                                <div class="col-sm-4">
                                 	<input id="rad_start_type_date" name="rad_start_type_date" class="form-control parsley-validated customdatepickerinput" type="text" value="<?php if(!empty($editRecord[0]['start_date']) && $editRecord[0]['start_date'] != '0000-00-00' && $editRecord[0]['start_date'] != '1970-01-01'){ echo date($this->config->item('common_date_format'),strtotime($editRecord[0]['start_date'])); }?>" placeholder="Specific Date" readonly="readonly">
                                </div>
                              </div>
                            </div>
                            <div class="col-xs-12"  style="display:none;">
                              <label for="text-input" style="display:none;">Priority:</label>
                              <div class="row">
                                <div class="col-sm-3">
                                  <label>
                            ...
                            <div class="col-xs-12 mrgb2">
                              ...
                              <textarea class="form-control" placeholder="e.g. Notes"  name="txtarea_interaction_notes"><?php if(!empty($editRecord[0]['interaction_notes'])){ echo htmlentities($editRecord[0]['interaction_notes']); }?></textarea>
                            </div>
                          </fieldset>
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-xs-12">
                          <fieldset class="edit_main_div template_main_div">
                            ...
                            <div class="row">
							 <div class="col-sm-12">
							  <label for="text-input"><?=$this->lang->line('common_label_category');?><span class="mandatory_field add_star"></span></label>
							  </div>
							  <div class="col-sm-12">
							...
							<!--<div class="row">
								<div class="col-sm-12 topnd_margin">
									<a class="text_color_red text_size" target="_blank" title="Add Category" href="<?=$this->config->item('user_base_url').'marketing_library_masters/add_record'?>">
                            ...
                            <div class="col-xs-12 mrgb2">
                              <label for="validateSelect">Template:</label><span class="mandatory_field add_star"></span>
							...
							<div class="">
                                ...
                                <a href="#basicModal" class="text_color_red text_size add_new_template" id="basicModal" data-toggle="modal"><i class="fa fa-plus-square"></i> Add Template </a>
									<!--<a class="text_color_red text_size add_new_template" title="Add Template" href="javascript:void(0);">
					...
					<div class="col-lg-4 col-sm-12 show_email_sms_div">
                        ...
                        <div class="col-xs-12">
                          <fieldset class="edit_main_div">
                            <legend class="edit_title">Email/SMS/Bomb Bomb Emails</legend>
							 ...
							 <div class="col-xs-12 mrgtop1">
                              ...
                              <div class="row">
								...
								<div class="col-sm-4">
                                  <label>
                                    <input type="radio" name="send_mailsms_auto" checked="checked" value="1" <?php if(isset($editRecord[0]['send_automatically']) && $editRecord[0]['send_automatically'] == '1'){ echo 'checked=checked'; }?> >
                                    Yes</label>
                                </div>
                                
								<div class="col-sm-4">
                                  <label>
                                    <input type="radio" name="send_mailsms_auto" value="0" <?php if(isset($editRecord[0]['send_automatically']) && $editRecord[0]['send_automatically'] == '0'){ echo 'checked=checked'; }?> >
                                    No</label>
                                </div>
							 ...
							 <div class="col-sm-12 mrgtop1 div_only_email div_attachment">
                             <div class="col-sm-12 mrgtop1 only_email">
							 	<label for="text-input"><?=$this->lang->line('email_attach');?></label>
								<p>
									<div id="mulitplefileuploader">
                                     <a class="btn btn-success">Upload</a></div>
                                     <label>Allowed File Types: jpeg,jpg,gif,png,zip,docx,txt,xls,xlsx,pdf,doc </label>
                                    </div>
                                    <div id="status">
                                    </div>
								</p>
								...
								<?php if(isset($attachment) && count($attachment) > 0) { 
										$this->load->view('user/interaction/attachmentlist');
									}
								 ?>
							 </div>
							 </div>
							 <div class="col-xs-12 mrgtop1 div_only_email">
                              ...
                              <div class="row">
								...
								<div class="col-sm-4">
                                  <label>
                                    <input type="radio" name="sign_include" checked="checked" value="1" <?php if(isset($editRecord[0]['include_signature']) && $editRecord[0]['include_signature'] == '1'){ echo 'checked="checked"'; }?>>
                                    Yes</label>
                                </div>
                                
								<div class="col-sm-4">
                                  <label>
                                    <input type="radio" name="sign_include"  value="0" <?php if(isset($editRecord[0]['include_signature']) && $editRecord[0]['include_signature'] == '0'){ echo 'checked="checked"'; }?>>
                                    No</label>
                                </div>
...
$(document).ready(function(){	
	var settings = {
		url: "<?php echo base_url();?>user/emails/upload_image",
		method: "POST",
		allowedTypes:"jpeg,jpg,gif,png,zip,docx,txt,xls,xlsx,pdf,doc",
		fileName: "myfile",
		multiple: true,
		onSubmit: function() {
			$('.div_attachment').block({ message: 'Please Wait...' }); 
		},
...
$('body').on('click','.add_new_template',function(e){
	...
	function selecttemplate(id,val){
		...
		if(id == 3 || id == 6 || id == 8)
		{
			...
			if(id == 8)
				$('.only_email').css('display','none');
			else
				$('.only_email').css('display','');
		}
		else
		...
		$.ajax({
		 type: "POST",
		 url: "<?php echo $this->config->item('user_base_url').$viewname.'/ajax_selecttemplate';?>",
		 dataType: 'json',
	...
	function selectcategory(id,val)
	{
		$.ajax({
		 ...
		 url: "<?php echo $this->config->item('user_base_url').$viewname.'/ajax_selecttemplate';?>",
		 dataType: 'json',
		 data: {loadId:id,'subcategory':subcategoryid,selected:val},
		 cache: false,
		 success: function(result){
