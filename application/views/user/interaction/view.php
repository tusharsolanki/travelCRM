          ...
          <div class="portlet">
			...
			<?php if(!empty($editRecord[0]['interaction_plan_id'])){?>
			  <span class="pull-right"><?php /*?><a title="Back" class="btn btn-secondary margin-left-5px" href="<?php echo $this->config->item('admin_base_url')?><?php echo $viewname.'/'.$editRecord[0]['interaction_plan_id'];?>"><?php echo $this->lang->line('common_back_title')?></a><?php */?> <a class="margin-left-5px btn btn-secondary" onclick="history.go(-1)" href="javascript:void(0)"><?php echo $this->lang->line('common_back_title')?></a></span>
				<!--<span class="pull-right"><a title="Edit" class="btn btn-secondary " href="<?php echo $this->config->item('user_base_url')?><?php echo $viewname;?>/edit_record/<?php echo $inte_plan_id;?>"><?php echo $this->lang->line('common_edit_title')?></a>--> </span>
     
              
			  <?php }else {?>
			  <span class="pull-right"><a title="Back" class="btn btn-secondary" href="<?php echo $this->config->item('user_base_url')?><?php echo $viewname.'/'.$inte_plan_id;?>"><?php echo $this->lang->line('common_back_title')?></a> </span> 
              <!--<span class="pull-right"><a title="Edit" class="btn btn-secondary " href="<?php echo $this->config->item('user_base_url')?><?php echo $viewname;?>/edit_record/<?php echo $inte_plan_id;?>"><?php echo $this->lang->line('common_edit_title')?></a> </span>-->		<?php } ?>
                  ...
                  <div class="row dt-rb">
                      ...
                      <div class="text-center pull-right"> 
					  		<?php if(!empty($editRecord)){?> 
								...
								<?php if(!empty($previous_interaction)){ ?>
									<a href="<?= $this->config->item('user_base_url').$viewname; ?>/view_record/<?=$previous_interaction?>"><i class="fa fa-backward btn btn-prev-next-avail arrow"></i></a>
								<?php }else{ ?>
									<a href="javascript:void(0);"><i class="fa fa-backward btn btn-tertiary arrow"></i></a>
								<?php } ?>
								
								<?php if(!empty($next_interaction)){ ?>
									<a href="<?= $this->config->item('user_base_url').$viewname; ?>/view_record/<?=$next_interaction?>"><i class="fa fa-forward btn btn-prev-next-avail arrow"></i></a>
								<?php }else{ ?>
                               ...
                               <?php foreach($user_list as $row)
									 {
                            			if(!empty($editRecord[0]['assign_to']) && $editRecord[0]['assign_to'] == $row['id'])
											$admin_name = $row['admin_name'].' ('.$row['email_id'].')';
									 }
									?>
                            <?php if(!empty($admin_name)){ echo $admin_name; }else{ echo "-"; }?>
                              </label>
                            </div>
                             </div>
                             </div>
					...
					<div class="col-lg-4 col-sm-12 show_email_sms_div">
					  ...
					  foreach($attachment as $row){?>
						<?php if(!empty($row['attachment_name'])){?><a href="<?php echo base_url()."uploads/attachment_file/".$row['attachment_name']; ?>"><?php echo ucfirst($row['attachment_name'])?></a><br/><?php } ?>
          <?php } } else { echo $this->lang->line('user_general_noreocrds');}?>
							 </div>
							 </div>
...
$('.AjaxUploaderQueueTable tbody tr td a').live( "click", function() {
		...
		$.ajax({
			type: "POST",
			url: '<?=base_url()?>user/emails/delete_attachment',
			data: {
