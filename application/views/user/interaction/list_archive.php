    ...
    <div class="col-md-12">
     ...
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=htmlentities(ucfirst($list[0]['plan_name']))?> : <?=$this->lang->line('interaction_archive');?></h3>
       ...
       <div class="table-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <div class="col-sm-6">
          <a class="btn  pull-right btn-success howler" href="<?=base_url('user/'.$viewname."/".$plan_id);?>" title="Back to List">Back to List</a>
          </div>
         </div>
         <div id="common_div">
	...
	function active_all_plans(name)
		{
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('user_base_url').$viewname."/".$plan_id.'/ajax_Active_all';?>",
			dataType: 'json',
			//async: false,
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/interaction/<?php echo $plan_id; ?>/view_archive/"+data,
					data: {
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search()
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>user/interaction/<?php echo $this->router->uri->segments[3]; ?>/view_archive",
				data: {
				result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
			},
			beforeSend: function() {
