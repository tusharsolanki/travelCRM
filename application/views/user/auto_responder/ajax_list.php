...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           ...
           <? if(in_array('auto_responder_add',$this->modules_unique_name) || in_array('auto_responder_edit',$this->modules_unique_name) || in_array('auto_responder_delete',$this->modules_unique_name)){ ?>
           ...
           <?php if(!empty($datalist) && count($datalist)>0){
					...
					?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<a class="btn btn-xs btn-success" title="Copy Record" href="<?= $this->config->item('user_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy copyicon"></i></a> &nbsp; 
                             <? } ?>
                                        <?php if(!empty($this->modules_unique_name) && in_array('auto_responder_edit',$this->modules_unique_name)){?> 
										<a class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>" title="Edit Record"><i class="fa fa-pencil"></i></a> &nbsp; 
                                         <? } ?>
                                        <?php if(!empty($this->modules_unique_name) && in_array('auto_responder_delete',$this->modules_unique_name)){?>
