<?php 
    /*
        @Description: Admin sms texts list
    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=$this->lang->line('smsautoresponse_list_head');?></h3>
      ...
      <div class="portlet-content">
       <ul class="nav nav-tabs" id="myTab1">
                <li <?php if($tabid == '' || $tabid == 1){?> class="active" <?php } ?> > <a title="SMS Library" data-toggle="tab" href="#my_library" onclick="load_view('my_library');"><?=$this->lang->line('smsautoresponse_list_head');?></a> </li>
                <li <?php if($tabid == 2){?> class="active" <?php } ?>> <a title="SMS Library" data-toggle="tab" href="#default_library" onclick="load_view('default_library');"> Default <?=$this->lang->line('smsautoresponse_list_head');?></a> </li>
            </ul>
        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          <div class="col-sm-6">
           ...
           <button class="btn btn-danger howler" data-type="danger" onclick="deletepopup1('0');" title="Delete Template">Delete SMS Auto Responder</button>
           <? } ?>
          </div>
          <div class="col-sm-6">
          <?php if(!empty($this->modules_unique_name) && in_array('sms_auto_responder_add',$this->modules_unique_name)){?>
          <a class="btn  pull-right btn-secondary-green howler" href="<?=base_url('user/'.$viewname.'/add_record');?>" title="Add Template">Add SMS Auto Responder</a>
          <? } ?>
          </div>
         </div>
          ...
          <div class="col-sm-11">
           ...
           <div class="dataTables_length mrg-right-10" id="DataTables_Table_0_filter">
            <label>
                ...
                <select class="DataTables_Table_0_length" size="1" aria-controls="DataTables_Table_0" onchange="contact_search1('changesearch');" id="selected_cat1" name="selected_cat1">
                 <option value="">Select Category</option>
                  <?php if(isset($category) && count($category) > 0){
                        foreach($category as $row1){
                            if(!empty($row1['id'])){?>
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>user/sms_texts_response/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>user/sms_texts_response/"+data,
					data: {
	...
	function update_data(parent_id,id)
	 {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>user/<?=$viewname?>/update_tempate",
				data: {
