...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
            ...
            <th width="15%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if(isset($sortfield) && $sortfield == 'sms_event_name'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('sms_event_name','<?php echo $sorttypepass;?>')">Event</a></th>
            ...
            <th width="15%" data-direction="desc" data-sortable="true" align="center" data-filterable="true" <?php if(isset($sortfield) && $sortfield == 'sms_message'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('sms_message','<?php echo $sorttypepass;?>')">Message</a></th>
              ...
              <? if(in_array('sms_auto_responder_add',$this->modules_unique_name) || in_array('sms_auto_responder_edit',$this->modules_unique_name) || in_array('sms_auto_responder_delete',$this->modules_unique_name)){ ?>
           ...
           <?php if(!empty($datalist) && count($datalist)>0){
					...
					?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<a title="Copy Template" class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy copyicon5"></i></a> &nbsp; 
                             <? } ?>
                                        <?php if(!empty($this->modules_unique_name) && in_array('sms_auto_responder_edit',$this->modules_unique_name)){?>
										<a title="Edit Template" class="btn btn-xs btn-success" href="<?= $this->config->item('user_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
										 <? } ?>
                                         <?php if(!empty($this->modules_unique_name) && in_array('sms_auto_responder_delete',$this->modules_unique_name)){?>
