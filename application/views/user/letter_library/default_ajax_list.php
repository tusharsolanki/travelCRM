           ...
           <?php if(!empty($default_datalist) && count($default_datalist)>0){
					...
					?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<td class="hidden-xs hidden-sm "><?=!empty($temptitleOutput)?ucfirst(strtolower($temptitleOutput)):'';?></td>
                            ...
                            <div id="temp_desc_<?php echo  $row['id'] ?>" style="display:none;">
									<?=!empty($row['letter_content'])?($row['letter_content']):'';?>
								</div>
							</td>
