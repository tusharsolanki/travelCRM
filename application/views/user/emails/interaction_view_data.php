       ...
       <div class="">
        <div class="tab-content" id="myTab1Content">
         ...
         <div class="tab-pane fade in active" id="home">
          ...
          <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('user_base_url')?><?=$viewname?>/interaction_mailsms" novalidate>
           <div class="row">
           <div class="col-sm-8">
		   <div class="row">
             <div class="col-sm-12">
              <label for="text-input"><?=$this->lang->line('email_to');?> : </label> <label for="text-input"><?=!empty($datalist[0]['first_name'])?ucwords($datalist[0]['first_name']):''?> <?=!empty($datalist[0]['last_name'])?ucwords($datalist[0]['last_name']):''?>(<?=!empty($datalist[0]['email_address'])?($datalist[0]['email_address']):''?>)</label>
             </div>
            </div>
           <div class="row">
             <div class="col-sm-12">
		   ...
		   <div class="row">
             <div class="col-sm-12">
			  ...
			  <label for="text-input"><?=!empty($datalist[0]['template_name'])?(($datalist[0]['template_name'])):''?></label>
             </div>
           </div>
           <div class='row mycalclass'>
            <div class="col-sm-12 form-group">
             <label for="text-input"><?=$this->lang->line('tasksubject_label_name');?> : </label>
             <label for="text-input"><input id="txt_template_subject" name="txt_template_subject" class="form-control parsley-validated" type="text" value="<?=!empty($datalist[0]['template_subject'])?ucfirst(strtolower($datalist[0]['template_subject'])):''?>" data-required="true" placeholder="e.g. Email Subject"></label>
           </div>
          </div>
            </div>
          ...
          <div class="col-sm-4">
            ...
            <div class="attachfiel">
			 ...
			 ?>
			 	<!--<a href="#" title="" onclick="download_file('<?=$attachment[$i]?>');"> <?=$attachment[$i]?></a>-->
                <a href="<?php echo base_url()?>uploads/attachment_file/<?=$attachment[$i]?>" title="" onclick="download_file('<?=$attachment[$i]?>');"> <?=$attachment[$i]?></a>
                <br />
			<?php
					//}	
				}
          ...
          <div class="col-sm-12 pull-left text-center margin-top-10">
 			<!--<a class="btn btn-primary" href="<?php echo $this->config->item('user_base_url').$viewname; ?>" title="Close">Close</a>-->
			...
			<a class="btn btn-primary" onclick="history.go(-1)" href="<?php echo $this->config->item('user_base_url').$viewname."/interaction_plan_queued_list/".$this->uri->segment(5); ?>" title="Close">Close</a>
...
// print_r($subcategory);
 ...
 {
 	var url='<?=base_url('user/'.$viewname.'/download_form')?>/'+str;
	window.location= url;
