					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr class="<? if($i%2==1){ echo 'bgtitle'; }?> <?php if($row['is_send'] == 0) { echo 'new_red_class'; } ?>" > 
							...
							<a href="<?=$this->config->item('user_base_url').$viewname;?>/view_data/<?=!empty($editRecord[0]['id'])?$editRecord[0]['id']:'';?>/<?=$row['ID']?>" class="contact_name_<?=$row['id']?>">
							<?=!empty($row['first_name'])?ucfirst(strtolower($row['first_name'])):'';?> <?=!empty($row['last_name'])?ucfirst($row['last_name']):'';?>
							</a>
							</td>
							<td class="hidden-xs hidden-sm ">
							<a href="<?=$this->config->item('user_base_url').$viewname;?>/view_data/<?=!empty($editRecord[0]['id'])?$editRecord[0]['id']:'';?>/<?=$row['ID']?>" class="subject_<?=$row['id']?>">
         ...
         <div class="row dt-rb" id="common_tb">
           ...
           <div class="dataTables_paginate paging_bootstrap float-right">
            ...
            <label>
             ...
             <option value="">Emails campaign</option>
...
function resend_mail(id)
{
	...
	$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('user_base_url').$viewname.'/resend_mail/';?>"+campaign_id+"/<?=$this->uri->segment(5)?>",
			data: {'single_id':id},
			beforeSend: function() {
			...
			//alert(result);
				$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('user_base_url').$viewname.'/sent_email'?>"+result,
					data: {
