...
function getVideo(video_id)
{
	$.ajax({
		 type: "POST",
		 url: "<?php echo $this->config->item('user_base_url').'bomb_library/VideoList';?>",
		 ...
		 success: function(html){
			 ...
			 /*if(flag == 0)
			 {
				 <?php if(!empty($editRecord[0]['video_id'])){ ?>
					save_video_id = '<?=$editRecord[0]['video_id']?>';
					selectVideo('0');
				<?php } ?>
				flag = 1;
			 }*/
    ...
    <div class="col-md-12">
     ...
     <div class="portlet">
      ...
      <div class="portlet-content">
       <div class="">
             ...
             <div class="col-sm-12 form-group">
		   	...
		   	<?php if($formAction == 'insert_data' && !empty($this->router->uri->segments[4])) { ?>
                ...
                <select  class="form-control parsley-validated" name="slt_interaction_type" id="slt_interaction_type" onchange="selecttemplate(this.value)"  >
                      <?php if(!empty($this->modules_unique_name) && in_array('email_blast_add',$this->modules_unique_name)) { ?><option value="6">Email</option> <?php } ?>
        ...
        <?php if(($formAction == 'insert_data' && empty($this->router->uri->segments[4])) || $formAction == 'update_data') { ?>
         ...
         <input id="txt_template_subject" name="txt_template_subject" class="form-control parsley-validated" type="text" value="<?php if(!empty($editRecord[0]['template_subject'])){ echo htmlentities($editRecord[0]['template_subject']); }?>" data-required="true" placeholder="e.g. Email Subject">
          </div>
          </div>
          </div>
          <div class="col-sm-4 margin-top-10px <?php if($viewname == 'emails') { echo 'display_none'; } ?>" id="add_new_video">
              <a href="#VideoModal" class="margin-top-15px add_new_video" id="VideoModal" data-toggle="modal"> 
	  ...
	  <!--<div class="fileUpload btn btn-primary">-->
	  	<div>
        <p> 
		<div id="mulitplefileuploader"> <a class="btn btn-secondary-green">Upload</a></div>
	  ...
	  <textarea name="email_message"  id="email_message" class="form-control parsley-validated" ><?=!empty($editRecord[0]['email_message'])?$editRecord[0]['email_message']:'';?></textarea>			  
                     ...
                     <legend>Schedule Blast</legend>
                       <div class="schedule_detail">
                         ...
                         <div class="col-sm-12 pull-left">  
                          <div class="float-left margin-left-15"><label class="checkbox margin-left-15">
                          Now
                            <div class="col-sm-3">
                         ...
                         <label class="checkbox">
                          Date Time :
                            ...
                            <label class="col-sm-3 col-lg-3 mrg27">Time</label>
                                <input name="send_time" placeholder="Time" readonly="readonly" id="send_time" type="text" <?php if(!empty($editRecord[0]['email_send_type']) && $editRecord[0]['email_send_type'] == '2') { ?> value="<?=!empty($editRecord[0]['email_send_time'])?date("h:i A", strtotime($editRecord[0]['email_send_time'])):''?>" <?php } ?> /><!--<i class="fa fa-clock-o"></i>-->
                                <button type="button" class="timepicker_disable_button_trigger timepick" onclick="send_time_focus();"><img src="<?=base_url('images/timecal.png')?>" alt="..." title="..."></button>
			...
			<div class="col-sm-4">
             ...
             <div class="unsubscribe" style="display:none;">
              ...
              Is Unsubscribe 
              <div class="float-left margin-left-15">
               <input type="checkbox" name="is_unsubscribe" id="is_unsubscribe" class="" value="1" checked="checked" />
              </div>
              </label></div> 
            </div>
            </div>
...
<input type="submit" class="btn btn-secondary" id="save_template" value="Save Template As" title="Save Template As" onclick="return validation_date()" name="submitbtn2" />
 <?php if($formAction == 'insert_data' && !empty($this->router->uri->segments[4])) { ?>
...
var send_blast_type = $("#slt_interaction_type").val();
  $.ajax({
     ...
     dataType: 'json',
	 data: {loadType:loadType,loadcategoryId:loadcategoryId,loadsubcategoryId:loadsubcategoryId,send_blast_type:send_blast_type},
     cache: false,
     success: function(result){
...
$(document).ready(function (){
	...
	var settings = {
		url: "<?php echo base_url();?>user/<?=$viewname?>/upload_image",
		method: "POST",
...
<script type="text/javascript">
	...
	{
		if($('.mycheckbox:checkbox[value='+contact_type_id+']:checked').length)
		{		
...
function selecttemplate(id,val){
    ...
    if(id == 6 || id == 8)
    {
            ...
            if(id == 8)
            {
				...
				$('form#emails').attr('action','<?=base_url('user/bomb_emails/insert_data')?>');
				} ,100);
            }        
            else
            {
               setTimeout(function(){$('.attachfiel').css('display','');
				$('#add_new_video').addClass('display_none');
                $('form#emails').attr('action','<?=base_url('user/emails/insert_data')?>');},100);
            }    
    }
    ...
    $.ajax({
     type: "POST",
     url: "<?php echo $this->config->item('user_base_url').'interaction/ajax_selecttemplate';?>",
     dataType: 'json',
...
<?php if($formAction == 'insert_data' && !empty($this->router->uri->segments[4])) { ?>
        ...
        //$('.attachfiel').css('display','none');
        $('#emails').attr('action','<?=base_url('user/emails/insert_data')?>');
        <? } ?>
});
