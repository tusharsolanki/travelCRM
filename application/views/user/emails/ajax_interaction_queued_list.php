					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
                            ...
                            <a href="<?php echo $this->config->item('user_base_url').$viewname?>/interaction_plan_queued_list/<?=$row['id']?>"><?=!empty($row['description'])?ucfirst(strtolower($row['description'])):'';?></a>
                            
							</td>
                            <td class="hidden-xs hidden-sm">
                             <a href="<?php echo $this->config->item('user_base_url').$viewname?>/interaction_plan_queued_list/<?=$row['id']?>"><?=!empty($row['template_name'])?ucfirst(strtolower($row['template_name'])):'';?></a>
							</td>
                            <td class="hidden-xs hidden-sm">
                            	<a class="btn btn-xs btn-success" href="<?php echo $this->config->item('user_base_url').$viewname?>/interaction_mailsms/<?=$row['id']?>"><i class="fa fa-play"></i></a>
                            </td>	
                          </tr>
          <?php } } else {?>
		  <tr>
