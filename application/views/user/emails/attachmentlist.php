                      ...
                      foreach($attachment as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<td class="hidden-xs hidden-sm text-center">
								<a href="#" class="btn btn-xs btn-primary" onclick="deletepopup1('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['attachment_name']))) ?>','<?=$filesize?>');"><i class="fa fa-times"></i>
...
function delete_all(id,name,filesize)
{
	$.ajax({
		type: "POST",
		url: "<?php echo $this->config->item('user_base_url').$viewname.'/ajax_delete_attachment';?>",
