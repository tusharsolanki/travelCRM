...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           ...
           <!-- <th class="sorting_disabled" role="columnheader" rowspan="1" colspan="1" aria-label="" width="5%">
            ...
            <th width="25%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'ipm.plan_name'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('ipm.plan_name','<?php echo $sorttypepass;?>')"> Communication </a></th>
           </tr>
           </thead>
					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($interaction_plan as $row){?>
						...
						<?php /*?>	<td class="">
                            ...
                            <a href="<?php echo $this->config->item('admin_base_url').$viewname?>/interaction_plan_queued_list/<?=$row['id']?>">
...
function interaction_mailsms(id,interaction_id)
{
	$.ajax({
			...
			//alert(result);
				$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('admin_base_url').$viewname.'/queued_list'?>"+result,
					data: {
...
function delete_all(id)
{
	$.ajax({
		type: "POST",
		url: "<?php echo $this->config->item('admin_base_url').$viewname.'/delete_record_trans';?>",
		dataType: 'json',
		async: false,
		data: {id:id},
		success: function(result){
			$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('admin_base_url').$viewname.'/queued_list'?>",
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
