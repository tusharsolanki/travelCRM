...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           ...
           <?php if(!empty($this->modules_unique_name) && in_array('text_blast_delete',$this->modules_unique_name)){?>
            ...
            <th width="27%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'sms_message'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('sms_message','<?php echo $sorttypepass;?>')">Message</a></th>
						...
						<tr class="<? if($i%2==1){ echo 'bgtitle'; }?> <?php if($row['is_sent_to_all'] == '0') {  echo 'new_red_class'; } ?>" > 
                            ...
                            <td class="hidden-xs hidden-sm text-center">
								<?php if($row['is_draft'] == '0' && strtotime(date('Y-m-d')) > strtotime($datetime)) { ?>
									<a class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/sent_sms/<?= $row['id'] ?>" title="View" > <i class="fa fa-envelope"></i></a>
								<?php } ?>
								<?php if($row['is_draft'] == '0' && strtotime(date('Y-m-d')) <= strtotime($datetime)) { 
							...
							<td class="hidden-xs hidden-sm text-center">
								...
								<a class="btn btn-xs btn-success" href="<?php echo $this->config->item('admin_base_url').$viewname.'/resend_sms/'.$row['id']; ?>" title="Resend" > Resend </a>
								<?php } ?>
							...
							<a class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>" title="Copy"><i class="fa fa-copy copyicon5"></i></a> &nbsp; 
                            <? } ?>
                             <?php if(!empty($this->modules_unique_name) && in_array('text_blast_edit',$this->modules_unique_name)){?>
                                        <?php if(($row['is_draft'] == 1) || ($row['is_draft'] == 0 && strtotime($datetime) >= strtotime(date('Y-m-d')))) { ?>
										<a class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>" title="Edit SMS Campaign"><i class="fa fa-pencil"></i></a> &nbsp; 
                                        <?php } ?>
                                          <?php } ?>
                                        <?php if(!empty($this->modules_unique_name) && in_array('text_blast_delete',$this->modules_unique_name)){?>
