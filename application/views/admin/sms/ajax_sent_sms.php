...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
			...
			<th width="10%" class="hidden-xs hidden-sm sorting_disabled" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade" width="7%"> Resend </th>
			
			<th width="20%" class="hidden-xs hidden-sm sorting_disabled" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade" width="7%"> Sent Date & Time </th>
					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr class="<? if($i%2==1){ echo 'bgtitle'; }?> <?php if($row['is_send'] == '0') { echo 'new_red_class'; } ?>" > 
							...
							<a href="<?=$this->config->item('admin_base_url').$viewname;?>/view_data/<?=!empty($editRecord[0]['id'])?$editRecord[0]['id']:'';?>/<?=$row['ID']?>">
...
function resend_mail(id)
{
	...
	$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/resend_sms/';?>"+campaign_id+"/<?=$this->uri->segment(5)?>",
			async: false,
			data: {'single_id':id},
			...
			//alert(result);
				$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('admin_base_url').$viewname.'/sent_sms'?>"+result,
					data: {
