						...
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
							...
							<td class="hidden-xs hidden-sm ">
							 ...
							 elseif(date('Y-m-d') >= $row['send_sms_date']) { ?>
                                 ...
                                 <a href="<?php echo $this->config->item('admin_base_url')?><?=$viewname?>/view_interaction_data/<?=$row['id']?>/<?=$this->uri->segment(4)?>" class="btn btn-xs btn-success" title="Send SMS"> <i class="fa fa-play"></i> </a>
							<?php } else { ?>
								 <!--<a href="#" class="btn btn-xs btn-success" onclick="interaction_mailsms(<?=$row['id']?>,<?=$row['interaction_id']?>)"> <i class="fa fa-play"></i> </a>-->
								 <i class="fa fa-play" title="Send SMS(Disabled)"></i>
							<?php } ?>
								&nbsp;
...
function delete_all(id)
{
	...
	$.ajax({
		...
		success: function(result){
			$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('admin_base_url').$viewname.'/interaction_plan_queued_list/'?>"+interaction_plan+"/"+pageno,
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
