    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-sm-12 contact_to">
          <div class="">
		  <?php $this->load->view('admin/sms/contact_to');?>
		  </div>
        </div>
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-sm-12 add_new_contact_popup">
		  ...
		  <?php $this->load->view('admin/sms/add_contact_popup_ajax');?>
		  </div>
        </div>
      </div>
      <div class="col-sm-12 text-center mrgb4">
                ...
                <div class="row">
                 <div class="col-sm-12 form-group">
                  <label for="select-multi-input">
                    ...
                    <textarea name="sms_message" placeholder="e.g. SMS Message" id="sms_message" class="form-control parsley-validated" data-required="true" onkeypress="return keypress(event);" ><?=!empty($editRecord[0]['sms_message'])?$editRecord[0]['sms_message']:'';?></textarea>
             ...
             <div class="col-sm-12">
              <label for="select-multi-input">
                <fieldset class="schedule_campaign">
                     <legend class="edit_title
mrg1">Schedule Campaign</legend>
							...
							<div class="col-sm-12 col-lg-5 col-md-8 sms_add_record">
                              <input name="send_date" placeholder="Specific Date" id="send_date" class="form-control parsley-validated" type="text" <?php if(!empty($editRecord[0]['sms_send_type']) && $editRecord[0]['sms_send_type'] == '2') { ?>value="<?=!empty($editRecord[0]['sms_send_date'])?$editRecord[0]['sms_send_date']:''?>" <?php } ?> readonly="readonly"/>	
                           	...
                           	<div class="col-sm-12 col-lg-4 col-md-9 sms_add_record">
								...
								<input name="send_time" placeholder="Time" id="send_time" readonly="readonly" type="text" <?php if(!empty($editRecord[0]['sms_send_type']) && $editRecord[0]['sms_send_type'] == '2') { ?> value="<?=!empty($editRecord[0]['sms_send_time'])?date("h:i A", strtotime($editRecord[0]['sms_send_time'])):''?>" <?php } ?> /> <button type="button" class="col-sm-3 col-md-2 col-lg-1 timepicker_disable_button_trigger timepick" onclick="send_time_focus()"><img src="<?=base_url('images/timecal.png')?>" alt="..." title="..."/></button>
							</div>
...
<input type="submit" class="btn btn-secondary-green" id="send_now" value="Send Now" title="Send Now" onclick="return validation();" name="submitbtn" />
...
<input type="submit" class="btn btn-secondary-green" id="save_template" value="Save Template As" title="Save Template As" onclick="return validation();" name="submitbtn2" />
<?php if($formAction == 'insert_data' && !empty($this->router->uri->segments[4])) { ?>
...
<?php } else { ?>
	<a class="btn btn-primary" href="javascript:history.go(-1);" id="btncancel" title="Close">Close</a>
<?php } ?>
         </div>
...
<script type="text/javascript">
	...
	function contact_search()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/sms/search_contact_ajax/",
			data: {
			result_type:'ajax',searchtext:$("#search_contact_popup_ajax").val()
		},
		beforeSend: function() {
	...
	{	
		$.ajax({
			type: "POST",
			dataType: 'json',
			url: "<?php echo base_url();?>admin/sms/add_contacts_to_email/",
			data: {
...
<script type="text/javascript">
	...
	function contact_ser_to()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/sms/search_contact_to/",
			data: {
			result_type:'ajax',searchtext:$("#search_contact_to").val(),contact_status:$('#slt_contact_status').val(),contact_source:$('#slt_contact_source').val(),contact_type:$('#contact_type').val(),sortfield:$("#sortfield_to").val(),sortby:$("#sortby_to").val(),perpage:$("#perpage_to").val(),search_tag:$("#search_tag_to").val()
		},
		beforeSend: function() {
	...
	function addcontactstoemailplan()
	{
		$.ajax({
			type: "POST",
			dataType: 'json',
			url: "<?php echo base_url();?>admin/sms/contacts_to_email/",
			data: {
