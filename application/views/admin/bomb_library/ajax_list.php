          ...
          <thead>
           ...
           <?php if(!empty($this->modules_unique_name) && in_array('email_library_delete',$this->modules_unique_name)){?>
            ...
            <th width="15%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'created_by'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('created_by','<?php echo $sorttypepass;?>')"><?=$this->lang->line('common_label_inserted_by')?></a></th>
             ...
             <th width="20%" class="hidden-xs hidden-sm sorting_disabled actionbtn" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade"><?php echo $this->lang->line('common_label_action')?></th>
             <? } ?>
           </tr>
           </thead>
					...
					?>
						<tr  <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
                         <?php if(!empty($this->modules_unique_name) && in_array('email_library_delete',$this->modules_unique_name)){?>
							...
							<a class="btn btn-xs btn-success" title="Copy Record" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy"></i></a> &nbsp; 
                             <? } ?>
