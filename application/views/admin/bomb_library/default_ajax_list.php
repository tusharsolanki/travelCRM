...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
            <?php if(!empty($this->modules_unique_name) && in_array('bomb_library_delete',$this->modules_unique_name)){?>
            ...
            <? if(in_array('bomb_library_edit',$this->modules_unique_name) || in_array('bomb_library_delete',$this->modules_unique_name)){ ?>
           ...
           <?php if(!empty($default_datalist) && count($default_datalist)>0){
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      ...
                      <?
						...
						$pattern = "{(%s)}";
						$map = array();
						if($bombdata != '' && count($bombdata) > 0)
						{
							foreach($bombdata as $var => $value)
							{
					...
					?>
						<tr  <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<td class="hidden-xs hidden-sm" id="temp_subject_<?php echo  $row['id'] ?>"><?=!empty($temptitleOutput)?ucfirst(strtolower($temptitleOutput)):'';?>
                              ...
                              <? if(in_array('bomb_library_edit',$this->modules_unique_name) || in_array('bomb_library_delete',$this->modules_unique_name)){ ?>
							<td class="hidden-xs hidden-sm">
							<?php if(!empty($this->modules_unique_name) && in_array('bomb_library_edit',$this->modules_unique_name)){?>
