...
function getVideo(video_id)
{
	$.ajax({
		 type: "POST",
		 url: "<?php echo $this->config->item('admin_base_url').$viewname.'/VideoList';?>",
    ...
    <div class="col-md-12">
     ...
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-tasks"></i> <?php if(empty($editRecord)){ echo $this->lang->line('bomblibrarytemplate_add_head');}
	   else if(!empty($insert_data)){ echo $this->lang->line('bomblibrarytemplate_add_head'); } 
	   else{ echo $this->lang->line('bomblibrarytemplate_edit_head'); }?> </h3>
          ...
          <div class="col-sm-4">
            ...
            <div class="custom_field">
             ...
             <?php foreach($tablefield_data as $row){ 
					if(!empty($row['name'])){
					$pattern = $this->config->item('bomb_param_pattern');
					if(empty($pattern))
						$pattern = "{(%s)}";
...
<input type="submit" class="btn btn-secondary-green" title="Save Template" value="Save Bomb Bomb Template" onclick="return setdefaultdata();" name="submitbtn" id="el_submitbtn" />
<?php if($this->uri->segment(4) == 'iframe'){ ?>
	...
	$("select#subcategory").multiselect({
	 ...
	 function setdefaultdata()
	 {
		...
		if(ck_edit == "")
		{
			...
			$.confirm({'title': 'Alert','message': " <strong> Please enter bomb message details."+"<strong></strong>",'buttons': {'ok'	: {'class'	: 'btn_center alert_ok'}}});
			return false;
		}
...
$('body').on('click','#upload',function(e){
	window.open('<?=$this->config->item('admin_base_url')?><?=$viewname?>/AddVideo',"mywindow","resizable=1");
});
...
function selectVideo()
{
	...
	if(video_id.trim() != '')
	{
		var html = '<div class="addpic pull-left">';
		html += $(".video_details_"+video_id).html();
		html += '</div>';
		html += '<a class="btn btn-xs btn-primary" onclick="delete_video();" href="javascript:void(0);" title="Remove Video"><i class="fa fa-times"></i></a>';
...
/*$("#VideoModal").click(function() {
	CKEDITOR.instances.email_message.insertHtml( '<img class="responsive_image" height="320" id="8dd8cd97-3a31-44d5-a49b-24f5ece592ee" name="bb_video" src="http://s3.amazonaws.com/bbemail/PROD/video_thumbs/8dd8cd97-3a31-44d5-a49b-24f5ece592ee.jpg" style="display:block" width="500" />', function() {
    CKEDITOR.instances.email_message.focus();
});
	//CKEDITOR.instances.email_message.insertText().focus();	
});*/
//CKEDITOR.instances.email_message.setData('<img class="responsive_image" height="320" id="8dd8cd97-3a31-44d5-a49b-24f5ece592ee" name="bb_video" src="http://s3.amazonaws.com/bbemail/PROD/video_thumbs/8dd8cd97-3a31-44d5-a49b-24f5ece592ee.jpg" style="display:block" width="500" />');
