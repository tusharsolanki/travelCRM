...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           ...
           <?php if(!empty($this->modules_unique_name) && ($viewname == 'emails' && in_array('email_blast_delete',$this->modules_unique_name)) || ($viewname == 'bomb_emails' && in_array('bomb_bomb_email_blast_delete',$this->modules_unique_name))){?>
            ...
            <th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'template_subject'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('template_subject','<?php echo $sorttypepass;?>')"><?=$this->lang->line('tasksubject_label_name')?></a></th>
			<th width="14%" class="hidden-xs text-center hidden-sm sorting_disabled" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade">Sent Emails</th>
			
			<th width="13%" class="hidden-xs text-center hidden-sm sorting_disabled" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade">All Resend</th>
						...
						<tr class="<? if($i%2==1){ echo 'bgtitle'; }?> <?php if($row['is_sent_to_all'] == '0') {  echo 'new_red_class'; } ?>"  > 
							...
							<td class="hidden-xs hidden-sm text-center">
								<?php if($row['is_draft'] == '0' && strtotime(date('Y-m-d')) > strtotime($datetime)) { ?>
									<a class="btn btn-xs btn-success " href="<?= $this->config->item('admin_base_url').$viewname; ?>/sent_email/<?= $row['id'] ?>" title="View" > <i class="fa fa-envelope"></i></a>
								<?php } ?>
								<?php if($row['is_draft'] == '0' && strtotime(date('Y-m-d')) <= strtotime($datetime)) { 
							...
							<td class="hidden-xs hidden-sm text-center">
								...
								<a class="btn btn-xs btn-success" href="<?php echo $this->config->item('admin_base_url').$viewname.'/resend_mail/'.$row['id']; ?>" title="Resend" > Resend </a>
								<?php } ?>
							...
							<?php if(!empty($this->modules_unique_name) && ($viewname == 'emails' && in_array('email_blast_add',$this->modules_unique_name)) || ($viewname == 'bomb_emails' && in_array('bomb_bomb_email_blast_add',$this->modules_unique_name))){?>
                                <a class="btn btn-xs btn-success copyicon4" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>" title="Copy"><i class="fa fa-copy"></i></a> &nbsp; 
                                <? } ?>
                               ...
                               <?php if(($row['is_draft'] == 1) || ($row['is_draft'] == 0 && strtotime($datetime) >= strtotime(date('Y-m-d')))) { ?> 
                                <a class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>" title="Edit Email Blast"><i class="fa fa-pencil"></i></a> &nbsp;
