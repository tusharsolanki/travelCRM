...
<tbody aria-relevant="all" aria-live="polite" role="alert" class="select_record_table select_contacts_box contact_type_to_table">
...
<script>
$('.contact_type_to_table').on('click', 'tr', function() {
    $(this).find('td:first :checkbox').trigger('click');
	var contact_type_id = $(':checkbox', this).val();
	contact_type_to_checkbox(contact_type_id);
})
.on('click', '.mycheckbox', function(e) {
    e.stopPropagation();
    $(this).closest('tr').toggleClass('selected', this.checked);
	contact_type_to_checkbox(this.value);
});

function remove_selected_type_to()
{
