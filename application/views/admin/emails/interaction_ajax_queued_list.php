...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
            ...
            <th width="25%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'ecr.template_subject'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('ecr.template_subject','<?php echo $sorttypepass;?>')"><?=$this->lang->line('tasksubject_label_name')?></a></th>
			...
			<th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'ecr.send_email_date'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('ecr.send_email_date','<?php echo $sorttypepass;?>')"> Queued Date </a></th>
			
			 <th width="15%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'ecr.sent_date'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"> Action </th>
            ...
            <form class="form parsley-form" enctype="multipart/form-data" name="interaction_mail" id="interaction_mail" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('admin_base_url')?><?=$viewname?>/view_interaction_data">
					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
							<?php
								if($row['email_type'] == 'Intereaction_plan')
									$datetime = $row['send_email_date'];
								else
							...
							<td class="hidden-xs hidden-sm ">
                             ...
                             <?php }
							 elseif(!empty($row['i_start_type']) && $row['i_start_type'] == 2 && !empty($row['interaction_id1']) && $row['is_done'] == '0'){?>
                             ...
                             <?php }
							 elseif(date('Y-m-d') >= $row['send_email_date']) { ?>
                        ...
                        <a href="<?php echo $this->config->item('admin_base_url')?><?=$viewname?>/view_interaction_data/<?=$row['id']?>/<?=$this->uri->segment(4)?>" class="btn btn-xs btn-success" title="Send Mail"> <i class="fa fa-play"></i> </a>
                        
							<?php } else { ?>
								 <i class="fa fa-play" title="Send Mail(Disabled)"></i>
							<?php } ?>
								&nbsp;
                                 <?php if(!empty($this->modules_unique_name) && in_array('email_blast_delete',$this->modules_unique_name)){?>
								<a href="javascript:void(0);" class="btn btn-xs btn-primary" onclick="deletepopup1('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['first_name'].$row['last_name'])))?>');" title="Delete"><i class="fa fa-times"></i></a>
                                <?php } ?>
							 </td>				
                          </tr>
          <?php } } else {?>
...
<script>
function form_submit(id,interaction_id)
{
	$("#single_id").val(id);
	$("#interaction_id").val(interaction_id);
	$("#interaction_mail").submit();
}
/*function interaction_mailsms(id,interaction_id)
{
	...
	$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/interaction_mailsms/';?><?=$this->uri->segment(4)?>",
			async: false,
			...
			//alert(result);
				$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('admin_base_url').$viewname.'/interaction_plan_queued_list/'.$this->uri->segment(4)?>/"+pageno,
					data: {
