...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
  <thead>
    <tr role="row">
      <?php if(!empty($this->modules_unique_name) && in_array('envelope_library_delete',$this->modules_unique_name)){?>
      ...
      <th width="18%" rowspan="1" colspan="1"><?=$this->lang->line('common_label_preview')?></th>
       ...
       <? if(in_array('envelope_library_add',$this->modules_unique_name) || in_array('envelope_library_edit',$this->modules_unique_name) || in_array('envelope_library_delete',$this->modules_unique_name)){ ?>
        <th class="hidden-xs hidden-sm sorting_disabled actionbtn7" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade" width="7%">
  ...
  <tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <?
						...
						$content1 = !empty($row['envelope_content'])?$row['envelope_content']:'';
						$title = !empty($row['template_subject'])?$row['template_subject']:''; 
						//$content = $headers;
    ...
    <tr <? if($i%2==1){ ?>class="bgtitle" <? }
      ...
      <td class="hidden-xs hidden-sm "id="temp_size_<?php echo  $row['id'] ?>" ><?php
        ...
        <strong>X</strong>
        <?=!empty($row['size_h'])?$row['size_h'].'"':'';?></td>
      <?  }
							?>
      
      <td class="hidden-xs hidden-sm "><a class="btn btn-secondary view_form_btn" data-toggle="modal" href="#basicModal" data-id="<?php echo  $row['id'] ?>" title="<?=$this->lang->line('common_label_preview')?>">
        <?=$this->lang->line('common_label_preview')?>
        </a>
        ...
        <div id="template_type_<?php echo  $row['id'] ?>" style="display:none;">
          <?php if(!empty($row['template_type']) && $row['template_type'] == '1') { echo ucwords('Fix');} else if($row['template_type'] == '2') { echo ucwords('Custom'); } else { echo '-';}?>
        </div>
        ...
        <!--<form class="form parsley-form" enctype="multipart/form-data" name="mail_out" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('admin_base_url').'mail_out/mail_out_data'?>" novalidate>
								...
								<input type="hidden" id="template_type" name="template_type" value="Envelope" />
      ...
      <? if(in_array('envelope_library_add',$this->modules_unique_name) || in_array('envelope_library_edit',$this->modules_unique_name) || in_array('envelope_library_delete',$this->modules_unique_name)){ ?>
      <td class="hidden-xs hidden-sm "><?php if(!empty($this->modules_unique_name) && in_array('envelope_library_add',$this->modules_unique_name)){?>
        <a class="btn btn-xs btn-success" title="Copy Envelope" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy copyicon5"></i></a> &nbsp;
        <? } ?>
		<?php if(!empty($this->modules_unique_name) && in_array('envelope_library_edit',$this->modules_unique_name)){?>
         <a class="btn btn-xs btn-success" title="Edit Envelope" href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp;
        <? } ?>
        <?php if(!empty($this->modules_unique_name) && in_array('envelope_library_delete',$this->modules_unique_name)){?>
        <button class="btn btn-xs btn-primary"  title="Delete Envelope" onclick="deletepopup1('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['template_name']))) ?>');"><i class="fa fa-times"></i></button>
        <? } ?></td>
      <? } ?>
    </tr>
    <?php } } else {?>
