...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
      ...
      <? } ?>
            ...
            <th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield1) && $sortfield1 == 'size_w'){if($sortby1 == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact1('size_w','<?php echo $sorttypepass1;?>')"><?=$this->lang->line('size_label_name')?></a></th>
             ...
             <? if(in_array('envelope_library_edit',$this->modules_unique_name) || in_array('envelope_library_delete',$this->modules_unique_name)){ ?>
      ...
      <? } ?>
           ...
           <?php
		    ...
		    if(!empty($default_datalist) && count($default_datalist)>0){
					...
					?>
						...
						?> > 
                          ...
                          <td class=""><div class="text-center">
                            ...
                            <? if(in_array('envelope_library_edit',$this->modules_unique_name) || in_array('envelope_library_delete',$this->modules_unique_name)){ ?>
							<td class="hidden-xs hidden-sm ">
								<?php /*?><a class="btn btn-xs btn-success" title="Copy Envelope" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy copyicon5"></i></a> &nbsp;  <?php */?>      
								...
								<a class="btn btn-xs btn-success" title="Edit Envelope" href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
                                  <? } ?>
        <?php if(!empty($this->modules_unique_name) && in_array('envelope_library_delete',$this->modules_unique_name)){?>
								<button class="btn btn-xs btn-primary"  title="Delete Envelope" onclick="deletepopup2('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['template_name']))) ?>');"><i class="fa fa-times"></i></button>
							  <? } ?>
