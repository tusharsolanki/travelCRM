                      ...
                      foreach($datalist as $row){// pr($row);?>
                                <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
                                    ...
                                    <?php } ?>
                                        <td class="hidden-xs hidden-sm ">
                                            <a href="<?= $this->config->item('admin_base_url'); ?>interaction/<?= $row['interaction_plan_id'] ?>" class="textdecoration">
                                            <?=!empty($row['plan_name'])?$row['plan_name']:'';?></a>
                                        </td>
                                            ...
                                            <div class="radio">
                                                <label class="">
                                                    <div <?php if(!empty($row['status']) && $row['status'] == 'On'){ echo 'class="fnt_bold"'; } else { echo 'class="fnt_normal"';}?>>
                                                        <input type="radio" name="status_<?=$row['id']?>" id="joomla_contact_type" onclick="change_status('On',<?=$row['id']?>);" <?php if(!empty($row['status']) && $row['status'] == 'On'){ echo 'checked="checked"'; }?> >On
                                                    </div>
                                                </label>
                                            </div>
                                            <div class="radio">
						<label class="">
                                                    <div <?php if(!empty($row['status']) && $row['status'] == 'Off'){ echo 'class="fnt_bold"'; } else { echo 'class="fnt_normal"';}?>>
                                                        <input type="radio" name="status_<?=$row['id']?>" id="joomla_contact_type" onclick="change_status('Off',<?=$row['id']?>);" <?php if(!empty($row['status']) && $row['status'] == 'Off'){ echo 'checked="checked"'; }?> >Off
                                                    </div>
						</label>
					   </div>
          ...
          <div class="col-sm-6">
           <div class="dataTables_paginate paging_bootstrap float-right">
            ...
            <label>
             ...
             <option value=""><?=$this->lang->line('label_ass_plan_per_page');?></option>
