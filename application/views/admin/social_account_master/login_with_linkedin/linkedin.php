...
class LinkedIn {
	...
	{
		$contact_url = $this->base_url . "/v1/people/~/connections" . $body;			
		$request = OAuthRequest::from_consumer_and_token($this->consumer, $this->access_token, "GET", $contact_url);
		$request->sign_request($this->signature_method, $this->consumer, $this->access_token);
		$auth_header = $request->to_header("https://api.linkedin.com"); # this is the realm
		$response = $this->httpRequest($contact_url, $auth_header, "GET");
		return $response;
	}
	function fetch1($method,$body = '',$access_token)
	{
		$contact_url = $this->base_url . "/v1/companies?is-company-admin=true" . $body;			
		$request = OAuthRequest::from_consumer_and_token($this->consumer, $access_token, "GET", $contact_url);
		$request->sign_request($this->signature_method, $this->consumer, $access_token);
		$auth_header = $request->to_header("https://api.linkedin.com"); # this is the realm
		$response = $this->httpRequest($contact_url, $auth_header, "GET");
		return $response;
	}
