   ...
   <?php
		...
		$i=1;
			...
			<tr <? if($i%2==1){ ?>class="bgtitle delete_document_trans_record<?= $row['id'] ?>" <? }?> > 
				...
				<?php if(!empty($row['doc_file'])  && file_exists($this->config->item('contact_documents_big_img_path').$row['doc_file'])){?>
				<a target="_blank" class="btn btn-xs btn-success" href="<?=base_url().'uploads/contact_docs/'; ?><?= $row['doc_file'] ?>"><i class="fa fa-download"></i></a> &nbsp; 
				<?php } ?>
				<a class="btn btn-xs btn-success" href="javascript:void(0);" onclick="editdoctransdata('<?= $row['id'] ?>')"><i class="fa fa-pencil"></i></a> &nbsp; 
				<a href="javascript:void(0);" class="btn btn-xs btn-primary" onclick="ajaxdeletetransdata('delete_document_trans_record','<?= $row['id'] ?>');"><i class="fa fa-times"></i></a></td>
				<?php } ?>
			</tr>
	<?php } }else{?>
			<tr>
