        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          <div class="col-sm-6">
			...
			<a href="<?=base_url('admin/'.$viewname);?>" class="btn btn-success howler" title="Back to List">Back to List</a>       
		   <!--<button class="btn btn-danger howler" data-type="danger" onclick="deletepopup1('0');">Delete User</button>-->
          </div>
          <!--<div class="col-sm-6">
          <a class="btn  pull-right btn-success howler" href="<?=base_url('admin/'.$viewname.'/');?>" title="Go to List">Go to List</a>
          </div>-->
         </div>
         <div id="common_div1">
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/user_management/2/"+uri_segment,
			data: {
			result_type:'ajax1',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val(),allflag:allflag
		},
		beforeSend: function() {
	...
	$('body').on('click','#common_tb a.paginclass_A',function(e){
		    $.ajax({
                ...
                url: $(this).attr('href'),
				data: {
                result_type:'ajax1',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
            },
			beforeSend: function() {
						$.blockUI({ message: '<?='<img src="'.base_url('images').'/ajaxloader.gif" border="0" align="absmiddle"/>'?> Please Wait...'});
					  },
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/user_management/2",
					data: {
					result_type:'ajax1',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
	...
	function active_all_plans(name)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('admin_base_url').$viewname.'/2/';?>"+data,
					data: {
					result_type:'ajax1',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val(),allflag:''
				},
				beforeSend: function() {
							$('#common_div1').block({ message: 'Loading...' }); 
						  },
