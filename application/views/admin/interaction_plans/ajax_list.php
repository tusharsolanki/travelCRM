						...
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> id="view_archive_<?php echo  $row['id'] ?>" > 
							...
							<td class="text-center">
								<?php if($row['total_interactions'] > 0){?>
									...
									<?php }else{?>
										<button class="btn btn-xs btn-secondary-new stop_interaction_plan" data-id="<?=$row['id']?>" title="Reset"> <i class="fa fa-stop"></i></button>
									<?php } ?>
							...
							<td class="hidden-xs hidden-sm text-center">
										...
										<? } ?>
										<!--<button class="btn btn-xs btn-primary" onclick="deletepopup1('<?php echo $row['id']; ?>','<?php echo ucfirst(strtolower($row['plan_name'])); ?>');"><i class="fa fa-times"></i></button>&nbsp;-->
