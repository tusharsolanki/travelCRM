                            ...
                            <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> id="view_archive_<?php echo  $row['id'] ?>" > 
                                ...
                                <td class="hidden-xs hidden-sm text-center">
                                    ...
                                    <? } ?>
										...
										<?php if(!empty($this->modules_unique_name) && in_array('communications_delete',$this->modules_unique_name)){?>
                                    <a title="Add to Archive" class="btn btn-xs btn-primary" href="javascript:void(0);" onclick="active_plan_single2('<?php echo  $row['id'] ?>','<?php echo  rawurlencode(ucfirst(strtolower($row['plan_name']))) ?>');"><i class="fa fa-archive"></i></a>
                               <? } ?>
