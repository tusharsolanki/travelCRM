...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
             <?php if(!empty($this->modules_unique_name) && in_array('letter_library_delete',$this->modules_unique_name)){?>
            ...
            <th width="20%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if(isset($sortfield1) && $sortfield1 == 'size_w'){if($sortby1 == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact1('size_w','<?php echo $sorttypepass1;?>')">Letter Size</a></th>
			
            <th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield1) && $sortfield1 == 'template_subject'){if($sortby1 == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact1('template_subject','<?php echo $sorttypepass1;?>')"><?=$this->lang->line('tasksubject_label_name')?></a></th>
            <th width="10%" rowspan="1" colspan="1"><?=$this->lang->line('common_label_preview')?></th>
            ...
            <?php if(in_array('letter_library_edit',$this->modules_unique_name) || in_array('letter_library_delete',$this->modules_unique_name)){ ?>
           ...
           <?php if(!empty($default_datalist) && count($default_datalist)>0){
					...
					?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
                            ...
                            <td class="hidden-xs hidden-sm "  id="temp_category_<?php echo  $row['id'] ?>">
                                <?=!empty($row['category'])?ucfirst(strtolower($row['category'])):'';?>
                            </td>
                            <td class="hidden-xs hidden-sm "  id="temp_size_<?php echo  $row['id'] ?>">
                            ...
                            <td class="hidden-xs hidden-sm "id="temp_name_<?php echo  $row['id'] ?>"><a class="btn btn-secondary view_form_btn" data-toggle="modal" title="<?=$this->lang->line('common_label_preview')?>"  href="#basicModal" data-id="<?php echo  $row['id'] ?>"><?=$this->lang->line('common_label_preview')?></a></td>
							 ...
							 <? if(in_array('letter_library_edit',$this->modules_unique_name) || in_array('letter_library_delete',$this->modules_unique_name)){ ?>
							<td class="hidden-xs hidden-sm ">
                            <?php if(!empty($this->modules_unique_name) && in_array('letter_library_edit',$this->modules_unique_name)){?>
                            ...
                            <a class="btn btn-xs btn-success" title="Edit Record" href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
                             <? } ?>
                             <?php if(!empty($this->modules_unique_name) && in_array('letter_library_delete',$this->modules_unique_name)){?>
                            <button title="Delete Record" class="btn btn-xs btn-primary" onclick="deletepopup2('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['template_name']))) ?>');"><i class="fa fa-times"></i></button>
                            <? } ?>
