          ...
          <thead>
           <tr role="row">
            ...
            <th width="15%" data-direction="desc" data-sortable="true" data-filterable="true" <?php if(isset($sortfield) && $sortfield == 'category'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}} ?> role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="descending" aria-label="Rendering engine: activate to sort column ascending"><a href="javascript:void(0);" onclick="applysortfilte_contact('category','<?php echo $sorttypepass;?>')"><?=$this->lang->line('common_label_category');?></a></th>		
             ...
             <th width="15%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'size_w'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('size_w','<?php echo $sorttypepass;?>')">Letter Size</a></th>
			
            <th width="14%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'template_subject'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('template_subject','<?php echo $sorttypepass;?>')"><?=$this->lang->line('tasksubject_label_name')?></a></th>
			
            <th width="9%" rowspan="1" colspan="1"><?php echo $this->lang->line('common_label_preview')?></th>
            ...
            <?php if(in_array('letter_library_add',$this->modules_unique_name) || in_array('letter_library_edit',$this->modules_unique_name) || in_array('letter_library_delete',$this->modules_unique_name)){ ?>
						...
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
                         <?php if(!empty($this->modules_unique_name) && in_array('letter_library_delete',$this->modules_unique_name)){?>
                            ...
                            <!--<form class="form parsley-form" enctype="multipart/form-data" name="mail_out" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('admin_base_url').'mail_out/mail_out_data'?>" novalidate>
								...
								<input type="hidden" id="template_type" name="template_type" value="Letter" />
                            ...
                            <div id="temp_desc_<?php echo  $row['id'] ?>" style="display:none;">
                             ...
                             <?php if(in_array('letter_library_add',$this->modules_unique_name) || in_array('letter_library_edit',$this->modules_unique_name) || in_array('letter_library_delete',$this->modules_unique_name)){ ?>
							<td class="hidden-xs hidden-sm ">
                            <?php if(!empty($this->modules_unique_name) && in_array('letter_library_add',$this->modules_unique_name)){?>
							<a class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>" title="Copy Record"><i class="fa fa-copy copyicon8"></i></a> &nbsp; 
                                        <? } ?>
