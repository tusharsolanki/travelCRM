      ...
      <div class="portlet-content">
        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
    	 ...
    	 <div class="col-sm-12 col-lg-7">
               ...
               <a title="Back to List" class="btn btn-success howler" href="<?php echo $this->config->item('admin_base_url')?><?php echo $viewname;?>">Back to List</a> 					
          </div>
          ...
          <div class="col-lg-5 col-sm-12 col-xs-12">
           <div class="dataTables_filter" id="DataTables_Table_0_filter">
            <label>
                ...
                <button class="btn btn-success howler" data-type="danger" onclick="contact_search('changesearch');" title="Search Contacts">Search</button>
                <button class="btn btn-success howler" data-type="danger" onclick="clearfilter_contact();" title="View All Contacts">View All</button>
            </label>
           </div>
          </div>
        ...
        <div class="table_large-responsive"> 
         <div id="common_div">
         <?=$this->load->view('admin/'.$viewname.'/archive_ajax_list')?>
         </div>
        </div>
       </div>
       <!-- /.table-responsive --> 
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/contacts/view_archive/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/contacts/view_archive/"+data,
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
	...
	{      
		...
		var user_id = $('#slt_user_type').val();
			...
			success: function(data_temp){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/contacts/view_archive/"+data_temp.page,
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
	...
	{      
			...
			else
			{
				if(name.length > 50)
					name = name.substr(0, 50)+'...';
				var msg = 'Are you sure want to add "'+name+'"';
			}
				$.confirm({'title': 'CONFIRM','message': " <strong> "+unescape(msg)+" "+"<strong>?</strong>",'buttons': {'Yes': {'class': '',
	   'action': function(){
							archive_all(id);
	...
	function archive_all(id)
		{
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/ajax_add_to_active_all';?>",
			dataType: 'json',
			//async: false,
