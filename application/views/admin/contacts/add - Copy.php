...
<span class="pull-right">
		...
		<?php }	?>   
		<a title="Back" class="btn btn-secondary" onclick="history.go(-1)" href="javascript:void(0)"><?php echo $this->lang->line('common_back_title')?></a> 
</span>       
        ...
        <ul class="nav nav-tabs" id="myTab1">
         ...
         <li <?php if($tabid == 3){?> class="active" <?php } ?>> <a title="Extra Information" data-toggle="tab" href="#profilenew"><?=$this->lang->line('contact_add_table_tab3_head');?></a> </li>
          <?php 
			...
			{
		 ?>
          <li <?php if($tabid == 4){?> class="active" <?php } ?>> <a title="Buyer Preferences" data-toggle="tab" href="#buyerpreferences"><?=$this->lang->line('contact_add_table_tab4_head');?></a> </li>
          <?php 
			}
		 	if(!empty($datalist))
			{
		 ?>
          <li <?php if($tabid == 5){?> class="active" <?php } ?>> <a title="Property Information" data-toggle="tab" href="#propertyinformation"><?=$this->lang->line('contact_add_table_tab6_head');?></a> </li>
		 <?php }} ?>
           ...
           <div class="col-sm-12 col-lg-7 new-mar-bot">
            ...
            <div class="row">
			 ...
			 <div class="col-sm-12  col-lg-4">
              ...
              <input id="txt_spousemiddle_name" name="txt_spousemiddle_name" maxlength="50" class="form-control parsley-validated" type="text" onkeypress="return isCharacterKey(event)" value="<?php if(!empty($editRecord[0]['spousemiddle_name'])){ echo $editRecord[0]['spousemiddle_name']; }?>" placeholder="e.g. max">
             </div>
			 <div class="col-sm-12  col-lg-4">
                    ...
                    <select class="form-control parsley-validated" name="slt_user[]" id="slt_user" multiple="multiple">
                       <?php if(!empty($user_list)){
                                foreach($user_list as $row){
                                    ...
                                    <option value="<?=$row['id']?>" <?php if(isset($user_add_list) && is_array($user_add_list) && in_array($row['id'],$user_add_list)){ echo "selected";}?>> <?=$row['plan_name']?>
 ...
 <a class="btn btn-primary" title="cancel" href="javascript:history.go(-1);">Cancel</a>
					...
					<div class="col-sm-12 col-lg-8">
						<div class="row">
							...
							<div class="col-sm-12 col-lg-7 form-group">
							  ...
							  <input id="txt_doc_name" name="txt_doc_name" class="form-control parsley-validated" type="text" value="<?php if(!empty($editRecord[0]['doc_name'])){ echo $editRecord[0]['doc_name']; }?>" placeholder="e.g. Document Name">
							</div>
							
							<div class="col-sm-12 col-lg-7 form-group">
							  <label for="text-input"><?=$this->lang->line('common_label_desc');?></label>
							  <textarea id="txtarea_doc_desc"  placeholder="e.g. Description" name="txtarea_doc_desc" class="form-control parsley-validated"><?php if(!empty($rowtrans['doc_desc'])){ echo $rowtrans['doc_desc']; }?></textarea>
							</div>
              ...
              <div class="col-sm-12 pull-left text-center margin-bottom-10">
                  <input type="submit" title="Save Contact" class="btn btn-secondary-green" value="Save Contact" name="submitbtn" onclick="return contact_step3();" />
				   ...
				   <?php 
                    ...
                    {
                 ?>
                  <input type="submit" title="Save and Continue" class="btn btn-secondary" value="Save and Continue" name="submitbtn" onclick="return contact_step3();" />
                  <?php } ?>
                  <a class="btn btn-primary" title="Cancel" href="javascript:history.go(-1);">Cancel</a>
              </div>
				...
				<div class="delete_field_trans_record<?=$rowtrans['id']?> padding-top-10 clear autooverflow row">
					<div class="col-sm-5 form-group">
					  ...
					  <select class="form-control parsley-validated contact_module select_field_type_change" name="slt_field_typee[]" id="slt_field_type_e_<?=$rowtrans['id']?>">
					   ...
					   <?php if(!empty($field_type)){
								foreach($field_type as $row){?>
									<option data-id="<?=$row['field_type']?>" <?php if(!empty($rowtrans['field_type']) && $rowtrans['field_type'] == $row['id']){ echo "selected"; }?> value="<?=$row['id']?>"><?=ucwords($row['name']);?></option>
								<?php } ?>
					   <?php } ?>
					  </select>
					 </div>
					 <div class="col-sm-5 form-group">
					  ...
					  <input id="txt_field_name_e_<?=$rowtrans['id']?>" name="txt_field_namee[]" class="form-control parsley-validated" type="text" value="<?php if(!empty($rowtrans['field_name'])){ echo $rowtrans['field_name']; }?>" >
					 </div>
    			 <div class="col-sm-1 text-center icheck-input-new">
				  <div class="">
					   <a class="btn btn-xs btn-primary mar_top_con_my" href="javascript:void(0);" title="Delete Field" onclick="return ajaxdeletetransdata('delete_field_trans_record','<?=$rowtrans['id']?>');"> <i class="fa fa-times"></i> </a>
					  </div>
					 </div>
				</div>
               <?php } ?>	
                ...
                <div class="row">   
				 <div class="col-sm-5 form-group">
				  ...
				  <select class="form-control parsley-validated contact_module select_field_type_change" name="slt_field_type[]" id="slt_field_type" onchange="" >
				   ...
				   <?php if(!empty($field_type)){
							foreach($field_type as $row){?>
								<option data-id="<?=$row['field_type']?>" value="<?=$row['id']?>"><?=ucwords($row['name']);?></option>
							<?php } ?>
				   <?php } ?>
				  </select>
				 </div>
              ...
              <label for="text-input"><?=$this->lang->line('contact_add_house_style');?></label>
				...
				<div class="row">
				 <div class="col-sm-8">
				  ...
				  <input id="txt_area_of_interest" name="txt_area_of_interest" class="form-control parsley-validated" type="text" value="<?php if(!empty($editRecord[0]['area_of_interest'])){ echo $editRecord[0]['area_of_interest']; }?>" placeholder="e.g. Interest" />
				 </div>
				</div>
                <div class="row">
				 <div class="col-sm-8">
                 ...
                 <div class="row">
				 <div class="col-sm-8">
				  <label for="text-input"><?=$this->lang->line('contact_add_no_of_bathrooms');?> (e.g. 1.5 Or 2)</label>
				 ...
				 <div class="col-sm-8">
				  <label for="text-input"><?=$this->lang->line('contact_add_buyer_preferences_notes');?></label>
				    <textarea name="textarea_buyer_preferences_notes" placeholder="e.g. Notes" id="textarea_buyer_preferences_notes" class="form-control parsley-validated"><?php if(!empty($editRecord[0]['buyer_preferences_notes'])){ echo $editRecord[0]['buyer_preferences_notes']; }?></textarea>
				 </div>
				</div>
            </div>
         ...
         <?php } ?>
          <?php 
			...
			{
		 ?>
         <div <?php if($tabid == 5){?> class="row tab-pane fade in active" <?php }else{ ?> class="row tab-pane fade in" <?php } ?> id="propertyinformation">
          <div id="common_div">
		  <?=$this->load->view($viewname.'/property_list');?>
          </div>
         </div>
         <?php } ?>
        </div>
             ...
             inlinehtml += '<div class="col-sm-5 form-group">';
              inlinehtml += '<select class="form-control parsley-validated contact_module select_field_type_change" name="slt_field_type[]" id="slt_field_type" onchange="">';
               inlinehtml += '<option value="">Please Select</option>';
							   <?php if(!empty($field_type)){
										foreach($field_type as $row){?>
											inlinehtml += '<option data-id="<?=$row['field_type']?>" value="<?=$row['id']?>"><?=$row['name']?></option>';
										<?php } ?>
							   <?php } ?>
              inlinehtml += '</select>';
             inlinehtml += '</div>';
...
function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/<?=$viewname?>/property_listing/"+uri_segment,
			data: {
