...
<style>
.class_to_remove_border tr td{ border:0!important;}
</style>
<?php
...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter class_to_remove_border" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           <tr role="row">
		   		...
		   		if(!empty($counter_data)){
		   			for($j=0;$j<count($counter_data);$j++)
					{
						...
						if(!empty($datalist[$j]))
						{
							...
							{ ?>
								<tr <?php if($k==0){if(!empty($counter_data[$j]['rowspan']))echo "style='border-top:1px solid #ddd;'";}?> <? if($i%2==1){ ?>class="bgtitle" <? }?> id="tr_<?php echo $datalist[$j][$k]['id'];?>" > 
                                        ...
                                        <td class="hidden-xs hidden-sm ">
                                        	...
                                        	<?php if(!empty($datalist[$j][$k]['full_address'])){
							
												$address=str_replace(', ',',',$datalist[$j][$k]['full_address']);
												$letters = array(',,,,,',',,,,',',,,',',,');
												$fruit   = array(',',',',',',',');
												$text    = $address;
										...
										<td class="hidden-xs hidden-sm text-center valign-middle" rowspan="<?php if(!empty($counter_data[$j]['rowspan']))echo $counter_data[$j]['rowspan'];?>">
											<a data-group="group<?=$j?>" class="btn btn-success merge_btn" data-toggle="modal" data-target=".bs-example-modal-lg" title="Merge Contacs">Merge</a>
...
<script type="text/javascript">
	...
	$('body').on('click','.merge_btn',function(e){
		...
		if(grouperror == 1)
		{
			$.confirm({'title': 'Alert','message': " <strong> Please select contacts from the same group. "+"<strong></strong>",'buttons': {'ok'	: {'class'	: 'btn_center alert_ok'}}});
		...
		{
			$.confirm({'title': 'Alert','message': " <strong> Please select at least two contacts from the same group. "+"<strong></strong>",'buttons': {'ok'	: {'class'	: 'btn_center alert_ok'}}});
		...
		else
		{
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/get_merge_contact_data_ajax';?>",
			//dataType: 'json',
			//async: false,
