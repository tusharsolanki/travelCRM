     ...
     <div class="portlet">
      ...
      <!-- /.portlet-header -->
		...
		<div class="portlet-content">
       	<form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?php echo $this->config->item('admin_base_url').'dashboard/generate_pdf1'?>" novalidate>
        <?php if(count($finlaOutputPrint) > 1){ ?>
        <input type="button" class="btn btn-secondary" value="Print All" onClick="Popup('0')" name="print" />
        <?php } ?>
        <!--<input type="submit" class="btn btn-secondary" value="Download" name="submit" id="downloadallid" />-->
			...
			<div class="table_large-responsive">
				<div class="col-sm-12">
					...
					<li><input type="button" class="btn btn-secondary" value="Print" onClick="Popup(<?=$interaction_id[$i]?>)" name="print" /></li><li>
						<input type="submit" class="btn btn-secondary" value="Download" name="download_<?=$interaction_id[$i]?>" id="download_<?=$interaction_id[$i]?>" onClick="download(<?=$interaction_id[$i]?>)" />
						...
						<input type="hidden" id="size_h_<?=$interaction_id[$i]?>" name="size_h_<?=$interaction_id[$i]?>" value="<?php echo $tmp_size_h[$j]; ?>" />
                          <input type="hidden" id="template_data_<?=$interaction_id[$i]?>" name="template_data_<?=$interaction_id[$i]?>" value="<?=!empty($template_data[$i])?$template_data[$i]:''?>" />
					...
					<li> <a class="btn btn-primary" href="<?php  echo $this->config->item('admin_base_url').'dashboard/letter_label_envelope_task';?>">Cancel</a></li>
					</ul>
				</div>
...
/*$('body').on('click','#downloadallid',function(e){
	$( "input[name*='download_']" ).trigger( "click" );
});*/
    ...
    function Popup(id)
    {
		...
		$.ajax({  
				type: "post",
				url:'<?=base_url('admin/dashboard/insert_data1')?>',
