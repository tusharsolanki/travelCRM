        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
            ...
            <? if(in_array('communications',$this->modules_unique_name)){ }else{$tabid = 4;}?>
             <? if(in_array('communications',$this->modules_unique_name)){ ?>
              <li <?php if($tabid == '' || $tabid == 1){?> class="active" <?php } ?> > <a title="Mailings" data-toggle="tab" href="#Mailings" onclick ="contact_search('changesearch','1');">Mailings</a> </li>      
               ...
               <? } ?>
               <li <?php if($tabid == 6){?> class="active" <?php } ?>> <a title="All Actions" data-toggle="tab" href="#All_Actions" onclick ="contact_search('changesearch','6');">To-Do</a> </li>
           </ul>
         ...
         <? if(in_array('communications',$this->modules_unique_name)){ ?>
            ...
            <div class="<?php if($tabid == 2){?> tab-pane fade in active<?php }else{ ?> tab-pane fade in <?php } ?>" id="Calls" >
                 <div class="row dt-rt">
                  <div class="col-sm-12">
                   <div class="dataTables_filter" id="DataTables_Table_0_filter">
                    <label>
                         <input class="" type="hidden" name="uri_segment" id="uri_segment" value="<?=!empty($uri_segment)?$uri_segment:'0'?>">
                        <input class="searchtext" type="text" name="searchtext_2" id="searchtext_2" title="Search Text" aria-controls="DataTables_Table_0" placeholder="Search..." value="<?=!empty($searchtext)?$searchtext:''?>">
                 ...
                 <div id="common_div_2">
                 <?=$this->load->view('admin/home/telephone_task_ajax_list')?>
                 </div>
            </div>
            <div class="<?php if($tabid == 3){?> tab-pane fade in active<?php }else{ ?> tab-pane fade in <?php } ?>" id="Text" >
                 <div class="row dt-rt">
                  <div class="col-sm-12">
                   <div class="dataTables_filter" id="DataTables_Table_0_filter">
                    <label>
                        <input class="searchtext" type="text" name="searchtext_3" id="searchtext_3" title="Search Text" aria-controls="DataTables_Table_0" placeholder="Search..." value="<?=!empty($searchtext)?$searchtext:''?>">
                 ...
                 <div id="common_div_3">
                 <?=$this->load->view('admin/home/sms_ajax_list',!empty($sms_data)?$sms_data:'');?>
                 </div>
            </div>
            <? } ?>
            <div class="<?php if($tabid == 4){?> tab-pane fade in active<?php }else{ ?> tab-pane fade in <?php } ?>" id="Tasks" >
                  ...
                  <div class="col-sm-12">
                   <div class="dataTables_filter" id="DataTables_Table_0_filter">
                    <label>
                        ...
                        <input class="searchtext" type="text" name="searchtext_4" id="searchtext_4" title="Search Text" aria-controls="DataTables_Table_0" placeholder="Search..." value="<?=!empty($searchtext)?$searchtext:''?>">
                 ...
                 <div id="common_div_4">
                 <?php if($tabid == 4){ 
                 	echo $this->load->view('admin/home/task_ajax_list');
                  } ?>
                 </div>
            </div>
             <? if(in_array('communications',$this->modules_unique_name)){ ?>
            <div class="<?php if($tabid == 5){?> tab-pane fade in active<?php }else{ ?> tab-pane fade in <?php } ?>" id="Emails" >
                 ...
                 <div id="common_div_5">
                 <?=$this->load->view('admin/home/email_ajax_list',!empty($emails_data)?$emails_data:'');?>
                 </div>
            </div>
             <? } ?>
            <div class="<?php if($tabid == 6){?> tab-pane fade in active<?php }else{ ?> tab-pane fade in <?php } ?>" id="All_Actions" >
                 ...
                 <div id="common_div_6">
                 <?=$this->load->view('admin/home/to_do_task_ajax_list')?>
                 </div>
            </div>
         </div>
         <input type="hidden" id="tab_id" name="tab_id" value="<?=!empty($tabid)?$tabid:'1'?>" />
         <input type="hidden" id="url" name="url" value="<?php echo base_url();?>admin/dashboard/to_do_task" />
         </div>
        </div>
       </div>
       <!-- /.table-responsive --> 
...
<script>
	...
	function contact_search(allflag,tab)
	{
		...
		//var tab_id = 1;
		if(tab == 1)
			url = "<?php echo base_url();?>admin/dashboard/letter_label_envelope_task";
		else if(tab == 2)
			url = "<?php echo base_url();?>admin/dashboard/telephone_task";
		else if(tab == 3)
			url = "<?php echo base_url();?>admin/dashboard/sms_task"
		else if(tab == 4)
			url = "<?php echo base_url();?>admin/dashboard/daily_task";
		else if(tab == 5)
			url = "<?php echo base_url();?>admin/dashboard/email_task";
		else if(tab == 6)
			url = "<?php echo base_url();?>admin/dashboard/to_do_task";
		$("#tab_id").val(tab);
		$("#url").val(url);
	...
	function delete_all(id)
	{
		var tab = $("#tab_id").val();
		if(tab == 4)
			var url = "<?php echo $this->config->item('admin_base_url').'dashboard/ajax_delete_all';?>"
		else
			var url = "<?php echo $this->config->item('admin_base_url').'dashboard/ajax_delete_task';?>"
...
function iscompleted(id,value)
{
	...
	if(tab == 4)
	{
		var url = "<?php echo base_url("admin/dashboard/iscompleted");?>";
	...
	else
	{
		if(value == '3')
			...
			var msg = "Are you sure that the task is completed";
		var url = "<?php echo base_url("admin/dashboard/is_completed");?>";
	}
...
'action': function(){
	$.ajax({
			...
			url: url,
			data: {
			selectedvalue:id,disposition:value,interaction_type:tab,selecteddate:$("#task_date").val()
		},
		beforeSend: function() {
						$.blockUI({ message: '<?='<img src="'.base_url('images').'/ajaxloader.gif" border="0" align="absmiddle"/>'?> Please Wait...'});
					  },
...
function resend_mail(id,contact_id)
{
	...
	if(tab == 3)
		url = "<?php echo $this->config->item('admin_base_url').'dashboard/send_sms/';?>"
	else
...
function phone_call(uri_segment){
	var frameSrc = '<?= $this->config->item('admin_base_url'); ?>dashboard/phone_call_popup/'+$("#sortfield_2").val()+'/'+$("#sortby_2").val()+'/'+uri_segment;
