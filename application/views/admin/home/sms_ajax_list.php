...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           ...
           <?php if(!empty($sms_datalist) && count($sms_datalist)>0){
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($sms_datalist as $row){?>
						<tr class="<? if($i%2==1) echo 'bgtitle '; if(strtotime($row['task_date']) < strtotime($dt)) echo 'new_red_class'; ?>" > 
                            ...
                            <!--<input type="hidden" id="temp_subject_<?=$row['id']?>" value="<?=!empty($row['template_subject'])?$row['template_subject']:''?>"/>
                            <input type="hidden" id="temp_name_<?=$row['id']?>" value="<?=!empty($row['template_name'])?$row['template_name']:''?>"/>-->
									...
									if(!empty($row['sms_message']))
									{
										...
										{?>
                                            ...
                                            <!--<div class="div_dash_more_data div_dash_display_none">
                                            	<?php echo ucfirst(strtolower($row['sms_message']));?>
                          ...
                          <div id="temp_desc_<?php echo  $row['id'] ?>" style="display:none;"><?=!empty($row['sms_message'])?ucfirst(strtolower($row['sms_message'])):''?></div>
          <?php } ?>
