       ...
       <div class="">
        <div class="tab-content" id="myTab1Content">
         ...
         <div class="tab-pane fade in active" id="home">
          ...
          <form class="form parsley-form" enctype="multipart/form-data" name="<?php echo $viewname;?>" id="<?php echo $viewname;?>" method="post" accept-charset="utf-8" action="<?= $this->config->item('admin_base_url'); ?>dashboard/phone_call_popup/<?=$this->uri->segment(4)?>/<?=$this->uri->segment(5)?>" novalidate data-validate="parsley">
		  <input id="id" name="id" type="hidden" value="<?php if(!empty($editRecord[0]['id'])){ echo $editRecord[0]['id']; }?>">
		   ...
		   <div class="row">
             ...
             <div class="col-sm-4 form-group">
              ...
              <input id="contact_name" name="contact_name" class="form-control parsley-validated" type="text" value="<?=!empty($editRecord[0]['contact_name'])?$editRecord[0]['contact_name']:''?>" readonly="readonly" >
             </div>
            ...
            <div class="row">
             ...
             <?php if(!empty($contact_disposition_master) && count($contact_disposition_master) > 0) { 
             	...
             	<input <?php if($i==0) { echo "checked=checked"; $i++; } ?> type="radio" name="disposition" id="disposition_<?=$row['id']?>" value="<?=$row['id']?>" onclick="addtxt(this.value);" />&nbsp; <?=$row['name']?> &nbsp; <br />
			 <?php 
             			}
					}
			 ?>
             ...
             <div class="col-sm-4 form-group disposition" style="display:none;">
              ...
              <input id="selecteddate" name="selecteddate" class="form-control" type="text" value="<?=date('Y-m-d',strtotime(date('Y-m-d')."+1 Days"))?>">
               ...
               <?php if(empty($editRecord[0]['interaction_id']) ||(!empty($editRecord[0]['interaction_id']) && $editRecord[0]['is_done'] == '1')) { ?>
                ...
                <input type="submit" class="btn btn-secondary" id="Next" value="Next" title="Next" name="nextbtn" />
                <?php } ?>
         	</div>
