      ...
      <div class="col-md-12">
        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <a class="btn  pull-right btn-secondary-green howler" href="<?=base_url('admin/'.$viewname.'/add_record');?>" title="Add Form">Add Form</a>
          <? } ?>
          </div>
         </div>
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/lead_capturing/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/lead_capturing/"+data,
					data: {
...
<script type="text/javascript">
	$('body').on('click','.view_form_btndata',function(e){
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/view_form_data';?>",
			data: {'id':formid},
			beforeSend: function() {
