...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           ...
           <?php if(!empty($this->modules_unique_name) && in_array('form_builder_delete',$this->modules_unique_name)){?>
            ...
            <th width="26%" class="hidden-xs hidden-sm <?php if(isset($sortfield) && $sortfield == 'plan_name'){if($sortby == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact('plan_name','<?php echo $sorttypepass;?>')"><?=$this->lang->line('contact_add_interaction_plan')?></a></th>
			<? } ?>
            <th width="19%" rowspan="1" colspan="1"><?=$this->lang->line('embed_form')?></th>
             <th width="19%" class="hidden-xs hidden-sm sorting_disabled actionbtn6" data-filterable="true" role="columnheader" rowspan="1" colspan="1" aria-label="CSS grade"><?php echo $this->lang->line('common_label_action')?></th>
           </tr>
           </thead>
					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
                        ...
                        <?php if(!empty($this->modules_unique_name) && in_array('form_builder_delete',$this->modules_unique_name)){?>
                            ...
                            <!-- <a href="<?= $this->config->item('admin_base_url'); ?>lead_capturing_view/<?= $row['id'] ?>" class="textdecoration"><?=!empty($row['form_title'])?ucfirst(strtolower($row['form_title'])):'';?></a> -->
