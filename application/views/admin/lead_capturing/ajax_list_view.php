					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<td class="hidden-xs hidden-sm "><a title="View Lead" href="<?= $this->config->item('admin_base_url').$viewname; ?>/view_record/<?= $row['id'] ?>"><?=!empty($row['form_title'])?ucfirst(strtolower($row['form_title'])):'';?></a></td>
							...
							<a class="btn btn-xs btn-success view_form_btndata" data-toggle="modal" href="#basicModal1" title="Preview Record" data-id="<?=!empty($row['id'])?$row['id']:'';?>"><i class="fa fa-search"></i></a> &nbsp; 
										...
										<button class="btn btn-xs btn-primary" title="Delete Record" onclick="deletepopup1('<?php echo $row['id'] ?>','<?php echo ucfirst(strtolower($row['form_title'])) ?>');"><i class="fa fa-times"></i></button>
