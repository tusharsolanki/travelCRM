            ...
            <div class="col-md-12">
                <div class="portlet">
					...
					$this->session->set_userdata('message_session', $newdata);?> </div><?php } ?>
                                ...
                                <div id="common_div">
                                    <?=$this->load->view('admin/'.$viewname.'/property_contact_form_ajax_list')?>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function contact_search(allflag)
    {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo $this->config->item('admin_base_url')?>leads_dashboard/property_contact_form/<?=$contact_id?>/"+uri_segment,
            data: {
