					...
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
                      foreach($datalist as $row){
						...
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
                            ...
                            <td class="hidden-xs hidden-sm "><?=!empty($row['platform'])?$row['platform']:'';?></td>
							<td class="hidden-xs hidden-sm text-center">
							<a title="Copy Template" class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>"><i class="fa fa-copy"></i></a> &nbsp; 
