          ...
          <thead>
           <tr role="row">
            ...
            <th width="20%" class="hidden-xs hidden-sm <?php if(isset($sortfield1) && $sortfield1 == 'template_subject'){if($sortby1 == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact1('template_subject','<?php echo $sorttypepass1;?>')"><?=$this->lang->line('tasksubject_label_name')?></a></th>
             ...
             <th width="10%" class="hidden-xs hidden-sm <?php if(isset($sortfield1) && $sortfield1 == 'platform'){if($sortby1 == 'asc'){echo "sorting_desc";}else{echo "sorting_asc";}} ?> " data-filterable="false" role="columnheader" rowspan="1" colspan="1" aria-label="Engine version"><a href="javascript:void(0);" onclick="applysortfilte_contact1('platform','<?php echo $sorttypepass1;?>')"><?=$this->lang->line('platform_label_name')?></a></th>
              ...
              <? if(in_array('social_media_posts_edit',$this->modules_unique_name) || in_array('social_media_posts_delete',$this->modules_unique_name)){ ?>
							...
							<td class="hidden-xs hidden-sm">
                             <?php if(!empty($this->modules_unique_name) && in_array('social_media_posts_edit',$this->modules_unique_name)){?>
										<a title="Edit Template" class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp;
                                         <? } ?>
                                         <?php if(!empty($this->modules_unique_name) && in_array('social_media_posts_delete',$this->modules_unique_name)){?> 
										<button class="btn btn-xs btn-primary" title="Delete Template" onclick="deletepopup2('<?php echo $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['template_name']))); ?>');"><i class="fa fa-times"></i></button>
									<? } ?>
