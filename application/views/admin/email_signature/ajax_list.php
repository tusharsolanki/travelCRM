<?php 
    /*
        @Description: Admin email Signature list
        @Author: Ruchi Shahu
   		@Date: 02-08-2014
    */
...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
  ...
  <tbody role="alert" aria-live="polite" aria-relevant="all">
    ...
    <tr <? if($i%2==1){ ?>class="bgtitle" <? }?> >
      ...
      <td class="hidden-xs hidden-sm "><?php echo  ucfirst(strtolower($row['full_signature'])) ?>
      ...
      <? if(in_array('email_signature_edit',$this->modules_unique_name) || in_array('email_signature_delete',$this->modules_unique_name)){ ?>
      <td class="hidden-xs hidden-sm text-center"><!--<span class="pubunpub_span_<?php echo  $row['id'] ?>">
        ...
        <? } ?>
        <?php /*
        <button class="btn btn-xs btn-primary" onclick="deletepopup('<?=addslashes($row['signature_name'])?>','','<?php echo $this->config->item('admin_base_url').$viewname;?>/delete_record/<?php echo  $row['id'] ?>');"> <i class="fa fa-times"></i> </button>
      ...
      <div id="DataTables_Table_0_length" class="dataTables_length row pagignation_margin_right">
        <label>
          <select name="DataTables_Table_0_length" size="1" aria-controls="DataTables_Table_0" onchange="changepages();" id="perpage">
            <option value="">
            <?=$this->lang->line('label_email_sign_per_page')?>
            </option>
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search()
		{
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>admin/email_signature/",
				data: {
				result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
			},
			beforeSend: function() {
...
function changedefaulttemplate(value)
{
	$.ajax({
			type: "POST",
			url: '<?php echo base_url("admin/email_signature/changedefaulttemplate");?>',
			data: {
			selectedvalue:value
		},
			success: function(html){
...
function changemark(id)
{
	...
	$.ajax({
		type: "POST",
		url: "<?php echo base_url();?>admin/email_signature/ifselected/",
		data: {
			id : app_id
		},
