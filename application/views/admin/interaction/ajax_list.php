          ...
          <?php if(!empty($datalist) && count($datalist)>0){
					$i=!empty($this->router->uri->segments[4])?$this->router->uri->segments[4]+1:1;
					  ...
					  foreach($datalist as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<a href="<?= $this->config->item('admin_base_url'); ?>interaction/view_record/<?= $row['id'] ?>" class="textdecoration">
					...
					<?php if($row['start_type'] == '1'){
							...
							<td class="hidden-xs hidden-sm text-center">
										<a title="Edit Action" class="btn btn-xs btn-success" href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?= $row['id'] ?>"><i class="fa fa-pencil"></i></a> &nbsp; 
