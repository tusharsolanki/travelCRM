        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <div class="col-sm-6">
		   ...
		   <a class="btn  pull-right btn-success howler margin-left-5px" href="<?=base_url('admin/'.$viewname."/".$plan_id.'/view_archive');?>" title="View Archive">View Archive</a>
           <?php if(isset($datalist[0]['plan_status']) && $datalist[0]['plan_status'] == 0){}else{ ?>
          <a class="btn  pull-right btn-secondary-green howler" href="<?=base_url('admin/'.$viewname.'/add_record/'.$plan_id);?>" title="Add Action">Add Action</a>
		  <?php } ?>
	...
	function active_all_plans(name)
		{
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname."/".$plan_id.'/ajax_Inactive_all';?>",
			dataType: 'json',
			//async: false,
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/interaction/<?php echo $plan_id.'/'; ?>"+data.page,
					data: {
...
<script>
	  ...
	  function applysortfilte_contact(sortfilter,sorttype)
		...
		function contact_search(allflag)
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url();?>admin/interaction/<?php echo $this->router->uri->segments[3]; ?>/"+uri_segment,
				data: {
...
$('body').on('click','.view_contacts_btn',function(e){
			...
			/*$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').'interaction/view_contacts_of_interaction_by_step';?>",
