          ...
          <div class="portlet">
			...
			<?php if(!empty($editRecord[0]['interaction_plan_id'])){?>
                        <span class="pull-right"> 
                             	<a title="View Contact" class="btn btn-secondary" href="<?= $this->config->item('admin_base_url').$viewname; ?>/view_record/<?=$inte_plan_id?>"><?php echo $this->lang->line('common_view_title')?></a> 
                           		<a title="Back" class="btn btn-secondary" href="<?php echo $this->config->item('admin_base_url')?><?php echo $viewname.'/'.$editRecord[0]['interaction_plan_id'];?>"><?php echo $this->lang->line('common_back_title')?></a> 
                        </span>
			  <?php }
			  else {?>
                  <span class="pull-right">
                           <a title="Back" class="btn btn-secondary" href="<?php echo $this->config->item('admin_base_url')?><?php echo $viewname.'/'.$inte_plan_id;?>"><?php echo $this->lang->line('common_back_title')?></a>
                   </span> 			<?php } ?>
            </div>
            <!-- /.portlet-header -->
                      ...
                      <div class="text-center pull-right"> 
					  		<?php if(!empty($editRecord)){?> 
								<?php if(!empty($previous_interaction)){ ?>
									<a href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?=$previous_interaction?>"><i class="fa fa-backward btn btn-prev-next-avail arrow"></i></a>
								<?php }else{ ?>
									<a href="javascript:void(0);"><i class="fa fa-backward btn btn-tertiary arrow"></i></a>
								<?php } ?>
								
								<?php if(!empty($next_interaction)){ ?>
									<a href="<?= $this->config->item('admin_base_url').$viewname; ?>/edit_record/<?=$next_interaction?>"><i class="fa fa-forward btn btn-prev-next-avail arrow"></i></a>
								<?php }else{ ?>
...
<option value="<?php echo $interaction_type[$i]['id'];?>" <?php if(!empty($editRecord[0]['interaction_type']) && $editRecord[0]['interaction_type'] == $interaction_type[$i]['id']){ echo "selected=selected"; } ?>><?php echo $interaction_type[$i]['name'];?></option>
      ...
      <input type="text" class="form-control" id="txt_interaction_stat_1" value="<?php if(isset($editRecord[0]['number_count']) && $editRecord[0]['start_type'] == '1'){ echo $editRecord[0]['number_count'];}?>" name="txt_interaction_stat_1" placeholder="Number" onkeypress="return isNumberKey(event);" >
                      ...
                      <div class="row">
                        <div class="col-xs-12">
                          <fieldset class="edit_main_div template_main_div">
                            ...
                            <div class="row">
							 <div class="col-sm-12">
							  <label for="text-input"><?=$this->lang->line('common_label_category');?></label><span class="mandatory_field add_star"></span>
							  </div>
							  <div class="col-sm-12">
							...
							<div class="row">
								<div class="col-sm-12 topnd_margin">
                                 <a href="#basicModal" class="text_color_red text_size add_new_category" id="basicModal" data-toggle="modal"><i class="fa fa-plus-square"></i> Add Category </a>
									<!--<a class="text_color_red text_size" target="_blank" title="Add Category" href="<?=$this->config->item('admin_base_url').'marketing_library_masters/add_record'?>">
							 ...
							 <div class="col-sm-12 mrgtop1 div_only_email div_attachment">
                              <div class="col-sm-12 mrgtop1 only_email">
								...
								<?php if(isset($attachment) && count($attachment) > 0) { 
										$this->load->view('admin/interaction/attachmentlist');
									}
								 ?>
							 </div>
							 </div>
                    ...
                    <!--<div class="col-lg-4 col-sm-12 show_bomb_div">
						<div class="row">
                        <div class="col-xs-12">
                          <fieldset class="edit_main_div">
                            <legend class="edit_title">Bomb Bomb Emails</legend>
							 <div class="col-xs-12 mrgtop1">
                              <label for="text-input" style="display:block;"><?=$this->lang->line('send_auto');?>:</label>
                              <div class="row">
...
$(document).ready(function(){
	var settings = {
		url: "<?php echo base_url();?>admin/emails/upload_image",
		method: "POST",
...
<script type="text/javascript">
	...
	function selectnewcategory()
	{
		$.ajax({
		 type: "POST",
		 url: "<?php echo $this->config->item('admin_base_url').$viewname.'/ajax_selectcategory';?>",
		 dataType: 'json',
		 data: {},
		 cache: false,
		 success: function(result){
