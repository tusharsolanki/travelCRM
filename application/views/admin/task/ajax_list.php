...
<table class="table table-striped table-bordered table-hover table-highlight table-checkable dataTable-helper dataTable datatable-columnfilter" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
           ...
           <?php if(!empty($this->modules_unique_name) && in_array('tasks_delete',$this->modules_unique_name)){?>
            ...
            <th width="6%" class="hidden-xs hidden-sm text-center"><?=$this->lang->line('completed_name')?></a></th>
							...
							<td class="hidden-xs hidden-sm text-center">
                            			...
                            			<?php if($this->uri->segment(3) != 'completed_task'){  ?>
                                          <?php if(!empty($this->modules_unique_name) && in_array('tasks_add',$this->modules_unique_name)){?>
                                            <a class="btn btn-xs btn-success" title="Copy Record" href="<?= $this->config->item('admin_base_url').$viewname; ?>/copy_record/<?= $row['id'] ?>/<?=$task_status?>"><i class="fa fa-copy copyicon"></i></a> &nbsp; 
                                            <? } ?>
...
'action': function(){
	$.ajax({
			type: "POST",
			url: '<?php echo base_url("admin/task/iscompleted");?>',
			data: {
			selectedvalue:value
		},
		beforeSend: function() {
