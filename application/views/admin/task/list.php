       ...
       <div class="table_large-responsive">
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="row dt-rt">
          ...
          <div class="col-sm-6">
           <!--<a class="btn btn-secondary pull-right btn-success howler margin-left-5px" title="Completed Task" href="<?=base_url('admin/'.$viewname.'/completed_task');?>">Completed Task</a>-->
           <?php if(!empty($this->modules_unique_name) && in_array('tasks_add',$this->modules_unique_name)){?>
          <a class="btn btn-secondary pull-right btn-secondary-green howler" title="Add Task" href="<?=base_url('admin/'.$viewname.'/add_record');?>">Add Task</a>
          <? } ?>
          </div>
         </div>
         <div id="common_div">
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/task/"+uri_segment,
			data: {
	...
	function delete_all(id)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/task/"+data,
					data: {
