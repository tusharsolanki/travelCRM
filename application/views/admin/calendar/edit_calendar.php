...
<?php if(empty($event)) {
?>  
	.tr_repeat_hourly, .tr_repeattype, .tr_repeat_daily, .tr_repeat_yearly, .tr_repeat_weekly, .tr_repeat_monthly, .tr_end_template, .tr_agent_listing {
 display: none;}
 <?php
} else {
...
<?php
 ...
 display: none;}  
    <?php } 
       if(!empty($event) && $event[0]['event_for'] == '1') { ?>
            .tr_agent_listing {display:none;}
       <?php } 
       if(!empty($event) && $event[0]['event_inserted_type'] == '2') { ?>
            #rdo_agent_event {display:none;}
       <?php } 
       
       ?>

    .ui-multiselect{width:350px !important;}
</style>
...
<script language="javascript" type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#Savebtn").click(function(){
            ...
            var event_for = $('input[name=create_event_for]:checked', '#fmEdit').val();
            var sel_user = $('#slt_user_type').val();
            type=$("#type").val();
            cur_date= Date();
            ...
            else if(event_for == '2' && (sel_user == null || sel_user == ''))
            {
                $.confirm({'title': 'Alert','message': " <strong> Please select atleast one user</strong>",'buttons': {'ok'	: {'class'	: 'uibutton large btn_center alert_ok'}}});
                $.unblockUI();
                return false;
            }
            else
        ...
        $("#Deletebtn").click(function(){
            ...
            if(confirm("Do you confirm to Cancel this Appointment")){
                ...
                $.post(DATA_FEED_URL + "admin/calendar/delete_calender",
                param,
                function(data){
                        if(data.IsSuccess)
                        {
...
<div id="mainfrm1">
  <div class="infocontainer">
      ...
      <form action="<?=base_url('admin/calendar/update_calender')?>" class="fform" id="fmEdit" method="post">
      ...
      <div class="date-hit-new654">
        ...
        <select id="stparttime" name="stparttime" style="width:115px;  height:33px !important; min-height:33px !important;" class="select_box form-control">
      ...
      <div class="date-hit-new654">
        ...
        <select id="etparttime" name="etparttime" style="width:115px; height:33px !important; min-height:33px !important;" class="select_box form-control">
          <option value="00:00">12:00 AM</option>
          <option value="00:15">12:15 AM</option>
          <option value="00:30">12:30 AM</option>
          <option value="00:45">12:45 AM</option>
      ...
      <table width="370" style="float:left;">
        ...
        <tr>
          <td width="127">
            <label><input type="checkbox" <?=!empty($event[0]['is_email']) && $event[0]['is_email']=='1'?'checked=checked':'';?>  id="is_email" name="is_email" />
            Email Before</label></td>
        ...
        <tr>
          <td>
            <label><input type="checkbox" <?=!empty($event[0]['is_popup']) && $event[0]['is_popup']=='1'?'checked=checked':'';?> cols="20" id="is_popup" name="is_popup" rows="2"/>
            Pop-Up Before</label></td>
          <td align="right">
        ...
        <tr>
          <td height="30">
            <label><input type="checkbox"  <?=!empty($event[0]['is_pop_by']) && $event[0]['is_pop_by']=='1'?'checked=checked':'';?>  id="pop_by" class="" name="pop_by" />
            Pop-By</label>
          </td>
        </tr>
        <tr>
          <td height="30">
            <label><input type="checkbox" cols="20" <?=!empty($event[0]['is_gift']) && $event[0]['is_gift']=='1'?'checked=checked':'';?> id="gift" name="gift" rows="2"/>
            Gift </label></td>
        </tr>
          
          <tr>
          <td colspan="2"><b class="mrg12">Create Event for:</b></td>
        </tr>
        <tr>
            <td colspan="3" height="30">
                <label class="lblwidth">
                    <input type="radio" name="create_event_for" class="cls_create_event_for" value="1" <?php if(!empty($event[0]['event_for']) && $event[0]['event_for'] == '1'){ echo 'checked="checked"'; } else { echo 'checked="checked"';}?>  />
                    Own </label>
              <label class="lblwidth" id="rdo_agent_event">
                    <input type="radio" name="create_event_for" class="cls_create_event_for" value="2" <?php if(!empty($event[0]['event_for']) && $event[0]['event_for'] == '2'){ echo 'checked="checked"'; ?><?php } ?> />
                    Agent </label>
          </td>
        </tr>
          <tr class="tr_agent_listing">
              <td colspan="3">
                    <select class="form-control ui-widget-header" name="slt_user_type[]" id="slt_user_type" multiple="multiple">
                           <!--<option value="">Please Select</option>-->
                        <?php if(!empty($user_list)){
                                 foreach($user_list as $row){
                                    $email = !empty($row['email_id'])?"(".$row['email_id'].")":'' ?>
                                     <option value="<?=$row['id']?>" <?php if(!empty($slt_user) && is_array($slt_user) && in_array($row['id'],$slt_user)){ echo "selected";}?>>
	  ...
	  <div class="clear"></div>
      <table width="97%" border="0" class="recbor" >
        <tr>
          <td colspan="2">
            <table width="100%">
			  	...
			  	<td valign="top">
					<table>
						...
						<tr class="tr_repeat_hourly">
						  ...
						  <tr class="tr_repeat_daily">
							<td colspan="3">
							  <label>
								...
								Every </label>
							  <input style="float:none;" type="text" id="everyDays" class="number form-control" onkeypress='return isNumberKey(event);' name="everyDays" value="<?=!empty($event[0]['everyDays']) && $event[0]['dailyType']== '1' && $event[0]['repeatType'] == '2'?$event[0]['everyDays']:'1'?>" />
							  Days </td>
						  </tr>
						  <tr class="tr_repeat_daily">
							<td colspan="3">
						  ...
						  <tr class="tr_repeat_weekly">
							<td colspan="3"> Repeat at Every
							  <input style="float:none;"type="text" id="everyWeeks" name="everyWeeks" class="number form-control" onkeypress='return isNumberKey(event);' value="<?=!empty($event[0]['everyWeeks']) && $event[0]['repeatType'] == '3'?$event[0]['everyWeeks']:'1'?>" />
							  Weeks on </td>
						  </tr>
						  <tr class="tr_repeat_weekly">
							<td colspan="3">
						...
						<label style="width:50px; display:block; margin-top:4px; margin-bottom:0px;">
							  ...
							  <input type="text" id="monthDate" name="monthDate" max="31" min="1"  class="number form-control mrg1" onkeypress='return isNumberKey(event);' style="width:132px;" value="<?=!empty($event[0]['monthDate']) && $event[0]['repeatType'] == '4' && $event[0]['monthlyType'] == 1?$event[0]['monthDate']:'1'?>" />
							  <td>
							 <label style="width:45px;"> of every</label>
							  <input type="text" id="monthCount" name="monthCount" class="number form-control" onkeypress='return isNumberKey(event);' style="width:45px;" value="<?=!empty($event[0]['monthCount']) && $event[0]['repeatType'] == '4' && $event[0]['monthlyType'] == 1?$event[0]['monthCount']:'1'?>" />
							  </td>
							 </td>
							<td>month </td>
						  </tr>
						...
						<label style="width:50px; display:block; margin-top:4px; margin-bottom:0px;">
							 ...
							 <td> <label style="width:45px;">of every</label>
							  <input type="text" id="nthMonthCount" name="nthMonthCount" class="number form-control" onkeypress='return isNumberKey(event);' style="width:45px;" value="<?=!empty($event[0]['nthMonthCount']) && $event[0]['monthlyType'] == 2 && $event[0]['repeatType'] == '4'?$event[0]['nthMonthCount']:'1'?>" /></td>
							  </td>
							  	<td>month </td>
						  </tr>
							...
							<td colspan="3">
							  <label>
								...
								Every </label>
							  <input style="float:none;" type="text" id="everyYears" class="number form-control" onkeypress='return isNumberKey(event);' name="everyYears" value="<?=!empty($event[0]['everyYears']) && $event[0]['repeatType'] == '5'?$event[0]['everyYears']:'1'?>" />
							  Years </td>
						  </tr>
					</table>
							</td>
					...
					<table width="100%">
						...
						<tr>
							<td width="12%" valign="middle">
								...
								<label><input type="radio" name="endTemplateType" class ="cls_endTemplateType" value="1" <?php if(isset($event[0]['endTemplateType'])){ if($event[0]['endTemplateType'] == 1 && $event[0]['ifRepeat'] == '1'){ echo 'checked="checked"';} }else{ echo 'checked="checked"';}?> />
								Never	</label>						</td>
							<td width="12%" valign="middle">
						
								<label><input type="radio" name="endTemplateType" class ="cls_endTemplateType" value="2" <?php if(!empty($event[0]['endTemplateType']) && $event[0]['endTemplateType'] == 2 && $event[0]['ifRepeat'] == '1'){ echo 'checked="checked"'; ?><?php } ?> />
								After	</label>						</td>
							<td width="30%" valign="middle"> <input type="text" id="endCounter" name="endCounter" class="number form-control inputbox" onkeypress='return isNumberKey(event);' value="<?=!empty($event[0]['endCounter'])?$event[0]['endCounter']:'1'?>" style="float:none;" />
						  Occurence </td>
							<td width="15%" valign="middle">
							
							 <label><input type="radio" name="endTemplateType" class ="cls_endTemplateType" value="3" <?php if(!empty($event[0]['endTemplateType']) && $event[0]['endTemplateType'] == 3 && $event[0]['ifRepeat'] == '1'){ echo 'checked="checked"'; ?><?php } ?> /> End on </label>
    ...
    ?>
            ...
            <tr>
            <td> <label><input type="radio" value="1"  id="event_type" class="required" name="event_type" /> Only this event</label></td>
            <td> <label><input type="radio" value="2" id="event_type" class="required" name="event_type" />Following events</label> </td>
            <td> <label><input type="radio" value="3" id="event_type" class="required" name="event_type" />All events</label></td>
            </tr>
            </table>
    <?php }
    } ?>
  ...
  <!-- Infocontainer End -->
  <div style="clear: both"></div>
  <!-- onclick="return select_box();" -->
  <div class="toolBotton btn-hit-654"> <a id="Savebtn" class="savebtn1 btn btn-success" href="javascript:void(0);"> <span class="Save"  title="Save the calendar">Save</span> </a>
...
<script language="javascript" type="text/javascript">
	$(document).ready(function()
	{
		...
		else
		{
                    ...
                    <?php if(!empty($event)) { ?>
                        for (var i = 0, len = options.length; i < len; ++i) {//alert(options[i].value); alert('<?php //$strtm_time?>');
		...
		else
		{
                ...
                <?php 
                if(!empty($event)) { ?>
                    var efor = $('input[name=create_event_for]:checked', '#fmEdit').val();
                    ...
                    if(efor == 2) {
                        //$(".tr_agent_listing").show();
                        $('#slt_user_type').addClass('required');
                    }
...
<script type="text/javascript">
	...
	$('.cls_repeatType').die( 'click').live('click', function() { 
        ...
        /*New Added 12-01-2015 Sanjay Moghariya*/
        //$( ".cls_create_event_for" ).click();
        $('.cls_create_event_for').die( 'click').live('click', function() { 
            remove_validatorclass();
            var event_for = $('input[name=create_event_for]:checked', '#fmEdit').val();
            
            if(event_for == 1) /*Own event*/
            {
                $(".tr_agent_listing").hide();
                $("#slt_user_type").removeClass('required');
            }
            else if(event_for == 2) /*Agent event*/
            {
                $(".tr_agent_listing").show();
                $("#slt_user_type").addClass('required');
            }
        ...
        function select_box()
        {
                var abc = $("#slt_user_type").multiselect("widget").find(":checkbox").filter(':checked').length;
                if(abc > 0)
                {}
                else
                {
                    ...
                    $.confirm({'title': 'Alert','message': " <strong> Please Select Atleast One User "+"<strong></strong>",'buttons': {'ok'	: {'class'	: 'btn_center alert_ok','action': function(){
                        $('#slt_user_type').focus();
        ...
        /*End New Added 12-01-2015 Sanjay Moghariya*/
