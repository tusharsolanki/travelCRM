                      ...
                      foreach($datalist as $row){?>
						<tr <? if($i%2==1){ ?>class="bgtitle" <? }?> > 
							...
							<td class="hidden-xs hidden-sm ">
                            	<?=!empty($row['admin_name'])?ucfirst(strtolower($row['admin_name'])):'';?>
							</td>
							...
							<td class="hidden-xs hidden-sm text-center">
                             <a class="btn btn-xs btn-secondary" href="javascript:void(0);" onclick="active_plan_single('<?php echo  $row['id'] ?>','<?php echo rawurlencode(ucfirst(strtolower($row['admin_name']))) ?>');" title="Archive">Un-Archive</a>	
                            </td>
                           </tr>
          <?php } } else {?>
		  <tr>
