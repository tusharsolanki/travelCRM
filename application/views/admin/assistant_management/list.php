    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?=$this->lang->line('assistant_list_head');?></h3>
      ...
      <div class="portlet-content">
       <div class="table_large-responsive">
		...
		<?php /*?>  <?php if(isset($remain_user[0]['number_of_users_allowed']) && isset($count_user[0]['total_count']) && $remain_user[0]['number_of_users_allowed'] > $count_user[0]['total_count']) { ?><?php */?>
          <a class="btn  pull-right btn-secondary-green howler margin-left-5px" title="Add User" href="<?=base_url('admin/'.$viewname.'/add_record');?>">Add Assistant</a>
		<?php /*?>  <?php } ?><?php */?>
          <a class="btn  pull-right btn-success howler" title="View Archive" href="<?=base_url('admin/'.$viewname.'/view_archive');?>">View Archive</a>
		  </div>
         </div>
         <div id="common_div">
...
function active_plan(name)
	{      
			...
			if(boxes.length == '0')
			{
			$.confirm({'title': 'Alert','message': " <strong> Please select Assistant "+"<strong></strong>",'buttons': {'ok'	: {'class'	: 'btn_center alert_ok'}}});
