...
?>
 <div id="content">
  <div id="content-header">
   <h1><?=$this->lang->line('assistant_header');?></h1>
  </div>
    ...
    <div class="col-md-12">
     <div class="portlet">
      <div class="portlet-header">
       <h3> <i class="fa fa-table"></i><?='Archived '.$this->lang->line('assistant_list_head');?></h3>
...
<script>
	...
	function contact_search(allflag)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/view_archive/';?>"+uri_segment,
			data: {
...
function active_plan(name)
	...
	function active_plan_single(name,id)
    {
		...
		if(id.length > 50)
			id = id.substr(0, 50)+'...';
           $.confirm({'title': 'CONFIRM','message': ' <strong> Are you sure want to add "'+id+'" to list '+'<strong>?</strong>','buttons': {'Yes': {'class': '',
            'action': function(){
	...
	function active_all_plans(name)
		{
			...
			success: function(data){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo $this->config->item('admin_base_url').$viewname.'/view_archive/';?>"+data,
					data: {
