   ...
   <?php
		...
		$i=1;
			...
			<tr <? if($i%2==1){ ?>class="bgtitle delete_document_trans_record<?= $row['id'] ?>" <? }?> > 
				...
				<td class="hidden-xs hidden-sm" id="document_type_<?=$row['id']?>">
				<?=!empty($row['name'])?ucfirst(strtolower($row['name'])):'';?>
                </td>
				...
				<td class="hidden-xs hidden-sm" id="document_doc_desc_<?=$row['id']?>"><?=!empty($row['doc_desc'])?ucfirst(strtolower($row['doc_desc'])):'';?></td>
