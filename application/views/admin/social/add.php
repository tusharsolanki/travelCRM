...
function FBLogin(){
	url = '<?php echo $this->config->item('admin_base_url').$viewname.'/fb_connection';?>';
	 $.ajax({
				 type: "POST",
				 //dataType: 'json',
				 url: url,
...
function FBLogin_send(){
	...
	FB.login(function(response){
		...
		{
		return false;
				var contact = "<?php echo !empty($this->router->uri->segments[4])?$this->router->uri->segments[4]:'' ?>"; 
				
				 $.ajax({
					 type: "POST",
					 url: "<?=base_url().'admin/'.!empty($fb_path)?$fb_path:''?>",
		   ...
		   // window.location.href = "<?=base_url().'admin/'.!empty($path_view)?$path_view:''?>";
		}
...
<script type="text/javascript">
	...
	function contact_ser_to()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/social/search_contact_to/",
			data: {
			result_type:'ajax',searchtext:$("#search_contact_to").val(),contact_status:$('#slt_contact_status').val(),contact_source:$('#slt_contact_source').val(),contact_type:$('#contact_type').val(),platform_id:$("#platform").val()
		},
		beforeSend: function() {
