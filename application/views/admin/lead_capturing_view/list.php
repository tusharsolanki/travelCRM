        ...
        <div role="grid" class="dataTables_wrapper" id="DataTables_Table_0_wrapper">
         ...
         <div class="col-sm-10">
		  	<div class="row">
					...
					<div class="col-sm-11 col-lg-8 col-md-9">
				<label class="margin-top-5px col-sm-6 col-md-5 col-xs-12"><?=$this->lang->line('user_assign_msg');?></label>
                <div class="col-sm-4 col-md-4 col-lg-5 row">
				<select class="form-control col-sm-5 col-md-5 col-lg-3 col-xs-7 parsley-validated margin-left-5px width20-per" name="slt_user_type[]" id="slt_user_type">
				   	...
				   	<?php if(!empty($userlist)){
							...
							foreach($userlist as $row){
								...
								<option value="<?=$row['id']?>"><?=ucwords($row['first_name']." ".$row['middle_name']." ".$row['last_name'].$email);?></option>
							<?php } ?>
				   <?php } ?>
				  </select>
                  </div>
...
<script>
	...
	function contact_search()
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url();?>admin/lead_capturing_view/<?php echo $this->router->uri->segments[3];?>",
			data: {
...
//delete all lead 
	...
	{      
		...
		var user_id = $('#slt_user_type').val();
			...
			$.ajax({
			type: "POST",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/assign_lead';?>",
			dataType: 'json',
			async: false,
			...
			success: function(data_temp){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/lead_capturing_view/<?php echo $this->router->uri->segments[3];?>/"+data_temp.page,
					data: {
					result_type:'ajax',searchreport:$("#searchreport").val(),perpage:$("#perpage").val(),searchtext:$("#searchtext").val(),sortfield:$("#sortfield").val(),sortby:$("#sortby").val()
				},
				beforeSend: function() {
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form class="form parsley-form1" enctype="multipart/form-data" name="assigncontact" id="assigncontact" method="post" data-validate="parsley" accept-charset="utf-8" action="<?= $this->config->item('admin_base_url')?><?php echo $viewname.'/assign_contact/'.$form_id;?>" novalidate>
		<div id="row_data">
        <div class="col-sm-12">
        	<div class="col-sm-4">
...
<script type="text/javascript">
	...
	$('body').on('click','.view_form_btn',function(e){
			...
			$("#id").val(formid);
					$.ajax({
			dataType: "json",
			url: "<?php echo $this->config->item('admin_base_url').$viewname.'/user_list';?>",
			data: {'id':formid},
			success: function(result){
					 $("#user").html("<option value='-1'>User</option>");
...
function contact_details(id)
{
	$.ajax({
		type: "POST",
		url:"<?php echo $this->config->item('admin_base_url').$viewname.'/contact_details';?>",
		data:{lead_id:id},
		beforeSend: function() {
