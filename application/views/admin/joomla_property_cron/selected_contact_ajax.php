...
if(!empty($contacts_data)){
  ...
  <tr class="<?php if($i%2==0){echo 'odd';}else{echo 'even';}$i++;?>">
	...
	<td class="text-center"><a href="javascript:void(0);" class="btn btn-xs btn-primary remove_selected_contact" data-group="<?=!empty($row['id'])?$row['id']:'';?>" data-id="<?=!empty($row['trans_id'])?$row['trans_id']:'';?>"> <i class="fa fa-times"></i></a></td>
  </tr>
<?php } ?>
<?php }else{ ?>
	<tr>
