...
{	
    ...
    */
    ...
    {	
		...
		//print_r($auto_selected_view_session);exit;
		...
		if($main == 1)	
		{
			...
			if(!empty($searchtext))
			{
				...
				if(empty($allflag))
				{
					if(!empty($searchsort_session['searchtext'])) {
						/*$data['searchtext'] = mysql_real_escape_string($searchsort_session['searchtext']);
			...
			$wherestring = 'email_send_type = 1 AND email_template_master.created_by IN ('.$this->user_session['agent_id'].')';
			if(!empty($searchtext) || !empty($selected_cat))
			{
				/*match=array('template_name'=>$searchtext,'template_subject'=>$searchtext);
				$wherestring='email_send_type = 1 AND created_by = '.$this->user_session['id'].'';*/
			...
			{
				$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','like',$config['per_page'], $uri_segment,$data['sortfield'],$data['sortby'],$group_by,$wherestring);
		...
		if($def == 1)
		{
			...
			$table = "email_template_master";
			//$fields = array('em.*,lm.user_type');
			$fields = array('email_template_master.*,mmlcm.category,lm.user_type,eem.name email_event_name');
			$join_tables = array(
			...
			if(!empty($default_searchtext) || !empty($default_selected_cat))
			{
				...
				if(!empty($default_selected_cat))
					$wherestring='email_template_master.template_category = '.$default_selected_cat.' AND email_template_master.email_send_type = 1 AND (lm.user_type = "1" OR lm.user_type = "2")';
				else
					$wherestring='email_template_master.email_send_type = 1 AND (lm.user_type = "1" OR lm.user_type = "2")';
...
/*
    ...
    */
    ...
    public function edit_record()
    {
		...
		{
			$msg = $this->lang->line('common_right_msg_auto_resp');
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('user/'.$this->viewName);	
		}
    ...
    */
    public function selectedview_session()
    {
		...
		$sess=$this->session->userdata('auto_selected_view_session');
		//print_r($sess);exit;
    }
