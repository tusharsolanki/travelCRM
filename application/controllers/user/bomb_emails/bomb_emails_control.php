...
//$uri_segment = $this->uri->segment(3);
		...
		$wherestring = 'ecm.email_type = "Campaign" AND ecm.created_by IN ('.$this->user_session['agent_id'].') AND ecm.email_blast_type = 1';
    ...
    */
    ...
    public function add_record()
    {
		...
		//pr($data['email_signature_daat']);exit;
		//$data['email_signature_data'] = $this->email_signature_model->select_records('',$match,'','=');
		//$data['tablefield_data']=$this->bomb_library_model->select_records3();
   ...
   */
 	...
 	public function send_mail($contact_id='',$headers='',$data='',$cdata='',$email_signature='')
	{
		...
		//exit;
		//$admin_id = $this->user_session['admin_id'];
    ...
    */
    ...
    public function edit_record()
    {
		...
		$where = 'id = '.$id.' AND created_by IN ('.$this->user_session['agent_id'].') AND email_blast_type = 1';
		...
		if(count($result) > 0)
		{
			if(($result[0]['is_draft'] == 0 && $result[0]['email_send_date'] == '0000-00-00') || ($result[0]['email_type'] == 'Intereaction_plan')) {
				redirect(base_url('user/'.$this->view));
			}
		}
		else
			redirect(base_url('user/'.$this->view));
		...
		//$cdata['tablefield_data']=$this->bomb_library_model->select_records3();
		$table1='custom_field_master';
		$where1=array('module_id'=>'1');
    ...
    */
    ...
    public function update_data()
    {
		...
		redirect(base_url('user/'.$this->view.'/'.$pagingid));
...
//$uri_segment = $this->uri->segment(5);
		...
		$table ="contact_master as cm";
		$fields = array('cm.*,ect.id as ID,ect.is_send,ect.template_subject,ect.sent_date,ect.info');
	...
	public function view_data()
	{
		...
		$wherestring = 'ecr.email_campaign_id = '.$data['campaign_id'].' AND ecr.id = '.$data['id']. ' AND ecm.created_by IN ('.$this->user_session['agent_id'].') AND ecm.email_blast_type = 1';
	...
	//pr($cdata['datalist']);exit;
		if(empty($cdata['datalist']))
		{
			...
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('user/'.$this->view.'/all_sent_mail');	
		}
		/*pr($cdata['datalist']);
		echo $this->db->last_query();exit;*/
		/*$email_cc = '';
   ...
   */
	...
	public function resend_mail()
	{
		...
		if((!empty($campaign_id) || !empty($single_id)) && !empty($udata[0]['bombbomb_username']) && !empty($udata[0]['bombbomb_password']))
		{
			...
			if(!empty($connection->status) && $connection->status == 'success')
			{
				...
				$group_by = 'ecr.id';
				$wherestring = "cm.is_subscribe = '0' AND ecr.is_send = '0' AND ecm.email_blast_type = 1 AND ecm.created_by IN (".$this->user_session['agent_id'].")";
				if(!empty($single_id))
		...
		else
		{
			...
			if(!empty($searchsort_session['uri_segment']))
				...
				$pagingid = 0;
			redirect('user/'.$this->view.'/'.$pagingid);
		}
	}
    ...
    */
	...
	public function all_sent_mail()
	{
		...
		$wherestring = "ecr.is_send = '1' AND (ecm.created_by IN (".$this->user_session['agent_id'].") OR ipi.assign_to IN (".$this->user_session['agent_id'].")) AND ecm.email_blast_type = 1";
		//ecm.is_draft = '0' AND
    ...
    */
	...
	public function queued_list()
	{
		...
		$wherestring = "ecm.email_type = 'Intereaction_plan' AND ecr.is_send = '0' AND ipm.status = '1' AND ecm.email_blast_type = 1 AND (ipm.created_by IN (".$this->user_session['agent_id'].") OR ipi.assign_to IN (".$this->user_session['agent_id']."))";
		$groupby = 'ipm.id';
		if(!empty($searchtext))
		{
    ...
    */
	...
	public function interaction_queued_list()
	{
		...
		$wherestring = "ecm.email_type = 'Intereaction_plan' AND ecr.is_send = '0' AND ipm.status = '1' AND ecm.email_blast_type = 1 AND ipm.id = ".$plan_id." AND (ipm.created_by IN (".$this->user_session['agent_id'].") OR ipi.assign_to IN (".$this->user_session['agent_id']."))";
		$groupby = 'ipi.id';
		if(!empty($searchtext))
		{
    ...
    */
	...
	public function interaction_plan_queued_list()
	{
		...
		$wherestring = "ecm.email_type = 'Intereaction_plan' AND ipi.id = ".$interaction_id." AND ecr.is_send = '0' AND ipi.status = '1' AND ecm.email_blast_type = 1 AND (ipm.created_by IN (".$this->user_session['agent_id'].") OR ipi.assign_to IN (".$this->user_session['agent_id']."))";
		$groupby = 'ecr.id';
		if(!empty($searchtext))
		{
		...
		/*if(empty($data['datalist']))
		{
			...
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('user/'.$this->view.'/queued_list');	
		}*/
    ...
    */
	...
	public function view_interaction_data()
	{
		...
		$id = $this->uri->segment(4);
		$table = "email_campaign_recepient_trans as ecr";
		$fields = array("ecm.email_message,ipm.created_by,ecm.template_subject,cm.first_name,cm.last_name,mml1.category as category,etm.template_name,ecr.*,esm.full_signature,cet.email_address,ecm.interaction_id,ipim.assign_to");
		$join_tables = array('email_campaign_master ecm'=>'ecm.id = ecr.email_campaign_id',
							'email_signature_master esm'=>'esm.id = ecm.email_signature',
							'contact_master cm'=>'ecr.contact_id = cm.id',
							'marketing_master_lib__category_master mml1'=>'mml1.id = ecm.template_category_id',
							...
							'interaction_plan_master as ipm'=>'ipm.id = ipim.interaction_plan_id'
							);
	...
	//pr($data['datalist']);exit;
		...
		else
		{
			...
			$this->obj->update_email_campaign_trans($cdata);
			$this->session->set_userdata('message_session', $newdata);
            redirect(base_url('user/'.$this->view.'/interaction_plan_queued_list/'.$this->uri->segment(5)));	
		}
    ...
    */
	...
	public function interaction_mailsms()
	{
		...
		$group_by = 'cet.contact_id';
		//$wherestring = "cet.is_default = '1' AND ecr.is_send = '0' AND cm.is_subscribe = '0' AND ecm.email_blast_type = 1 AND ecr.id = ".$id;
		$wherestring = "cet.is_default = '1' AND cm.is_subscribe = '0' AND ecr.is_send = '0' AND ecm.email_blast_type = 1 AND (ipm.created_by IN (".$this->user_session['agent_id'].") OR ipi.assign_to IN (".$this->user_session['agent_id'].")) AND '".date('Y-m-d')."' >= ecr.send_email_date";
		if($this->input->post('submitbtn'))
		{
		...
		if(!$this->input->post('submitbtn'))
		{
			if(!empty($interaction_plan_id))
				redirect('user/'.$this->view.'/interaction_queued_list/'.$interaction_plan_id);
			else
				redirect('user/'.$this->view.'/queued_list/'.$interaction_plan_id);
		}
		elseif(!empty($interaction_plan_id))
			redirect('user/'.$this->view.'/interaction_plan_queued_list/'.$interaction_id.'/'.$pagingid);
		else
			redirect('user/'.$this->view);
	}
