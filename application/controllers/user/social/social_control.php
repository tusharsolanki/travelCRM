...
{	
    ...
    */
	...
	//	echo "hi"; exit;
		...
		$wherestring = "scm.created_by IN (".$this->user_session['agent_id'].")";
		if(!empty($searchtext))
		{
	...
	//	$data['total_social'] = $this->obj->total_social($this->user_session['id']);
...
;
		...
		//$data['profile_trans_data'] = $this->contacts_model->select_social_name_twitter($this->user_session['id']);
    ...
    */
	...
	/*	$facebook_api_key=$this->config->item('facebook_api_key');
        ...
        . "&client_secret=".$facebook_secret_key."&code=" . $_GET['code'];
			...
			$match = array("parent"=>'0');
            $data['category'] = $this->marketing_library_masters_model->select_records1('',$match,'','=','','','','id','desc','marketing_master_lib__category_master');
			$data['main_content'] = "user/".$this->viewName."/facebook_page";
        	$this->load->view('user/include/template', $data);    
		}
		else
		{
				...
				$publish = $facebook->api('/'.$page_id.'/feed', 'post',
						...
						'name' => 'LiveWire CRM',
						'link' => 'http://topsdemo.in/qa/livewire_crm/v.1.0/user/',
			...
			//        ));
				...
				<script type="text/javascript">
					...
					function refreshParent() {*/
						window.opener.location.href = '<?=base_url('user/'.$this->viewName)?>';
					/*}*/
				</script>
