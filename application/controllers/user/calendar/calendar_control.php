...
{	
    ...
    */
    ...
    {
        //$data['main_content'] = "user/".$this->viewName."/google_login";
        $cdata['sucess']='';
        $this->load->view("user/".$this->viewName."/google_login",$data);	
    }
    ...
    function outputCalendar($eventFeed)
    {
            ...
            foreach($eventFeed as $caleve)
            {
                ...
                foreach ($caleve['items'] as $event) 
                {
                    ...
                    {
                        $admin_time_zone = !empty($this->user_session['date_timezone'])?$this->user_session['date_timezone']:$this->config->item('default_timezone');
            ...
            ?>
            <script type="text/javascript">window.location.href = "<?=base_url()."user/".$this->viewName?>"</script>
            <?php
         ...
         * function outputCalendar($client)
	{
	 	$gdataCal = new Zend_Gdata_Calendar($client);
            ...
            foreach ($eventFeed as $event) 
            {
                ...
                if(!empty($record) && count($record) > 0)
                {
                    $insert_id=$this->obj->update_calandar($data);
					//echo $insert_id;
					$event_data['calendar_id']=$record[0]['id'];
					$this->obj->update_record_tran($event_data);
    ...
    public function view_record()
    {
        ...
        try{
			...
			if(!empty($public) && $public == '1')
				...
				$match = "is_public = '0'";
            $calendar_data = $this->obj->get_records2($start_date,$end_date,'',$match,$user_id);
           // pr($calendar_data);
    ...
    */
    public function edit_calender()
    {
        ...
        if(isset($id) && $id !=0)
        {
            ...
            $data['event'] = $this->obj->getmultiple_tables_records1($table,$fields,$join_tables,'left','','','',$per_page,$uri_segment,$sortfield,$sortby,$group_by,$where);
            //pr($data['event']);exit;
            //$data['event'] = $this->obj->select_records('',$match,'','=');
            //	pr($data['event']);exit;
        }
		
        $this->load->view('user/calendar/edit_calendar', $data);
    }
    ...
    */
    function update_calender()
    {
		...
		{
			$cal_m_data['id']= $id;
			$transaction_result1 = $this->obj->select_calendar_master($cal_m_data);
		}
			
		
		if(!empty($transaction_result1) || empty($id))
		{
			...
			if(!empty($is_popup))
			{
				...
				if(!empty($idata['popup_time_before']))
				{
					...
					if(!empty($idata['popup_time_type']) && $idata['popup_time_type']=='1')
					{
						...
						$counttype='Hours';
						$newtaskdate1 = date($this->config->item('log_date_format'),strtotime($idata['start_date']."- ".$idata['popup_time_before']." ".$counttype));
					...
					{
						$counttype='Days';
						$newtaskdate1 = date($this->config->item('common_date_format'),strtotime($idata['start_date']."- ".$idata['popup_time_before']." ".$counttype));
						$idata['reminder_popup_date'] = date('Y-m-d H:i:s',strtotime($newtaskdate1));	
					}
				}
			}	
        ...
        if(!empty($ifRepeat) && $ifRepeat=='1')
        {
            ...
            // Insert Yearly Sanjay M. 17-10-2104
            if($repeatType=='5')
            {	
                $idata['repeatType']=$this->input->post('repeatType');
        ...
        if(!empty($id))
        {
            ...
            $changetextc['modified_by'] = $this->user_session['id'];
		...
		$match = array("id"=>$ctid);
                ...
                if(!empty($event_type) && $event_type == 1)
                {
                    ...
                    $ddata['id']=$cmid;
                    $ddata['modified_by'] = $this->user_session['id'];
                ...
                else if(!empty($event_type) && $event_type == 2)
                {
                    ...
                    /*$following['event_start_date']=date('Y-m-d', strtotime($this->input->post('stpartdate')));
                    $following['event_start_time'] = $this->input->post('stparttime');
                    $following['event_end_date'] = date('Y-m-d', strtotime($this->input->post('etpartdate')));
                    $following['event_end_time'] = $this->input->post('etparttime');*/
                    //pr($following);exit;	

                    //$update_id = $this->obj->update_following_trandata($following,$cmid,$ctid,$event_type); 
                        ...
                        {	
                            ...
                            else if($endTemplateType == '3') // End on any random date
                            {
                                ...
                                if(!empty($result) && !empty($transaction_result))
                                {
                                    ...
                                    if($result[0]['repeatType'] != $repeatType || $result[0]['dailyType'] != $dailyType || $result[0]['endTemplateType'] != $endTemplateType || strtotime($start_date) != strtotime($oldstdate) || strtotime($end_date) != strtotime($oldetdate) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm))) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm))) || $result[0]['everyDays'] != $everyDays || strtotime(date('Y-m-d',strtotime($result[0]['endTemplateDate']))) != $endtempdt || strtotime(date('H:i:s',strtotime($result[0]['endTemplateTime']))) != $endtmptm)
                                    {
                                        if(!empty($this->user_session))
                                        {
                                            $idata['status']='1';
                        ...
                        else if($dailyType=='2') // Every Weekdays
                        {
                            if($endTemplateType == '1') // Never
                            {
                                if(!empty($result) && !empty($transaction_result))
                                {
                                    $oldstdate=date('Y-m-d',strtotime($transaction_result[0]['event_start_date']));
                                    $oldetdate=date('Y-m-d', strtotime($transaction_result[0]['event_end_date']));
                                    if($result[0]['repeatType'] != $repeatType || $result[0]['dailyType'] != $dailyType || $result[0]['endTemplateType'] != $endTemplateType || strtotime($start_date) != strtotime($oldstdate) || strtotime($end_date) != strtotime($oldetdate) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm))) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm))))
                                    {
                                        if(!empty($this->user_session))
                                        {
                                            $idata['status']='1';
                            ...
                            else if($endTemplateType == '3') // End on any random date
                            {
                                ...
                                if(!empty($result) && !empty($transaction_result))
                                {
                                    ...
                                    if($result[0]['repeatType'] != $repeatType || $result[0]['dailyType'] != $dailyType || $result[0]['endTemplateType'] != $endTemplateType || strtotime($start_date) != strtotime($oldstdate) || strtotime($end_date) != strtotime($oldetdate) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm))) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm))) || strtotime(date('Y-m-d',strtotime($result[0]['endTemplateDate']))) != $endtempdt || strtotime(date('H:i:s',strtotime($result[0]['endTemplateTime']))) != $endtmptm)
                                    {
                                        if(!empty($this->user_session))
                                        {
                                            $idata['status']='1';
                    ...
                    {	
                        ...
                        if($endTemplateType == '1') // Never
                        {
                            if(!empty($result) && !empty($transaction_result))
                            {
                                ...
                                if(($result[0]['repeatType'] != $repeatType) || ($result[0]['endTemplateType'] != $endTemplateType) || (strtotime($start_date) != strtotime($oldstdate)) || (strtotime($end_date) != strtotime($oldetdate)) || (strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm)))) || (strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm)))) || ($result[0]['everyWeeks'] != $everyWeeks) || ($weekdaysflag == 1))
                                {
                                    if(!empty($this->user_session))
                                    {
                                        $idata['status']='1';
                        ...
                        else if($endTemplateType == '3') // End on any random date
                        {
                            ...
                            if(!empty($result) && !empty($transaction_result))
                            {
                                ...
                                if(($result[0]['repeatType'] != $repeatType) || ($result[0]['endTemplateType'] != $endTemplateType) || (strtotime($start_date) != strtotime($oldstdate)) || (strtotime($end_date) != strtotime($oldetdate)) || (strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm)))) || (strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm)))) || ($result[0]['everyWeeks'] != $everyWeeks) || ($weekdaysflag == 1) || (strtotime(date('Y-m-d',strtotime($result[0]['endTemplateDate']))) != $endtempdt) || (strtotime(date('H:i:s',strtotime($result[0]['endTemplateTime']))) != $endtmptm))
                                {
                                    if(!empty($this->user_session))
                                    {
                                        $idata['status']='1';
                        ...
                        if($monthlyType == '1')
                        {
                            ...
                            if($endTemplateType == '1') // Never
                            {
                                if(!empty($result) && !empty($transaction_result))
                                {
                                    $oldstdate=date('Y-m-d',strtotime($transaction_result[0]['event_start_date']));
                                    $oldetdate=date('Y-m-d', strtotime($transaction_result[0]['event_end_date']));
                                    if($result[0]['repeatType'] != $repeatType || $result[0]['monthlyType'] != $monthlyType || $result[0]['endTemplateType'] != $endTemplateType || strtotime($start_date) != strtotime($oldstdate) || strtotime($end_date) != strtotime($oldetdate) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm))) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm))) || $result[0]['monthDate'] != $monthDate || $result[0]['monthCount'] != $monthCount)
                                    {
                                        if(!empty($this->user_session))
                                        {
                                            $idata['status']='1';
                            ...
                            else if($endTemplateType == '3') // End on any random date
                            {
                                ...
                                if(!empty($result) && !empty($transaction_result))
                                {
                                    ...
                                    $oldetdate=date('Y-m-d', strtotime($transaction_result[0]['event_end_date']));
                                    $endtmptm = strtotime(date('H:i:s',strtotime($this->input->post('endTemplateTime'))));
                                    if(($result[0]['repeatType'] != $repeatType) || ($result[0]['monthlyType'] != $monthlyType) || ($result[0]['endTemplateType'] != $endTemplateType) || (strtotime($start_date) != strtotime($oldstdate)) || (strtotime($end_date) != strtotime($oldetdate)) || (strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm)))) || (strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm)))) || ($result[0]['monthDate'] != $monthDate) || ($result[0]['monthCount'] != $monthCount) || (strtotime(date('Y-m-d',strtotime($result[0]['endTemplateDate']))) != $endtempdt) || (strtotime(date('H:i:s',strtotime($result[0]['endTemplateTime']))) != $endtmptm))
                                    {
                                        if(!empty($this->user_session))
                                        {
                                            $idata['status']='1';
                            ...
                            if(!empty($nthDay) && !empty($nthDate) && !empty($nthMonthCount))
                            {
                                ...
                                if($endTemplateType == '1')
                                {
                                    if(!empty($result) && !empty($transaction_result))
                                    {
                                        $oldstdate=date('Y-m-d',strtotime($transaction_result[0]['event_start_date']));
                                        $oldetdate=date('Y-m-d', strtotime($transaction_result[0]['event_end_date']));
                                        if($result[0]['repeatType'] != $repeatType || $result[0]['monthlyType'] != $monthlyType || $result[0]['endTemplateType'] != $endTemplateType || strtotime($start_date) != strtotime($oldstdate) || strtotime($end_date) != strtotime($oldetdate) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm))) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm))) || $result[0]['nthDay'] != $nthDay || $result[0]['nthDate'] != $nthDate || $result[0]['nthMonthCount'] != $nthMonthCount)
                                        {
                                            if(!empty($this->user_session))
                                            {
                                                $idata['status']='1';
                                ...
                                else if($endTemplateType == '3')
                                {
                                    ...
                                    if(!empty($result) && !empty($transaction_result))
                                    {
                                        ...
                                        $oldetdate=date('Y-m-d', strtotime($transaction_result[0]['event_end_date']));
                                        $endtmptm = strtotime(date('H:i:s',strtotime($this->input->post('endTemplateTime'))));
                                        if($result[0]['repeatType'] != $repeatType || $result[0]['monthlyType'] != $monthlyType || $result[0]['endTemplateType'] != $endTemplateType || strtotime($start_date) != strtotime($oldstdate) || strtotime($end_date) != strtotime($oldetdate) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm))) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm))) || $result[0]['nthDay'] != $nthDay || $result[0]['nthDate'] != $nthDate || $result[0]['nthMonthCount'] != $nthMonthCount || (strtotime(date('Y-m-d',strtotime($result[0]['endTemplateDate']))) != $endtempdt) || (strtotime(date('H:i:s',strtotime($result[0]['endTemplateTime']))) != $endtmptm))
                                        {
                                            if(!empty($this->user_session))
                                            {
                                                $idata['status']='1';
                    ...
                    // Recurrence Type = 'Yearly' Sanjay Moghariya 17-10-2014
                    ...
                    {	
                        ...
                        if($endTemplateType == '1') // Never
                        {
                            if(!empty($result) && !empty($transaction_result))
                            {
                                $oldstdate=date('Y-m-d',strtotime($transaction_result[0]['event_start_date']));
                                $oldetdate=date('Y-m-d', strtotime($transaction_result[0]['event_end_date']));
                                if($result[0]['repeatType'] != $repeatType || $result[0]['endTemplateType'] != $endTemplateType || strtotime($start_date) != strtotime($oldstdate) || strtotime($end_date) != strtotime($oldetdate) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_start_time']))) != strtotime(date('H:i:s',strtotime($st_tm))) || strtotime(date("H:i:s",strtotime($transaction_result[0]['event_end_time']))) != strtotime(date('H:i:s',strtotime($et_tm))) || $result[0]['everyYears'] != $everyYears)
                                {
                                    if(!empty($this->user_session))
                                    {
                                        $idata['status']='1';
                ...
                {
                    /* Old code
                    //$following['calendar_id']=$this->input->post('calendar_id'); // Calendar Master id
                    $cmid=$this->input->post('calendar_id'); // Calendar Master id
                    ...
                    //$following['event_end_time'] = $this->input->post('etparttime');
                    //pr($following);exit;	
                    $update_id = $this->obj->update_record_tran($following,$cmid); 
                    End Old code*/
            ...
            else /*If recurrence is removed at edit time. Sanjay M. 10-09-2014*/
            {
                ...
                else
                {
                    ...
                    $idata['everyDays'] = 0;$idata['everyWeeks'] = 0;$idata['everyWeekonMon'] = 0;
    ...
    /*
        ...
        @Input      : start date, total day (after 5 year),  end date, total days between start and end date, repeat event on every n day, insert data array
    ...
    */
	...
	$id=$this->input->post('calendarId');
        ...
        $match = array("id"=>$transaction_result[0]['calendar_id'],'created_by'=>$this->user_session['id']);
        ...
        for($i=0;$i < count($transaction_result1);$i++)
        {
            ...
            {
                if(($transaction_result1[$i]['task_user_id'] == $this->user_session['id']) OR ($transaction_result1[$i]['created_by'] == $this->user_session['id']))
                {
                    $flag = 1;
                    break;
                }
        ...
        if($flag == 1 || empty($id))
        {
            ...
            $fields = array('crt.id as cal_tran_id','crt.calendar_id');
            $join_tables = array(
            ...
            if(!empty($datalist))
            {
                    try
                    {
                            $data['id']=$datalist[0]['calendar_id'];
                            $data['start_date']=$start_date;
                            $data['start_time']=$start_time;
                            $data['end_date']=$end_date;
                            $data['end_time']=$end_time;
                            $update_id = $this->obj->update_record($data);
    ...
    */
    function delete_calender()
    {
        ...
        $transaction_result = $this->obj->getmultiple_tables_records($table,$fields,'','','',$match);
      	...
      	$data['id']= $transaction_result[0]['calendar_id'];
        $transaction_result1 = $this->obj->select_calendar_master($data);
		
        if(!empty($transaction_result1))
        {
