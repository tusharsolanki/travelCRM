...
{	
    ...
    */
    ...
    public function index()
    {
		...
		$fields = array('plm.id,property_title,plm.assign_date,plm.created_date,plm.created_by,plm.assign_to','CONCAT_WS(" ",address_line_1,address_line_2) as address','city,mls_no,property_type_name,price,year_built,new_price,CASE WHEN new_price IS NOT NULL then new_price ELSE price END AS my_price,CASE lm.user_type WHEN 2 THEN admin_name ELSE CONCAT_WS(" ",um.first_name,um.last_name) END as agent_name','count(DISTINCT plct.id) as contact_counter,plm.created_date');
		...
		$group_by = 'plm.id';
		$where = 'plm.created_by IN ('.$this->user_session['agent_id'].') OR plm.assign_to IN ('.$this->user_session['agent_id'].')';
		if(!empty($searchtext))
		{
			$having = "address LIKE '%".$searchtext."%' OR city LIKE '%".$searchtext."%' OR mls_no LIKE '%".$searchtext."%' OR property_type_name LIKE '%".$searchtext."%' OR year_built LIKE '%".$searchtext."%' OR my_price LIKE '%".$searchtext."%' OR CASE WHEN plm.created_by IN (".$this->user_session['agent_id'].") THEN DATE_FORMAT(plm.created_date,'%Y-%m-%d %H:%i:%s') > '".$searchtext."' ELSE (plm.assign_to IN (".$this->user_session['agent_id'].") AND  DATE_FORMAT(plm.assign_date,'%Y-%m-%d %H:%i:%s') > '".$searchtext."') END";

			$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','like',$config['per_page'],$uri_segment,$data['sortfield'],$data['sortby'],$group_by,$where,'','',$having);
			//echo $this->db->last_query();exit;
            $config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','like','','','','',$group_by,$where,'','1',$having);
		}
		else
		{
    ...
    */
    ...
    public function add_record()
    {
		...
		*/
		...
		$cdata['contact_list'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config['per_page'], $uri_segment,'cm.first_name','asc',$group_by,$where);
		$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$where,'','1');
    ...
    */
	...
	//	float: right;margin-right:90px
		...
		if(!empty($lastId))
		{
			...
			//$addcontactdata = $this->input->post('finalcontactlist');
		...
		{
			redirect('user/'.$this->viewName.'/edit_record/'.$lastId.'/'.($tabid+1));
		}
    ...
    */
    ...
    public function edit_record()
    {
		...
		$match = array('id'=>$id);
        //$result = $this->obj->select_records('',$match,'','=');
		$where = 'id = '.$id.' AND (created_by IN ('.$this->user_session['agent_id'].') OR assign_to IN ('.$this->user_session['agent_id'].'))';
        $result = $this->obj->getmultiple_tables_records('property_listing_master','','','','','','','','','','','',$where);
		//pr($result);exit;
		...
		if(count($result) == 0)
		{
			$msg = $this->lang->line('common_right_msg_listing_manager');
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('user/'.$this->viewName);		
		}
		...
		//$where=array('lm.status'=>"'1'");
		$where = 'lm.status = "1" AND lm.id IN ('.$this->user_session['agent_id'].') AND lm.user_type = "3"';
		$cdata['user_list']=$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','like','', '','','',$group_by,$where);
    ...
    */
    ...
    public function update_data()
    {
		...
		else if($tabid == 3)
		{
			...
			{  
				...
				$cddata['created_by']=$this->user_session['id'];
				$cddata['status']='1';
		...
		else if($tabid == 4)
		{
			...
			if(trim($cddata['doc_type']) != '' || trim($cddata['doc_name']) != '' || trim($cddata['doc_desc']) != '' || trim($cddata['doc_file']) != '')
			{
				if($this->input->post('doc_id') == '')
				{
					...
					$cddata['modified_date'] = date('Y-m-d H:i:s');
					$cddata['modified_by']=$this->user_session['id'];
		...
		else if($tabid == 8)
		{
			...
			if(trim($change['open_house_date']) != '')
			{
				...
				if(trim($change['open_house_time']) != '')
					$cddata['open_house_time']=date("H:i",strtotime($change['open_house_time']));
	...
	// Update Contacts Tab data
		...
		{
			redirect('user/'.$this->viewName.'/edit_record/'.$property_id.'/'.($submitvaltab2).'#documenets');		
		}

		else if($redirecttype == 'Save and Add More Photos')
		{
			redirect('user/'.$this->viewName.'/edit_record/'.$property_id.'/'.($submitvaltab2).'#photo');		
		}

		else if($redirecttype == 'Save and Add More Open Houses')
		{
			redirect('user/'.$this->viewName.'/edit_record/'.$property_id.'/'.($submitvaltab2).'#Open_houses');		
		}


		else if($redirecttype == 'Save and Add More Showings')
		{
			redirect('user/'.$this->viewName.'/edit_record/'.$property_id.'/'.($submitvaltab2).'#showings');		
		}

		else if($redirecttype == 'Save and Add More Price')
		{
			redirect('user/'.$this->viewName.'/edit_record/'.$property_id.'/'.($submitvaltab2).'#price_change');		
		}
		
		else if($redirecttype == 'Save and Add More Offers')
		{
			redirect('user/'.$this->viewName.'/edit_record/'.$property_id.'/'.($submitvaltab2).'#Offers');		
		}
		...
		else
		{
			if($tabid == 9)
				$tabid = $tabid + 1;
			redirect('user/'.$this->viewName.'/edit_record/'.$property_id.'/'.($tabid+1));
		}
    ...
    */
	...
	public function search_contact_ajax()
    {
		...
		$fields = array('cm.id','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','cm.company_name','cet.email_address');
		$join_tables = array(
							'(SELECT * FROM contact_emails_trans where is_default = "1") as cet'=>'cet.contact_id = cm.id and cet.is_default = "1"',
    ...
    */
	...
	{
		$data['name'] = trim($this->input->post('txt_name'));
    ...
    <?php
		...
		while(!feof($file_handle)) 
		{
			...
			if(!empty($line_of_text) && $i != 0 && !empty($line_of_text[1]))
			{
				...
				if($this->user_session['user_type'] == '3')
					$cdata['assign_to'] = $this->user_session['id'];
				else
				{
					$match = array('id'=>$this->user_session['id']);
					$udata = $this->admin_model->get_user('',$match,'','=');
					if(count($udata) > 0)
						$cdata['assign_to'] = $udata[0]['agent_id'];
				}
				$cdata['created_by'] = $this->user_session['id'];
				$cdata['created_date'] = date('Y-m-d H:i:s');
				$cdata['status'] = '1';
