<?php 
/*
    @Description: User SMS Auto Response Controller
...
{	
    ...
    */
    ...
    {	
		//check user right
		check_rights('sms_auto_responder');
		...
		$wherestring = 'sms_send_type = 1 AND is_default = 0 AND sms_text_template_master.created_by IN ('.$this->user_session['agent_id'].')';
		...
		if(!empty($searchtext) || !empty($selected_cat))
		{
			...
			if(!empty($selected_cat))
				$wherestring .= ' AND sms_text_template_master.template_category = '.$selected_cat;
		...
		$table = "sms_text_template_master";
		$fields = array('sms_text_template_master.*,mmlcm.category,lm.user_type,sem.name sms_event_name');
		$join_tables = array('marketing_master_lib__category_master mmlcm'=>'mmlcm.id = sms_text_template_master.template_category',
							 'sms_event_master sem'=>'sem.id = sms_text_template_master.sms_event',
							 'login_master as lm'=> 'lm.id = sms_text_template_master.created_by',
							);
		...
		if(!empty($searchtext1) || !empty($selected_cat1))
		{
			...
			if(!empty($selected_cat1))
				$wherestring='sms_text_template_master.sms_send_type = 1 AND sms_text_template_master.template_category = '.$selected_cat1.' AND (lm.user_type = "1" OR lm.user_type = "2")';
			else
				$wherestring='sms_text_template_master.sms_send_type = 1 AND (lm.user_type = "1" OR lm.user_type = "2")';
		...
		{
			$wherestring='sms_text_template_master.sms_send_type = 1 AND (lm.user_type = "1" OR lm.user_type = "2")';
			//$match = array('sms_text_template_master.sms_send_type' => 1, 'sms_text_template_master.is_default' => 0);
			//$data['datalist'] = $this->obj->select_records('',$match,'','','',$config['per_page'],$uri_segment,$sortfield,$sortby);	
			$data['default_datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','=',$config1['per_page'], $uri_segment,$data['sortfield1'],$data['sortby1'],$group_by,$wherestring);
