...
{ 
	...
	/*
		@Description: Function for Get new contact,new property list,assign letter/label/envelope/email/SMS/to-do task/task/ count
    ...
    */
    ...
    public function display_dashbord()
    {
		...
		$fields = array('tm.id','tm.task_name','tm.task_date','tm.is_email','tm.email_time_before','tm.email_time_type','tm.is_popup','tm.popup_time_before','tm.popup_time_type','CONCAT_WS(" ",um.first_name,um.middle_name,um.last_name) as user_name','tm.created_date','tm.reminder_email_date ','tm.reminder_popup_date','tut.is_close as close_popup','CONCAT_WS(" ",um1.first_name,um1.middle_name,um1.last_name) as user_name_own','tm.created_by','tut.id as trans_id','tm.is_close');
		$join_tables = array(
							'task_user_transcation as tut' => 'tut.task_id  = tm.id',
							'login_master as lm' => 'lm.id = tut.user_id',
							'user_master as um' => 'um.id = lm.user_id',
							'user_master as um1' => 'um1.id = tm.created_by' //// if User assinged own Task
							
							
							);
		$group_by='tm.id';
		//$match = array('tut.user_id'=>$id,'tm.created_by'=>$id);
		$where = 'tut.user_id IN ('.$this->user_session['agent_id'].') OR tm.created_by IN ('.$this->user_session['agent_id'].')';	
			...
			//pr($task_notification);exit;
					for($j=0;$j < count($task_notification);$j++)
					{
						...
						if($task_notification[$j]['task_date'] == $now_date)
						{
							...
							{ 
								$data['today_task_user_name'][$i]=$task_notification[$j]['user_name_own'];
							}
...
//		pr($data['user_name_popup']);
		...
		$where = 'ecm.created_by IN ('.$this->user_session['agent_id'].') AND ecm.email_send_date = "'.$now_date.'"';
		...
		$where = 'scm.created_by IN ('.$this->user_session['agent_id'].') AND scm.sms_send_date = "'.$now_date.'"';
		...
		$where='cm.created_by IN ('.$this->user_session['agent_id'].') ANd cm.is_popup = "1"';
	...
	//pr($calendar_events);exit;
		...
		$join_tables = array('user_contact_trans uct'=>'uct.contact_id = cm.id');
		$fields = array('cm.id');
		...
		$where1 = array('et.status'=>1,'et.created_by'=>$this->user_session['id']);
		...
		if(!empty($admin_data))
		{
			...
			$where = '(cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].') AND (DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "'.$data['contact_last_seen'].'"  OR DATE_FORMAT(uct.created_date,"%Y-%m-%d %H:%i:%s") > "'.$data['contact_last_seen'].'")';
		}
		else
			$where = '(cm.created_by IN ('.$this->user_session['agent_id'].')  OR uct.user_id = '.$this->user_session['agent_user_id'].') AND (DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "0000-00-00 00:00:00" OR DATE_FORMAT(uct.created_date,"%Y-%m-%d %H:%i:%s") > "0000-00-00 00:00:00")';
		//$where=array('cm.created_by'=> $id,'DATE_FORMAT(cm.created_date,"%Y-%m-%d")'=>"'$dt'");
		$contact_count =$this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$where,'','1');
		//echo $this->db->last_query();
		...
		if(!empty($admin_data))
		{
			...
			$where = '(created_type = "1") AND (cm.created_by ='.$this->user_session['id'].') AND (DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "'.$data['contact_last_seen'].'"  OR DATE_FORMAT(uct.created_date,"%Y-%m-%d %H:%i:%s") > "'.$admin_data[0]['manual_contact_last_seen'].'")';
		}
		else
			$where = '(created_type = "1") AND (cm.created_by ='.$this->user_session['id'].') AND (DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "0000-00-00 00:00:00")';
			$contact_manual_count = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$where,'','1');
		...
		if(!empty($admin_data))
		{
			...
			$where = 'cm.created_by ='.$this->user_session['id'].' AND created_type = "5" and DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "'.$admin_data[0]['form_lead_last_seen'].'"';
		}
		else
			$where = 'cm.created_by ='.$this->user_session['id'].' AND created_type = "5" and DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "0000-00-00 00:00:00"';
		...
		if(!empty($admin_data))
		{
			...
			$where = 'CASE WHEN cm.created_by IN ('.$this->user_session['agent_id'].') THEN DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "'.$data['listing_last_seen'].'" ELSE cm.assign_to IN ('.$this->user_session['agent_id'].') AND  DATE_FORMAT(cm.assign_date,"%Y-%m-%d %H:%i:%s") > "'.$data['listing_last_seen'].'" END';
			//$where = '(cm.created_by IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "'.$data['listing_last_seen'].'") OR (cm.assign_to IN ('.$this->user_session['agent_id'].') AND  DATE_FORMAT(cm.assign_date,"%Y-%m-%d %H:%i:%s") > "'.$data['listing_last_seen'].'")';
		}
		else
			$where = 'CASE WHEN cm.created_by IN ('.$this->user_session['agent_id'].') THEN DATE_FORMAT(cm.created_date,"%Y-%m-%d %H:%i:%s") > "0000-00-00 00:00:00" ELSE cm.assign_to IN ('.$this->user_session['agent_id'].') AND  DATE_FORMAT(cm.assign_date,"%Y-%m-%d %H:%i:%s") > "0000-00-00 00:00:00" END';
		//$where=array('cm.created_by'=> $id,'DATE_FORMAT(cm.created_date,"%Y-%m-%d")'=>"'$dt'");
		...
		///////////////END NEW LEADS. 11-11-2014 SANJAY CHABHADIYA//////////////////////
		
		///////////////TASK. 11-11-2014 SANJAY CHABHADIYA//////////////////////
		$table = "task_master as tm";
		$fields = array('tm.id');
		$join_tables = array(
			'task_user_transcation as tut' => 'tut.task_id  = tm.id',
		...
		//$group_by='tm.id';
		//$where=array('tut.user_id'=> $id,'DATE_FORMAT(tm.task_date,"%Y-%m-%d")'=>"'$dt'",'tut.is_completed'=>"'0'");
		$where = 'tut.user_id IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(tm.task_date,"%Y-%m-%d") = "'.$dt.'" AND tm.is_completed = "0"';
		$task_count = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'','','','','','','','','',$where,'','1');
		$data['task_count'] = !empty($task_count)?$task_count:0;
		if(strtotime($dt) <= strtotime($current_date))
		{
			//$where=array('tut.user_id'=> $id,'DATE_FORMAT(tm.task_date,"%Y-%m-%d") < '=>" '$current_date'",'tut.is_completed'=>"'0'");
			$where = 'tut.user_id IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(tm.task_date,"%Y-%m-%d") < "'.$dt.'" AND tm.is_completed = "0"';
			$task_overdue_count = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'','','','','','','','','',$where,'','1');
		}
		$data['task_overdue_count'] = !empty($task_overdue_count)?$task_overdue_count:0;

		///////////////END TASK. 11-11-2014 SANJAY CHABHADIYA//////////////////////
		
		///////////////TELEPHONE TASK (CALL). 11-11-2014 SANJAY CHABHADIYA//////////////////////
		$table = "interaction_plan_contact_communication_plan as ipccp";
		$join_tables = array('contact_master as cm' => 'cm.id = ipccp.contact_id',
							 'interaction_plan_interaction_master as ipim'=>'ipim.id = ipccp.interaction_plan_interaction_id'																 							 );
		$fields = array('ipccp.id');
				
		//$where = array('ipim.assign_to'=> $id,'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d")'=>"'$dt'",'ipccp.interaction_type'=>"'4'",'ipccp.is_done'=>"'0'");
		$where = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") = "'.$dt.'" AND ipccp.interaction_type = "4" AND ipccp.is_done = "0"';
		...
		{
			$where = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") < "'.$dt.'" AND ipccp.interaction_type = "4" AND ipccp.is_done = "0"';
			//$where=array('ipim.assign_to'=> $id,'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") < '=>"'$current_date'",'ipccp.interaction_type'=>"'4'",'ipccp.is_done'=>"'0'");
		...
		///////////////END TELEPHONE TASK (CALL). 11-11-2014 SANJAY CHABHADIYA//////////////////////
		
		///////////////EMAIL TASK. 11-11-2014 SANJAY CHABHADIYA//////////////////////
		$table = "interaction_plan_contact_communication_plan as ipccp";
		$join_tables = array('contact_master as cm' => 'cm.id = ipccp.contact_id',
							 'interaction_plan_interaction_master as ipim'=>'ipim.id = ipccp.interaction_plan_interaction_id'																 							 );
		$fields = array('ipccp.id,ipccp.task_date');
		 
		//$where=array('ipim.assign_to'=> $id,'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d")'=>"'$dt'",'ipccp.interaction_type'=>'6','ipccp.is_done'=>"'0'");
		$where = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") = "'.$dt.'" AND ipccp.interaction_type = "6" AND ipccp.is_done = "0"';
		...
		if(strtotime($dt) <= strtotime($current_date))
		{
			//$where=array('ipim.assign_to'=> $id,'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") < '=>"'$current_date'",'ipccp.interaction_type'=>'6','ipccp.is_done' => "'0'");
			$where = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") < "'.$dt.'" AND ipccp.interaction_type = "6" AND ipccp.is_done = "0"';
		...
		///////////////END EMAIL TASK. 11-11-2014 SANJAY CHABHADIYA//////////////////////
		
		///////////////SMS TASK. 11-11-2014 SANJAY CHABHADIYA//////////////////////
		$table = "interaction_plan_contact_communication_plan ipccp";
		$join_tables = array('contact_master as cm' => 'cm.id = ipccp.contact_id',
							 'interaction_plan_interaction_master as ipim'=>'ipim.id = ipccp.interaction_plan_interaction_id'																 							 );
		$fields = array('ipccp.id,ipccp.task_date');
		
		//$where=array('ipim.assign_to'=> $id,'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d")'=>"'$dt'",'ipccp.interaction_type'=>"'3'",'ipccp.is_done'=>"'0'");
		$where = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") = "'.$dt.'" AND ipccp.interaction_type = "3" AND ipccp.is_done = "0"';
		...
		if(strtotime($dt) <= strtotime($current_date))
		{
			//$where=array('ipim.assign_to'=> $id,'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") < '=>"'$current_date'",'ipccp.interaction_type'=>"'3'",'ipccp.is_done'=>"'0'");
			$where = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") < "'.$dt.'" AND ipccp.interaction_type = "3" AND ipccp.is_done = "0"';
		...
		$where = "ipim.assign_to IN (".$this->user_session['agent_id'].") AND DATE_FORMAT(ipccp.task_date,'%Y-%m-%d') = '$dt' AND ipccp.is_done = '0' AND (ipccp.interaction_type = '1' OR ipccp.interaction_type = '2' OR ipccp.interaction_type = '5')";
		...
		$data['task_lel_count'] = !empty($task_lel_count)?$task_lel_count:0;
	   	...
	   	if(strtotime($dt) <= strtotime($current_date))
		{
			$where = "ipim.assign_to IN (".$this->user_session['agent_id'].") AND DATE_FORMAT(ipccp.task_date,'%Y-%m-%d') < '$current_date' AND ipccp.is_done = '0' AND (ipccp.interaction_type = '1' OR ipccp.interaction_type = '2' OR ipccp.interaction_type = '5')";
		...
		$fields = array('ipccp.id,ipccp.task_date');
        //$match = array();
		$where = "ipim.assign_to IN (".$this->user_session['agent_id'].") AND DATE_FORMAT(ipccp.task_date,'%Y-%m-%d') = '$dt' AND ipccp.is_done = '0' AND ipccp.interaction_type = '7'";
		...
		if(strtotime($dt) <= strtotime($current_date))
		{
			$where = "ipim.assign_to IN (".$this->user_session['agent_id'].") AND DATE_FORMAT(ipccp.task_date,'%Y-%m-%d') < '$current_date' AND ipccp.is_done = '0' AND ipccp.interaction_type = '7'";
		...
		{
			$fields = array('id,contact_pic');
			$match = array('id'=>$result[0]['user_id']);
			$data['prifile_pic'] = $this->user_management_model->select_records($fields,$match,'','=');
		}
		//////////////////////// End calendar events ////////////////////////
	...
	public function popup_changes()
	{
		...
		for($k=0;$k<count($myarray_cal);$k++)
		{
			$cdata['id']=$myarray_cal[$k];
			$cdata['is_close']='1';
	...
	*/
    public function daily_task()
	{
		...
		if(!empty($session_data))
			...
			$curr_date = date('Y-m-d');
		$data['dt'] = $curr_date;
		$where = 'tut.user_id IN ('.$this->user_session['agent_id'].') AND tm.is_completed = "0"';
		if(!empty($searchtext))
		{
			$data['datalist'] = $this->obj->dashboard_task_list($curr_date, $config['per_page'], $uri_segment,$searchtext, $data['sortfield'],$data['sortby'],'',$where);
			$config['total_rows'] = $this->obj->dashboard_task_list($curr_date, '', '',$searchtext, $data['sortfield'],$data['sortby'],'1',$where);
		}
		else
		{
			$data['datalist'] = $this->obj->dashboard_task_list($curr_date, $config['per_page'], $uri_segment,'', $data['sortfield'],$data['sortby'],'',$where);
			//pr($data['datalist']);exit;
			$config['total_rows'] = $this->obj->dashboard_task_list($curr_date, '', '','', $data['sortfield'],$data['sortby'],'1',$where);
		}
	...
	*/
	public function form_lead_list()
	{
		...
		if(!empty($current_date))
				redirect('user/'.$this->viewName.'/form_lead_list');
		}
		$table = "contact_master as cm";
       

        $group_by='cm.id';
		$fields = array('ls.id as form_id,cm.id,cm.is_subscribe','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','ld.created_ip','ld.domain_name','ld.created_date as filled_date','ls.form_title','ld.id as lead_id');
        $join_tables = array(
    ...
    public function iscompleted()
	{
		...
		$task_id = $this->input->post('selectedvalue');
		//update user task
		if(!empty($task_id))
		{
			$udata['task_id'] = $task_id;
			$udata['user_id'] = $user_id['id'];
			$udata['is_completed'] = '1';
			$udata['completed_date'] = date('Y-m-d h:i:s');
			$udata['completed_by'] = $this->user_session['id'];
			$this->task_model->update_task($udata);
			//echo $this->db->last_query();
		}
		
		$where = array('id'=>$task_id,'created_by IN_query IN'=>$this->user_session['agent_id']);
		$fields = array('id','is_completed');
		$userlist = $this->task_model->select_records($fields,'','','','','','','','',$where);
		//echo $this->db->last_query();
		//pr($userlist);exit;
	...
	*/
	public function ajax_delete_all()
	{
		...
		$idata['user_id'] = $this->user_session['id'];
		if(!empty($id))
		{
			$idata['task_id'] = $id;
			$this->task_model->delete_single_user_task($idata);

			$match = array('id'=>$id,'created_by'=>$this->user_session['id']);
			$fields = array('id');
			$userlist = $this->task_model->select_records($fields,$match,'','=');
			if(count($userlist) > 0)
		...
		elseif(!empty($array_data))
		{
			...
			{
				$idata['task_id'] = $array_data[$i];
				$this->task_model->delete_single_user_task($idata);
				$match = array('id'=>$array_data[$i],'created_by'=>$this->user_session['id']);
				$fields = array('id');
				$userlist = $this->task_model->select_records($fields,$match,'','=');
				if(count($userlist) > 0)
	...
	*/
	...
	public function email_task($flag='')
	{
		...
		/*if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = array('ipim.assign_to'=> $this->user_session['id'],'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") <= '=>"'$dt'",'ipccp.interaction_type'=>"'6'",'ipccp.is_done'=>"'0'");
		else
			$wherestring = array('ipim.assign_to'=> $this->user_session['id'],'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d")'=>"'$dt'",'ipccp.interaction_type'=>"'6'",'ipccp.is_done'=>"'0'");*/
		if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") <= "'.$dt.'" AND ipccp.interaction_type = "6" AND ipccp.is_done = "0"';
		else
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") = "'.$dt.'" AND ipccp.interaction_type = "6" AND ipccp.is_done = "0"';
		...
		if(!empty($searchtext))
		{
			...
			$data['datalist'] = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'],$uri_segment,$data['sortfield']." ".$data['sortby'].",cm.id",'',$group_by,$wherestring);
	...
	*/
	...
	public function letter_label_envelope_task()
	{
		...
		if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = "ipim.assign_to IN (".$this->user_session['agent_id'].") AND DATE_FORMAT(ipccp.task_date,'%Y-%m-%d') <= '$dt' AND ipccp.is_done = '0' AND (ipccp.interaction_type = 1 OR ipccp.interaction_type = 2 OR ipccp.interaction_type = 5)";
		else
			$wherestring = "ipim.assign_to IN (".$this->user_session['agent_id'].") AND DATE_FORMAT(ipccp.task_date,'%Y-%m-%d') = '$dt' AND ipccp.is_done = '0' AND (ipccp.interaction_type = 1 OR ipccp.interaction_type = 2 OR ipccp.interaction_type = 5)";
		...
		if(!empty($searchtext))
		{
			...
			$match = array();
			//$match = array('CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name)'=>$searchtext,'ltm.template_name'=>$searchtext,'etm.template_name'=>$searchtext,'lltm.template_name'=>$searchtext,'CONCAT_WS(" >> ",ipm.plan_name,ipim.description)'=>$searchtext);
			$data['datalist'] = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','=',$config['per_page'],$uri_segment,$data['sortfield']." ".$data['sortby'].",cm.id",'',$group_by,$wherestring,'','',$having);
			$config['total_rows'] = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','=','','','','',$group_by,$wherestring,'','1',$having);
		}
		else
		{
			$data['datalist'] = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config['per_page'],$uri_segment,$data['sortfield']." ".$data['sortby'].",cm.id",'',$group_by,$wherestring);
			//pr($data['datalist']);exit;
			$config['total_rows'] = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','','',$wherestring,$group_by,'1');
		}
	...
	/*
		@Description: Function for complete the task than update the status
		@Author     : Sanjay Chabhadiya
		@Input      : Task id
		@Output     : 
		@Date       : 8-11-2014
	...
	public function mail_out()
	{
		...
		if(!empty($id))
		{
			...
			for($j=0;$j<count($id);$j++)
			{
				...
				$finlaOutput = '';
				$finlaOutputPrint = '';
				//pr($interaction_communication_plan);
			...
			$data['main_content'] = "user/home/list";
			$this->load->view('user/include/template', $data);	
		}
		else
	...
	{	
		...
		if(!empty($interaction_id))
		{
			...
			for($i=0;$i<count($interaction_id);$i++)
			{
				...
				/*$data['size_w']  = $mm * $this->input->post('size_w_'.$interaction_id[$i]);
				$data['size_h']  = $mm * $this->input->post('size_h_'.$interaction_id[$i]);*/
				//pr($data);
				if(!empty($data))
				{
	...
	*/
	...
	public function telephone_task()
	{
		...
		{
			//$searchtext = $this->input->post('searchtext');
			$data['searchtext'] = stripcslashes($searchtext);
		} else {
			if(empty($allflag))
			{
				if(!empty($searchsort_session['searchtext'])) {
		...
		/*if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = array('ipim.assign_to'=> $this->user_session['id'],'ipccp.is_done' => "'0'",'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") <= '=>"'$dt'",'ipccp.interaction_type'=>'4');
		else
			$wherestring = array('ipim.assign_to'=> $this->user_session['id'],'ipccp.is_done' => "'0'",'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d")'=>"'$dt'",'ipccp.interaction_type'=>'4');*/
		if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND ipccp.is_done = "0" AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") <= "'.$dt.'" AND ipccp.interaction_type = "4"';
		else
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND ipccp.is_done = "0" AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") = "'.$dt.'" AND ipccp.interaction_type = "4"';
		$group_by = 'ipccp.id';
		if(!empty($searchtext))
	...
	*/
	...
	public function phone_call_popup()
	{
		...
		if($this->input->post('submitbtn'))
		{
			...
			$idata['completed_by'] = $this->user_session['id'];
			...
			if($this->input->post('finish') == 1)
				$data['finish'] = 1;
				//redirect('user/'.$this->viewName.'/telephone_task');
			//$this->
		}
	...
	*/
	...
	public function sms_task()
	{
		...
		/*if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = array('ipim.assign_to'=> $this->user_session['id'],'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") <= '=>"'$dt'",'ipccp.interaction_type'=>"'3'",'ipccp.is_done'=>"'0'");
		else
			$wherestring = array('ipim.assign_to'=> $this->user_session['id'],'DATE_FORMAT(ipccp.task_date,"%Y-%m-%d")'=>"'$dt'",'ipccp.interaction_type'=>"'3'",'ipccp.is_done'=>"'0'");*/
		if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") <= "'.$dt.'" AND ipccp.interaction_type = "3" AND ipccp.is_done = "0"';
		else
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") = "'.$dt.'" AND ipccp.interaction_type = "3" AND ipccp.is_done = "0"';
		$group_by = 'ipccp.id';
		if(!empty($searchtext))
    ...
    */
	...
	public function to_do_task()
	{
		...
		if(strtotime($dt) <= strtotime(date('Y-m-d')))
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND ipccp.is_done = "0" AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") <= "'.$dt.'" AND ipccp.interaction_type = "7"';
		else
			$wherestring = 'ipim.assign_to IN ('.$this->user_session['agent_id'].') AND ipccp.is_done = "0" AND DATE_FORMAT(ipccp.task_date,"%Y-%m-%d") = "'.$dt.'" AND ipccp.interaction_type = "7"';
    ...
    */
	...
	public function view_error_data()
	{
		...
		$where = array('status'=>1,'created_by'=>$this->user_session['id']);
		/*if(!empty($searchtext))
		{
