...
//$uri_segment = $this->uri->segment(3);
		...
		$wherestring = 'ecm.email_type = "Campaign" AND ecm.created_by IN ('.$this->user_session['agent_id'].') AND ecm.email_blast_type = 0';
		...
		pr($data['udata']);exit;*/
		//$data['send_mail'] = $this->obj->total_emails($this->user_session['id']);
		//pr($config);
    ...
    */
    ...
    public function add_record()
    {
		...
		//check user right
		if(!in_array('email_blast_add',$this->modules_unique_name) && !in_array('bomb_bomb_email_blast_add',$this->modules_unique_name) && !in_array('label_add',$this->modules_unique_name)){ 
		show_404();
		}
		...
		$data['contact_list'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config['per_page'],'','ctm.id','desc','',$where);
...
*/
		...
		$where='(cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].')';		
		$data['contact_list_cc'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config1['per_page'],'','ctm.id','desc','',$where);
		$config1['total_rows'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','','',$where,'','1');
		...
		$where='(cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].')';		
		$data['contact_list_bcc'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config_bcc['per_page'],'','ctm.id','desc','',$where);
		$config_bcc['total_rows'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','','',$where,'','1');
		...
		$where = 'cm.is_subscribe = "0" AND (cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].') AND (cet.is_default = "1" OR cet.email_type = 1)';
		...
		$match = 'created_by IN ('.$this->user_session['agent_id'].')';
		$data['email_signature_data'] =$this->obj->getmultiple_tables_records($table,'','','','','','','','','','','',$match);
		//pr($data['email_signature_data']);exit;
    ...
    */
    ...
    public function insert_data()
    {
		...
		if($this->input->post('submitbtn') || $this->input->post('submitbtn1'))
		{
			...
			if($this->input->post('submitbtn') && $data['email_send_type'] != 2)
			{
				...
				if($this->input->post('email_cc'))
				{
					...
					if(!empty($contact_type_cc))
					{
						for($i=0;$i<count($contact_type_cc);$i++)
						{
							$contact_type_data = '';
							$table = "contact_master as cm";
							$fields = array('cm.id,cet.email_address');
							$join_tables = array(
							...
							$where = 'cct.contact_type_id = '.$contact_type_cc[$i].' AND cm.is_subscribe = "0" AND cet.is_default = "1" AND (cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].')';
							$group_by='cm.id';
				...
				if($this->input->post('email_bcc'))
				{
					...
					if(!empty($contact_type_bcc))
					{
						for($i=0;$i<count($contact_type_bcc);$i++)
						{
							...
							$where = 'cct.contact_type_id = '.$contact_type_bcc[$i].' AND cm.is_subscribe = "0" AND cet.is_default = "1" AND (cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].')';
							$group_by='cm.id';
							$contact_type_data = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$where);
							/*$table ="contact_contacttype_trans as cct";   
							$fields = array('cct.*');
				...
				$group_by = 'lm.id';
				$wherestring = "lm.id = ".$this->user_session['id'];
			...
			if(!empty($contact_type))
			{
				for($i=0;$i<count($contact_type);$i++)
				{
					...
					$cdata['contact_type'] = $contact_type[$i];
					$table = "contact_master as cm";
					$fields = array('cm.id,cet.id as email_trans_id');
					$join_tables = array(
							'contact_contacttype_trans  as cct'=>'cct.contact_id = cm.id',
							'contact__type_master as ctm'=>'ctm.id = cct.contact_type_id',
							'user_contact_trans as uct'=>'uct.contact_id = cm.id',
					...
					$where = 'cct.contact_type_id = '.$contact_type[$i].' AND (cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].') AND cet.is_default = "1" AND cm.is_subscribe = "0"';
					$group_by='cm.id';
					/*$table ="contact_contacttype_trans as cct";
					$fields = array('cm.*');
					$join_tables = array('contact_master as cm'=>'cm.id = cct.contact_id',
   ...
   */
 	...
 	public function send_mail($contact_id='',$headers='',$data='',$cdata='',$send_mail_count='',$email_signature='')
	{
		...
		if(!empty($contact_id))
		{
			...
			//$wherestring = "cet.is_default = '1' AND (cet.is_default = '1' OR cet.email_type = 1)";
    ...
    */
    ...
    public function edit_record()
    {
		...
		$table = 'email_campaign_master';
		$where = 'id = '.$id.' AND created_by IN ('.$this->user_session['agent_id'].') AND email_blast_type = 0';
		...
		//$where = array('cm.is_subscribe'=>"'0'");
		$cdata['contact'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config_to1['per_page'],'0','cm.first_name','asc',$group_by,$where);
		...
		$table = 'email_signature_master';
		$match = 'created_by IN ('.$this->user_session['agent_id'].')';
		$cdata['email_signature_data'] =$this->obj->getmultiple_tables_records($table,'','','','','','','','','','','',$match);		
		...
		if(!empty($email_cc))
		{
			...
			if(!empty($email_cc))
			{
				...
				$cdata['email_cc'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','cm.first_name','asc',$group_by,$where,$where_in);
		...
		if(!empty($email_bcc))
		{
			...
			if(!empty($email_bcc))
			{
				...
				//$where = array('cm.is_subscribe'=>"'0'"); 
				$where = "cm.is_subscribe = '0' AND (cet.is_default = '1' OR cet.email_type = 1)";
				$cdata['email_bcc'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','cm.first_name','asc',$group_by,$where,$where_in);
				/*echo $this->db->last_query();
		...
		$cdata['contact_list_cc'] = $this->contact_type_master_model->select_records('','','','','',$config1['per_page'],'0','id','desc');
		//echo $this->db->last_query();
		...
		$cdata['contact_list_bcc'] = $this->contact_type_master_model->select_records('','','','','',$config_bcc['per_page'],'0','id','desc');
		//echo $this->db->last_query();
		...
		$group_by='cet.id';
		$where='cm.is_subscribe = "0" AND (cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].') AND (cet.is_default = "1" OR cet.email_type)';
	...
	/*	$match = array('is_subscribe'=>'0');
		$cdata['contact_cc'] = $this->contacts_model->select_records('',$match,'','=','',$config_to1['per_page'],'','id','desc');
		//echo $this->db->last_query();
		...
		$group_by='cm.id';
		$where = array('cm.is_subscribe'=>"'0'");*/
		$cdata['contact_cc'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config_to1['per_page'],'0','cm.first_name','asc',$group_by,$where);
		$config_to1['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$where,'','1');
		...
		$config_to1['base_url'] = site_url($this->user_type.'/'."emails/search_contact_bcc");
		/*$match = array('is_subscribe'=>'0');
		$cdata['contact_bcc'] = $this->contacts_model->select_records('',$match,'','=','',$config_to1['per_page'],'','id','desc');
		//echo $this->db->last_query();
		$config_to1['total_rows'] = count($this->contacts_model->select_records('',$match,'','='));*/
		...
		$group_by='cm.id';
		$where = array('cm.is_subscribe'=>"'0'");*/
		$cdata['contact_bcc'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config_to1['per_page'],'0','cm.first_name','asc',$group_by,$where);
		$config_to1['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$where,'','1');
		$this->pagination->initialize($config_to1);
    ...
    public function update_data()
    {
		...
		if($this->input->post('submitbtn') || $this->input->post('submitbtn1'))
		{
			...
			if($this->input->post('submitbtn') && !empty($id) && $data['email_send_type'] != 2)
			{
				...
				$table = "login_master lm";
				$fields = array('um.first_name,um.middle_name,um.last_name,lm.email_id');
				$join_tables = array(
								 	 'user_master um'=>'um.id = lm.user_id',
								  	);
				$group_by = 'lm.id';
			...
			if(!empty($contact_type))
			{
				for($i=0;$i<count($contact_type);$i++)
				{
					...
					$where = 'cct.contact_type_id = '.$contact_type[$i].' AND (cm.created_by IN ('.$this->user_session['agent_id'].' OR uct.user_id = '.$this->user_session['agent_user_id'].')';
					$group_by='cm.id';
					$contact_type_data = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$where);
   ...
   */
	...
	public function ajax_templatedata()
	{
		...
		{
			$table = "bomb_template_master as em";
			$fields = array('em.*');
			$join_tables = array(
									'login_master as lm' => 'lm.id = em.created_by',
								 );
			$wherestring='em.template_category = '.$category_id.' AND (lm.user_type = "1" OR lm.user_type = "2" OR em.created_by IN ('.$this->user_session['agent_id'].'))';
			$group_by = 'em.id';
			$data['templatedata'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','em.id','desc',$group_by,$wherestring);
		
			echo json_encode($data['templatedata']);
		}
		else
		{
			$table = "email_template_master as em";
			$fields = array('em.*');
			$join_tables = array(
									'login_master as lm' => 'lm.id = em.created_by',
								 );
			$wherestring='em.template_category = '.$category_id.' AND em.email_send_type = 2 AND (lm.user_type = "1" OR lm.user_type = "2" OR em.created_by IN ('.$this->user_session['agent_id'].'))';
			$group_by = 'em.id';
			$data['templatedata'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','em.id','desc',$group_by,$wherestring);
	...
	public function sent_email()
	{
		...
		{
			$msg = $this->lang->line('common_right_msg_email_sent');
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('user/'.$this->view);	
		}
...
//$uri_segment = $this->uri->segment(5);
		...
		$join_tables = array('email_campaign_recepient_trans as ect'=>'ect.contact_id = cm.id','user_contact_trans as uct'=>'uct.contact_id = cm.id');
		$wherestring = "ect.email_campaign_id = ".$email_campaign_id; //." AND is_send = '1'";
		//$wherestring='(ect.email_campaign_id = '.$email_campaign_id.' AND cm.created_by = '.$this->user_session['id'].' OR uct.user_id = '.$this->user_session['user_id'].')';
		$group_by = 'cm.id';
		if(!empty($searchtext))
		{	
...
SEPARATOR ',') as attachment_name,cm.first_name,cm.last_name,mml1.category as category,etm.template_name,ecr.*,esm.full_signature,ecr.email_address,cet.email_address as default_email_address");
		...
		$wherestring = 'ecr.email_campaign_id = '.$data['campaign_id'].' AND ecr.id = '.$data['id']. ' AND ecm.created_by IN ('.$this->user_session['agent_id'].') AND ecm.email_blast_type = 0';
	...
	//pr($cdata['datalist']);exit;
		...
		{
			$msg = $this->lang->line('common_right_msg_email_send_his');
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('user/'.$this->viewName.'/all_sent_mail');	
		}
		/*pr($cdata['datalist']);
		echo $this->db->last_query();exit;*/
		$email_cc = '';
	...
	public function search_contact_ajax_cc()
    {
		...
		$this->pagination->initialize($config);		
		$data['pagination_cc'] = $this->pagination->create_links();
        $this->load->view("user/".$this->viewName."/add_contact_popup_ajax_cc", $data);
	}
   ...
   */
	...
	public function search_contact_ajax_bcc()
    {
		...
		$this->pagination->initialize($config);		
		$data['pagination_bcc'] = $this->pagination->create_links();
        $this->load->view("user/".$this->viewName."/add_contact_popup_ajax_bcc", $data);
	}
   ...
   */
	...
	public function resend_mail()
	{
		...
		if(!empty($contact_id))
		{
			...
			$group_by = 'cet.contact_id';
			$wherestring = "ecr.is_send = '0' AND cm.is_subscribe = '0' AND ecr.email_campaign_id = ".$campaign_id." AND (cet.is_default = '1' OR cet.email_type = 1)";
			...
			if(count($campaign_data) > 0)
			{
				...
				if(!empty($fromdata))
					$from = "From: ".trim($fromdata[0]['first_name'])." ".trim($fromdata[0]['middle_name'])." ".trim($fromdata[0]['last_name'])." <".$fromdata[0]['email_id'].">\r\n";
				$data['from_email'] = !empty($fromdata[0]['email_id'])?$fromdata[0]['email_id']:'';
				$data['from_name'] = !empty($from)?$from:'';
				$data['from_cc'] = $from_cc;
				$data['from_bcc'] = $from_bcc;
    ...
    */
	...
	{	
		...
		//$wherestring = "ecr.is_send = '1'";
		$wherestring = 'ecr.is_send = "1" AND ecm.created_by IN ('.$this->user_session['agent_id'].') AND ecm.email_blast_type = 0';
		//ecm.is_draft = '0' AND
    ...
    */
	...
	public function queued_list()
	{
		...
		$wherestring = "ecm.email_type = 'Intereaction_plan' AND ecr.is_send = '0' AND ipm.status = '1' AND ipi.status='1' AND ecm.email_blast_type = 0 AND (ipm.created_by IN (".$this->user_session['agent_id'].") OR ipi.assign_to IN (".$this->user_session['agent_id']."))";
		$groupby = 'ipm.id';
    ...
    */
	...
	public function interaction_queued_list()
	{
		...
		$match=array('ipm.plan_name'=>$searchtext);
		$wherestring = "ecm.email_type = 'Intereaction_plan' AND ecr.is_send = '0' AND ecm.email_blast_type = 0 AND ipm.status = '1' AND ipm.created_by IN (".$this->user_session['agent_id'].')';
		$groupby = 'ipm.id';
		$data['interaction_plan'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'],$uri_segment,$sortfield,$sortby,$groupby,$wherestring);
		$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','',$sortfield,$sortby,$groupby,$wherestring,'','1');
	...
	public function interaction_plan_queued_list()
	{
		...
		$join_tables = array('email_campaign_master as ecm'=>'ecm.id = ecr.email_campaign_id',
							...
							/*email_campaign_master as ecm'=>'ecm.id = ecr.email_campaign_id',
							 ...
							 'interaction_plan_interaction_master as ipi'=>'ipi.id = ecm.interaction_id',
							 'interaction_plan_master as ipm'=>'ipm.id = ipi.interaction_plan_id'*/
		$wherestring = "ecm.email_type = 'Intereaction_plan' AND ipm.id = ".$interaction_plan." AND ecr.is_send = '0' AND ecm.email_blast_type = 0 AND ipi.status = '1' AND (ipm.created_by IN (".$this->user_session['agent_id'].") OR ipi.assign_to IN (".$this->user_session['agent_id']."))";
		$groupby = 'ecr.id';
		if(!empty($searchtext))
		{
		...
		{
			$msg = $this->lang->line('common_right_msg_email_queued');
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
    ...
    */
	...
	{
		//You do not have permission to view queued in emails 

		$id = $this->uri->segment(4);
		$table = "email_campaign_recepient_trans as ecr";
		$fields = array("ecm.email_message,ecm.created_by,ecm.template_subject,GROUP_CONCAT(DISTINCT ect.attachment_name
SEPARATOR ',') as attachment_name,cm.first_name,cm.last_name,mml1.category as category,etm.template_name,ecr.*,esm.full_signature,cet.email_address,ecm.interaction_id,ipim.assign_to");
		$join_tables = array('email_campaign_master ecm'=>'ecm.id = ecr.email_campaign_id',
							...
							'interaction_plan_interaction_master as ipim'=>'ipim.id = ecm.interaction_id'
							);
		$group_by = 'cet.contact_id';
		$wherestring = "cet.is_default = '1' AND cm.is_subscribe = '0' AND ecr.is_send = '0' AND ecm.email_blast_type = 0 AND ecr.id = ".$id;
    ...
    */
	...
	public function interaction_mailsms()
	{
		...
		$group_by = 'cet.contact_id';
		$wherestring = "cet.is_default = '1' AND ecr.is_send = '0' AND cm.is_subscribe = '0' AND ecm.email_blast_type = 0 AND ecr.id = ".$id;
		...
		if(count($result) > 0)
		{
			...
			$group_by = 'lm.id';
			$wherestring = "lm.id = ".$this->user_session['agent_id'];
			$fromdata = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$wherestring);
			
			if(!empty($fromdata))
				$headers .= "From: ".trim($fromdata[0]['first_name'])." ".trim($fromdata[0]['middle_name'])." ".trim($fromdata[0]['last_name'])." <".$fromdata[0]['email_id'].">\r\n";*/
			//pr($fromdata);exit;
			...
			if($remain_emails > 0)
			{
				...
				if(!empty($result[0]['email_address']))
				{
					...
					if(!empty($result))
					{
						...
						common_rescheduled_task($interaction_plan_id);*/
					}
				}
			}
			else
    ...
    */
	...
	function search_contact_to()
	{
		...
		$where .= '(cet.is_default = "1" OR cet.email_type = 1) AND (cm.is_subscribe = "0" AND cm.created_by IN ('.$this->user_session['agent_id'].') OR uct.user_id = '.$this->user_session['agent_user_id'].')';
		$table = "contact_master as cm";
		$fields = array('cm.id,cet.id as email_trans_id,cet.email_type','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','cm.company_name','cet.email_address');
		$join_tables = array(
							'contact_emails_trans as cet'=>'cet.contact_id = cm.id',
	...
	function search_contact_cc()
	{
		...
		$data['pagination_contact_cc'] = $this->pagination->create_links();
        $this->load->view("user/".$this->viewName."/contact_cc", $data);
	}
	...
	function search_contact_bcc()
	{
		...
		$data['pagination_contact_bcc'] = $this->pagination->create_links();
        $this->load->view("user/".$this->viewName."/contact_bcc", $data);
	}
	...
	public function contacts_to_email()
	{
		...
		if(!empty($email_trans_id))
			$data['contacts_data'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','cm.first_name','asc',$group_by,$where,$where_in);
		//echo $this->db->last_query();
		echo json_encode($data['contacts_data']);
	}
