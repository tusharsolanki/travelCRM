...
{	
    ...
    */
    ...
    {	
		...
		if($main == 1)	
		{
			...
			$wherestring = 'label_template_master.created_by IN ('.$this->user_session['agent_id'].')';
		...
		if($def == 1)
		{
			...
			$selected_cat = $this->input->post('selected_cat');
			$data['default_sortfield']		= 'label_template_master.id';
			...
			$join_tables = array(
								...
								'marketing_master_lib__category_master mmlcm'=>'mmlcm.id = label_template_master.template_category',
							);
			$group_by = 'label_template_master.id';
			if(!empty($default_searchtext) || !empty($default_selected_cat))
			{
				...
				if(!empty($default_selected_cat))
					$wherestring='label_template_master.template_category = '.$default_selected_cat.' AND (lm.user_type = "1" OR lm.user_type = "2")';
				else
    ...
    */
    ...
    public function edit_record()
    {
        ...
        $result = $this->obj->getmultiple_tables_records('label_template_master','','','','','','','','','','','',$where);
		$cdata['editRecord'] = $result;
		if(empty($cdata['editRecord']))
		{
			$msg = $this->lang->line('common_right_msg_label_lib_script');
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('user/'.$this->viewName);	
		}
