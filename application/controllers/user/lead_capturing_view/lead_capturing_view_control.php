...
*/
    ...
    {	
		...
		/*$match3=array('status'=> '1','id'=>$this->user_session['user_id']);
    ...
    */
    public function user_list()
    {
		...
		$join_tables = array('login_master as lm' => 'um.id = lm.user_id');
        $match=array('um.status'=> '1','um.id'=>$this->user_session['user_id']);
		$data['userlist'] = $this->contact_masters_model->getmultiple_tables_records($table,$fields,$join_tables,'left','',$match,'=','','','','asc');
	
		/*$match=array('status'=> '1','id'=>$this->user_session['user_id']);
    ...
    public function assign_contact()
    {
		...
		if(!empty($data1))
		{
			...
			if(!empty($lastId))
			{
				...
				$cdata6['created_by']=$this->user_session['id'];
			...
			redirect(base_url('user/'.$this->viewName.'/'.$formid));
		}
	}	
	 /*
    ...
    */
	...
	public function assign_lead()
	{
		...
		for($i=0;$i<count($array_data);$i++)
		{
			...
			if(empty($lead_check))
			{
				...
				$data1 = $this->obj->select_records1('',$match1,'','=');
				if(!empty($data1[0]['first_name_data'])||!empty($data1[0]['last_name_data'])||$data1[0]['single_line_data']||$data1[0]['paragraph_data'])
				{
					//data for contact master tabel
					$cdataname['first_name']=$data1[0]['first_name_data'];
					$cdataname['last_name']=$data1[0]['last_name_data'];
	...
	public function contact_details11()
	{
