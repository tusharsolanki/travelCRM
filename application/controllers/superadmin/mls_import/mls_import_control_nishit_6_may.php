...
{	
    ...
    */
    ...
    function dump_database($table_name,$id='',$minute='',$hour='',$day='',$month='')
    {
        ...
        if($worked == 0)
        {
            echo "Import to staging database".$table_name.'<br>';
            $this->dump_master_database($table_name,$id,$minute,$hour,$day,$month);
        }
    }
    function dump_master_database($table_name,$id='',$minute='',$hour='',$day='',$month='')
    {
        //ENTER THE RELEVANT INFO BELOW
        ...
        if($worked == 0)
        {
            //echo "Import to master database".$table_name.'<br>';
    ...
    function import_database1()
    {
        try
        {
            $mysqlDatabaseName ='asheville';
            $mysqlUserName ='asheville';
            $mysqlPassword ='asheville';
            $mysqlHostName ='104.130.52.103';
            $mysqlExportPath ='rest1.sql';
           ...
           /* switch($worked){
            ...
            break;*/
            //$command = " mysqldump -u root -p'' livewire_mls foo | ssh 104.130.52.103 mysql -u asheville -p'asheville' asheville foo";
            //$command="mysqldump -u root -p  livewire_mls | ssh 104.130.52.103 mysql -u asheville -p asheville asheville foo";
           /* $command='ssh -f -L3310:104.130.52.103:3306 asheville@104.130.52.103 -N';
            $command1='mysqldump -u root -p  livewire_mls | ssh 104.130.52.103 mysql -u asheville -p asheville asheville foo';
            //mysqldump -u user1 -ppassword1 databasename > dump.sql
           
            $d=system($command);
            $d=system($command1);*/
            //mysqldump -u asheville -p asheville -h 104.130.52.103 asheville > dump.sql
            //exec('mysqldump -u root -p livewire_mls foo | mysql -u root -p live_test foo');
            echo $worked;
           if($worked == 0)
           {
                $this->dump_database();
    ...
    function dump_database1()
    {
        //ENTER THE RELEVANT INFO BELOW
        $mysqlDatabaseName ='drashti';
        $mysqlUserName ='root';
        $mysqlPassword ='';
                    ...
                    foreach($property_data as $row)
                    {
                        ...
                        if(empty($res))
                        {
                                ...
                                if(count($data) >= 100)
                                {
                                        ...
                                        //$this->mls_model->insert_record1($data,$this->mls_master_db);
                                        $i = 0;
                                        unset($data);
                                }
                                $i++;
    ...
    public function retrieve_member_data()
    {
        ...
        foreach($mls_data as $key=>$value)
        {
            ...
            foreach($property_data as $row)
            {
                ...
                {
                    $data[$i]['created_date']       =date('Y-m-d h:i:s');
                    $id=$this->obj->insert_record6($data,$this->mls_staging_db);
                }
                else
                {
                    $id=$this->obj->update_record6($data,$this->mls_staging_db);
                    //echo $this->db->last_query();
                }
            }
            if(!empty($data) && count($data) > 0)
    ...
    public function retrieve_office_data()
    {
        ...
        foreach($mls_data as $key=>$value)
        {
            ...
            foreach($property_data as $row)
            {
                ...
                {
                    $data[$i]['created_date']       =date('Y-m-d h:i:s');
                    $id=$this->obj->insert_record7($data,$this->mls_staging_db);
                }
                else
                {
                    $id=$this->obj->update_record7($data,$this->mls_staging_db);
                    //echo $this->db->last_query();
                }
            }
            if(!empty($data) && count($data) > 0)
    ...
    public function retrieve_school_data()
    {
        ...
        foreach($mls_data as $key=>$value)
        {
            ...
            foreach($property_data as $row)
            {
                ...
                {
                    $data[$i]['created_date']       =date('Y-m-d h:i:s');
                    $id=$this->obj->insert_record8($data,$this->mls_staging_db);
                }
                else
                {
                    $id=$this->obj->update_record8($data,$this->mls_staging_db);
                    //echo $this->db->last_query();
                }
            }
            if(!empty($data) && count($data) > 0)
            ...
            for($month_range=1;$month_range<=12;$month_range++)
            {
            	for($weekdaysdata=1;$weekdaysdata<=28;$weekdaysdata+=7)
                {
	                ...
	                //{
						...
						{
							$new_url = base_url().'superadmin/mls_import/retrieve_listing_data1/'.$temp_start_date.$temp_start_date1.$starttime.'/'.$temp_end_date.$temp_end_date1.$endtime.'/'.$row_prop_type;
    ...
    */
    function import_member_map()
    {
        ...
        if(!empty($member_table_name))
        {
            ...
            //$table_name='',$getfields='', $match_values = '', $condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$sort='',$where_clause='',$db_name='',$totalrows=''
            //$match = array('id'=>4483);
            $mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$member_table_name,'','','','','','');    
            //echo $this->db->last_query();    
            //pr($mapping_table_data);exit;
            $i = 0;
            $j = 0;
    ...
    */
    function import_area_community_map()
    {
        ...
        if(!empty($area_community_table_name))
        {
            ...
            //pr($mapping_data);
            $mls_default_id   = array();
            $mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$area_community_table_name,'','','','','');    
    ...
    */
    function import_amenity_map()
    {
        ...
        if(!empty($amenity_table_name))
        {
            ...
            //pr($mapping_data);
            $mls_default_id   = array();
            $mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$amenity_table_name,'','','','','');    
            //pr($mapping_table_data);exit;
            //$mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$school_table_name);    
    ...
    */
    function import_prop_history_map()
    {
        ...
        if(!empty($property_history_table_name))
        {
            ...
            //pr($mapping_data);
            $mls_default_id   = array();
            $mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$property_history_table_name,'','','','','');    
            //pr($mapping_table_data);exit;
            //$mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$school_table_name);    
    ...
    */
    function import_property_map()
    {
        ...
        if(!empty($main_table))
        {
            ...
            $mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$main_table,'','','','','',25000);    
            //echo $this->db->last_query();
            //pr($mapping_table_data);exit;
            //$mapping_table_data   = $this->obj->select_records_common($this->mls_master_db.'.'.$school_table_name);    
