...
*/
    ...
    function import_member_map()
    {
        ...
        if(!empty($member_table_name))
        {
            ...
            else
            {
                // Get maximum update date
                $field = array('id','max(modified_date) as modified_date');
                $res_data=$this->mls_model->select_records6($field,'','','=','','1','0');
                //pr($res_data);exit;
                //echo $this->db->last_query();
                $begin_date = $res_data[0]['modified_date'];
                $curr_date  = date('Y-m-d h:i:s');
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($usdata[$i]) == count($row))
                            {
                              if(count($usdata) >= 100)
                                {   
                                    echo 'update100<br>';
                                    //$this->mls_model->update_batch_record($usdata,$member_table_name,'id',$this->mls_staging_db);
                                    //$this->mls_model->update_batch_record($usdata,$member_table_name,'id',$this->mls_master_db);
    ...
    */
    function import_property_map()
    {
        ...
        $our_database_table   = 'mls_property_list_master';
        $image_database_table = 'mls_property_image';
            ...
            if(!empty($mapping_table_data))
            {
                foreach($mapping_table_data as $odata)
                {
                    ...
                    if(!empty($ln_numner))
                    {
                        if(!empty($image_table_name))
                        {
                            ...
                            if(!empty($mapping_data_id))
                            {
                                ...
                                $query ='SELECT * FROM '.substr($image_table_name, 4).' WHERE '.$ln_numner[1] .' = '.$odata[$mapp_id[1]];
                                $query = $this->db1->query($query);
                    ...
                    if(!empty($odata) && count($odata) > 0)              
                    {
                        ...
                        else if(!empty($insert_mapdata[$i]['ST']) && ($insert_mapdata[$i]['ST'] == 'Pending' || $insert_mapdata[$i]['ST'] == 'In Due Diligence'))
                        ...
                        if(empty($res)) //Insert data
                        {
                            ...
                            /*if(!empty($image_data))
                            {
                                ...
                                foreach ($image_data as $image) 
                                {
                                    //Get mapping data
                                    $field = array('id','mls_id','mls_master_field_id','mls_master_field','mls_field','mls_field_table','mls_transection_field');
                                    $match = array('mls_id'=>$mls_id,'table_id'=>8);
                                    $mapping_data  = $this->obj->select_records_common('mls_type_of_mls_mapping_trans',$field,$match,'','=','','','','','');                     
                                    if(!empty($mapping_data))
                                    {
                                        foreach($mapping_data as $row)
                                        {
                                            ...
                                            $mls_map_table      = $row['mls_field_table'];
                                            $mls_map_tran_field = explode('.',$row['mls_transection_field']);
                                            $insert_imgdata[$n][$mls_field]='';

                                            $insert_imgdata[$n][$mls_field] =  $image[$mls_map_field[1]];
                                        }
                                        //Get image url
                                        $match = array('mls_id' => $mls_id);
                                        $mls_details             = $this->obj->select_records_common('mls_type_of_mls_master','',$match,'','=');
                                        $iurl = !empty($mls_details[0]['mls_image_url'])?$mls_details[0]['mls_image_url']:'';
                                        $insert_imgdata[$n]['image_url']= $iurl.'/IMG-'.$odata[$mapp_id[1]].'_'.$t.'.jpg';
                                        $insert_imgdata[$n]['image_no'] = $t;

                                    }   
                                    $n++;
                                    $t++;
                                }
                                $this->mls_model->insert_batch_common($insert_imgdata,$image_database_table);
                                pr($insert_imgdata);exit;
                            }*/
                        }
                        else //Update data
                        {
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($usdata[$i]) == count($row))
                            {
                              if(count($usdata) >= 100)
                                {   
                                    echo 'update100<br>';
                                    $this->mls_model->update_batch_record($usdata,$main_table,'id',$this->mls_staging_db);
                                    $this->mls_model->update_batch_record($usdata,$main_table,'id',$this->mls_master_db);
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($udata[$i]) == count($row))
                            {
                              if(count($udata) >= 100)
                                {   
                                    echo 'u100230<br>';
                                    $this->mls_model->update_batch_record($udata,$our_database_table,'property_id');
                                    $i = 0;
                                    unset($udata);
                                }  
                            }
