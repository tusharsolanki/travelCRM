    ...
    */
    ...
    public function retrieve_amenity_data()
    {
                ...
                if(!empty($amenity))
                {
                    ...
                    //{
                        ...
                        foreach($property_data as $row)
                        {
                            ...
                            $fields=array('id');
                            $match=array('property_type'=>!empty($type['name'])?$type['name']:'','code'=>!empty($row['Code'])?$row['Code']:'','value_code'=>!empty($row['Values']['Code'])?$row['Values']['Code']:'');
                            $res1=$this->mls_model->select_records1('',$match,'','=','','','','','','');
        ...
        {    
            ...
            $url = base_url().'superadmin/mls_update/retrieve_amenity_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    */
    public function retrieve_listing_data()
    {
        ...
        else
        {
            ...
            $res_data=$this->mls_model->select_records3($field,$match,'','=','','1','0');
            //pr($res_data);
            $begin_date = str_replace(" ","T",$res_data[0]['UD']);
            $curr_date  = str_replace(" ","T",date('Y-m-d h:i:s'));
        ...
        else
        {
            ...
            $last_data1 = array( 
                            'mls_id'            => $mls_id,
                            'table_type'        => '8',
                            'last_updated_date' => $begin_date,
                            'modified_date'     => date('Y-m-d H:i:s')
                            );
            $last_data1['created_date']  = date('Y-m-d H:i:s');
            $this->mls_model->insert_common('mls_last_updated_date_data',$last_data1);
        }
        //echo $begin_date;
        //echo $curr_date;exit;
        //$begin_date =    $this->uri->segment(5);
        //$curr_date  =    $this->uri->segment(6);
        //$begin_date = '2015-07-01T00:00:00';
        //$curr_date  = '2015-07-01T00:10:00';
        ...
        if(!empty($propty_type))
        {
            foreach($propty_type as $type)  
            {
                ...
                if(!empty($mls_data))
                {
                    ...
                    $field_data_cron = array(
                                'cron_name'=>'retrieve_listing_data_strat',
                                'p_type'=>$type['name'],
                    ...
                    foreach($mls_data as $key=>$value)
                    {
                        ...
                        $n = 0;
                        $k = 0;
                        if(!empty($property_data) && empty($property_data[0]))
                        {
                            //echo "1<br>";
                            $mydata = array();
                            $mydata[0] = $property_data;
                            $property_data = $mydata;
                        }
                        foreach($property_data as $row)
                        {       
                            if(!empty($row['LN']))
                            {
                                ...
                                $match=array('LN'=>!empty($row['LN'])?$row['LN']:'','mls_id'=>!empty($mls_id)?$mls_id:''); 
                                $resl=$this->mls_model->select_records3('',$match,'','=','','','','','','','','','','');
                                //echo $this->db->last_query();
                                //pr($resl);exit;
                                ...
                                foreach($mapping_data as $row_field)
                                {
                                    ...
                                    else
                                    {
                                            ...
                                            { 
                                                ...
                                                if(!empty($mlsmap_fields))
                                                {
                                                    ...
                                                    if(in_array($mls_map_field[1],$msfield))
                                                    {
                                                        ...
                                                        if(!empty($retivedata))
                                                        {
                                                            $insert_mapdata[$n][$mls_field] =  $retivedata[0][$mls_map_tran_field[1]];    
                                                        }
                                                    }
                                                }
                                                else
                                        ...
                                        {
                                            //Case : value -> value
                                            if($mls_map_field[1] == 'LONGI')
                                            {
                                                $insert_mapdata[$n][$mls_field] =  !empty($row['LONG'])?$row['LONG']:'';    
                                            }
                                            else if($mls_map_field[1] == 'OUT1')
                                            {
                                                $insert_mapdata[$n][$mls_field] =  !empty($row['OUT'])?$row['OUT']:'';
                                            }
                                            else if($mls_map_field[1] == 'ASCC')
                                            {
                                                $insert_mapdata[$n][$mls_field] =  !empty($row['ASC'])?$row['ASC']:'';
                                            }
                                            else
                                            {
                                ...
                                if($mls_id == '2')
                                {
                                    $insert_mapdata[$n]['full_address'] = !empty($row['field_StreetNoIDX'])?$row['field_StreetNoIDX'].',':''.!empty($row['field_StreetNameIDX'])?$row['field_StreetNameIDX'].',':'';
                                    $insert_mapdata[$n]['full_address'] = substr($insert_mapdata[$n]['full_address'],0,-1);
                                    $insert_mapdata[$n]['HOD'] = !empty($row['field_HOAAnnualDues'])?$row['field_HOAAnnualDues'].',':''.!empty($row['field_HOAAnnualDuesPeriod'])?$row['field_HOAAnnualDuesPeriod'].',':'';
                                ...
                                else //Update data
                                {
                                    ...
                                    $insert_mapdata[$n]['modified_date']      = date('Y-m-d h:i:s');
                ...
                //exit;
                    //pr($insert_mapdata);exit;
                    //pr($data);exit;
                    echo 'Insert -'.count($data);
                    echo ' Update -'.count($cdata);
                    echo '<br>';
                    echo 'Insert LW-'.count($insert_mapdata);
                    echo ' Update LW-'.count($sdata);
                    /*echo 'Insert -'.!empty(count($data))?count($data):'N/A';
                    echo ' Update -'.!empty(count($cdata))?count($cdata):'N/A';
                    echo '<br>';
                    echo 'Insert LW-'.!empty(count($insert_mapdata))?count($insert_mapdata):'N/A';
                    echo ' Update LW-'.!empty(count($sdata))?count($sdata):'N/A';*/
                    ...
                    if(!empty($data)  && count($data) > 0)
                    {
                        ...
                        foreach ($data as $row) 
                        {
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                {
                                    if(isset($idata[$i]['modified_date']))
                                        unset($idata[$i]['modified_date']);
                                    if(count($idata) >= 100)
                                    {
                    ...
                    if(!empty($insert_mapdata)  && count($insert_mapdata) > 0)
                    {
                        ...
                        foreach ($insert_mapdata as $row) 
                        {
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                {
                                    if(isset($insdata[$i]['modified_date']))
                                        unset($insdata[$i]['modified_date']);
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/retrieve_listing_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    public function retrieve_listing_data_indi()
    {
        //print ini_get('max_execution_time');
        ini_set('display_errors',1);
        error_reporting(E_ALL);
        ...
        if(!empty($propty_type))
        {
            foreach($propty_type as $type)  
            {
                ...
                if(!empty($mls_data))
                {
                    ...
                    //pr($insert_mapdata);exit;
                    echo '<br>Type -'.$type['name'];
                    echo '<br>';
                    echo $insstamas = 'Insert -'.!empty($data)?count($data):'0';
                    echo $updstamas = ' Update -'.!empty($cdata)?count($cdata):'0';
                    echo '<br>';
                    echo $insLW = 'Insert LW-'.!empty($insert_mapdata)?count($insert_mapdata):'0';
                    echo $updLW = ' Update LW-'.!empty($sdata)?count($sdata):'0';
                    /*echo 'Insert -'.!empty(count($data))?count($data):'N/A';
                    echo ' Update -'.!empty(count($cdata))?count($cdata):'N/A';
                    echo '<br>';
                    echo 'Insert LW-'.!empty(count($insert_mapdata))?count($insert_mapdata):'N/A';
                        ...
                        foreach ($insert_mapdata as $row) 
                        {
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($insdata[$i]) == count($row))
                                {
                                    ...
                                    /*$this->mls_model->insert_record_mapping($insdata[$i]);
                                    echo $this->db->last_query();
                                    exit;*/
                                }
                            }
    ...
    */
    ...
    public function retrieve_area_community_data()
    {
        ...
        if(!empty($mls_data))
        {
            foreach($mls_data as $key=>$value)
            {
                ...
                foreach($property_data as $row)
                {
                    ...
                    $fields=array('id');
                    $match=array('area'=>!empty($row['Area'])?$row['Area']:'','community'=>!empty($row['Community'])?$row['Community']:'');
                    $res2=$this->mls_model->select_records2('',$match,'','=','','','','','','');
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/retrieve_area_community_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    public function retrieve_image_data_indi()
    {
        ini_set('display_errors', '1');
        error_reporting(E_ALL);
        ...
        if(!empty($img_counter_data) && count($img_counter_data) > 0)
        {
            ...
            $insert_cron_id_main = $this->mls_model->insert_cron_test($field_data_cron);
            // Get maximum update date
            $field = array('ID','LN','modified_date','UD','PIC');
           
            //$getfields='', $match_values = '', $condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$sort='',$where_clause='',$db_name='',$totalrows='',$wherestring=''
            $match = array('mls_id' => $mls_id);
            $res=$this->mls_model->select_records3($field,$match,'','=','',$num,$offset,'','','');
            //echo $this->db->last_query();
            //pr($res);exit;
                       ...
                       if(!empty($mls_data1))
                       {
                            ...
                            if(!empty($mls_data['Images']['Image'][0]))
                            {
                                for($i=0;$i<count($mls_data['Images']['Image']);$i++)
                                {
                                    ...
                                    if($img_order == 0)
                                        {$name = $listingID.".jpg";}
                                    else{$name = (strlen($img_order) == 1)?$listingID . "_0" .$img_order. ".jpg":$listingID . "_" .$img_order. ".jpg";}
                                    ...
                                    if(!empty($cdnUrl1) && !empty($cdnUrl2))
                                    {
                                        ...
                                        if(empty($res))
                                        {
                                            ...
                                            $this->obj->insert_record4($data,$this->mls_master_db);
                                        }
                                        else
                                        {
                                            $this->obj->update_record4($data,$this->mls_staging_db);
                                            $this->obj->update_record4($data,$this->mls_master_db);
                                        }
                                        
                                        $fields=array('id');
                                        $match=array('Image_id'=>!empty($row1[$i]['ImageId'])?$row1[$i]['ImageId']:'','mls_id'=>$mls_id);
                                        $res1=$this->mls_model->select_records4('',$match,'','=','','','','','','');
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/retrieve_image_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    public function retrieve_image_data()
    {
        ...
        $field_data_cron = array(
            'cron_name' => 'update_retrieve_image_data_start',
            'created_ip' =>$created_ip,
            'created_date'=>date('Y-m-d H:i:s')
            );
        $insert_cron_id_main = $this->mls_model->insert_cron_test($field_data_cron);
        ...
        if(!empty($last_updated_date[0]['last_updated_date']))
        {
            ...
            $curr_date  = date('Y-m-d h:i:s', strtotime('-5 minutes'));
             //Insert last updated date
        ...
        else
        {
            ...
            $res_data=$this->mls_model->select_records3($field,$match,'','=','','1','0');
            //pr($res_data);exit;
            $begin_date = $res_data[0]['UD'];
            $curr_date  = date('Y-m-d H:i:s', strtotime('-5 minutes'));
            
        }
        //$begin_date ='2015-07-01T00:00:00';
        //$curr_date  = '2015-07-01T02:00:00';
        $last_data = array( 
                                'mls_id'            => $mls_id,
                                'table_type'        => '8',
                                'last_updated_date' => $curr_date,
        ...
        // Get maximum update date
        $field = array('ID','LN','modified_date','UD','PIC');
        $wherestring = '(UD >="'.$begin_date.'" AND UD <="'.$curr_date.'")';
        //$getfields='', $match_values = '', $condition ='', $compare_type = '', $count = '',$num = '',$offset='',$orderby='',$sort='',$where_clause='',$db_name='',$totalrows='',$wherestring=''
        $match = array('mls_id' => $mls_id);
        $res=$this->mls_model->select_records3($field,$match,'','=','','','','','','','','',$wherestring);
        //echo $this->db->last_query();
        //pr($res);exit;
                        ...
                        if(!empty($mls_data['Images']['Image'][0]))
                        {
                            for($i=0;$i<count($mls_data['Images']['Image']);$i++)
                            {
                                ...
                                if(!empty($cdnUrl1) && !empty($cdnUrl2))
                                {
                                    ...
                                    $fields=array('id');
                                    $match=array('Image_id'=>!empty($row1[$i]['ImageId'])?$row1[$i]['ImageId']:'');
                                    $res=$this->mls_model->select_records4('',$match,'','=','','','','','','');
    ...
    */
    ...
    public function retrieve_listing_history_data()
    {
        ...
        else
        {
            ...
            $field = array('ID','max(change_date) as change_date');
            $match = array('mls_id' => $mls_id);
            $res_data=$this->mls_model->select_records5($field,$match,'','=','','1','0');
            //echo $this->db->last_query();
        ...
        if(!empty($propty_type))
        {
            foreach($propty_type as $type)  
            {
                ...
                if(!empty($mls_data))
                {
                    foreach($mls_data as $key=>$value)
                    {
                        ...
                        foreach($property_data as $row)
                        {
                            ...
                            $res=$this->mls_model->select_records5('',$match,'','=','','','','','','');
                            if(empty($res))
                            {
                                $data[$i]['created_date']       = date('Y-m-d h:i:s');
                                $adata[$i] = $data[$i];
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($idata[$i]) == count($row))
                            {
                                if(count($idata) >= 100)
                                {
                                    ...
                                    //$this->mls_model->insert_record5($idata);
                                    $i = 0; 
                                    unset($idata);
                                }
                            }
                            ...
                            if(count($udata[$i]) == count($row))
                            {
                              if(count($udata) >= 100)
                                {   
                                    ...
                                    //$this->mls_model->update_record5($udata);
                                    $i = 0;
                                    unset($udata);
                                }  
                            }
                ...
                //Mapping database
                //Insert more than 100 for mapping DB
                if(!empty($adata)  && count($adata) > 0)
                {
                    ...
                    foreach ($adata as $row) 
                    {
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($aidata[$i]) == count($row))
                            {
                                if(count($aidata) >= 100)
                                {
                                    $this->mls_model->insert_record5($aidata);
                                    $i = 0; 
                                    unset($aidata);
                                }
                            }
                ...
                //Update more than 100 for mapping DB
                if(!empty($acdata) && count($acdata) > 0)
                {
                    $i = 0;
                    foreach ($acdata as $row) 
                    {
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($audata[$i]) == count($row))
                            {
                              if(count($audata) >= 100)
                                {   
                                    $this->mls_model->update_record5($audata);
                                    $i = 0;
                                    unset($audata);
                                }  
                            }
                ...
                //For mapping DB
                if(!empty($aidata) && count($aidata) > 0)
                {
                    $this->mls_model->insert_record5($aidata);
                    unset($aidata);   
                }
                //For mapping DB
                if(!empty($audata) && count($audata) > 0)
                {
                   $this->mls_model->update_record5($audata);
                   unset($audata);  
                }
       ...
       // $cron_time = 'cron_time';
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/retrieve_listing_history_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    */
    ...
    public function retrieve_member_data()
    {
                ...
                foreach($property_data as $row)
                {
                    ...
                    if(empty($res)) 
                    {
                            ...
                            /*if(count($data) >= 100)
                            {
                                ...
                                $this->mls_model->insert_record6($data);
                                $i = 0;
                                unset($data);
                            }*/
                            $i++;
                    ...
                    else
                    {
                            ...
                            /*if(count($cdata) >= 100)
                            {
                                    ...
                                    $this->mls_model->update_record6($cdata);
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($idata[$i]) == count($row))
                            {
                                if(count($idata) >= 100)
                                {
                                    ...
                                    //$this->mls_model->insert_record6($idata);
                                    $i = 0; 
                                    unset($idata);
                                }
                            }
                            ...
                            if(count($udata[$i]) == count($row))
                            {
                              if(count($udata) >= 100)
                                {   
                                    ...
                                    //$this->mls_model->update_record6($udata);
                                    $i = 0;
                                    unset($udata);
                                }  
                            }
                    ...
                    foreach ($adata as $row) 
                    {
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($aidata[$i]) == count($row))
                            {
                                if(count($aidata) >= 100)
                                {
                                    $this->mls_model->insert_record6($aidata);
                                    $i = 0; 
                                    unset($aidata);
                                }
                            }
                    ...
                    foreach ($acdata as $row) 
                    {
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($audata[$i]) == count($row))
                            {
                              if(count($audata) >= 100)
                                {   
                                    $this->mls_model->update_record6($audata);
                                    $i = 0;
                                    unset($audata);
                                }  
                            }
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/retrieve_member_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    */
    ...
    public function retrieve_office_data()
    {
                ...
                foreach($property_data as $row)
                {
                    ...
                    if(empty($res))
                    {
                            ...
                            /*if(count($data) >= 100)
                            {
                                ...
                                $this->mls_model->insert_record7($data);
                                $i = 0;
                                unset($data);
                            }*/
                            $i++;
                    ...
                    else
                    {
                            ...
                            /*if(count($cdata) >= 100)
                            {
                                    ...
                                    $this->mls_model->update_record7($cdata);
                                    //$this->mls_model->update_record1($cdata,$this->mls_master_db);
                                    //pr($cdata); exit;
                                    $j = 0;
                                    unset($cdata);
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($idata[$i]) == count($row))
                            {
                                if(count($idata) >= 100)
                                {
                                    ...
                                    //$this->mls_model->insert_record7($idata);
                                    $i = 0; 
                                    unset($idata);
                                }
                            }
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($aidata[$i]) == count($row))
                            {
                                if(count($aidata) >= 100)
                                {
                                    ...
                                    //$this->mls_model->insert_record7($idata,$this->mls_staging_db);
                                    //$this->mls_model->insert_record7($idata,$this->mls_master_db);
                                    $this->mls_model->insert_record7($aidata);
                                    $i = 0; 
                                    unset($aidata);
                                }
                            }
                            ...
                            if(count($udata[$i]) == count($row))
                            {
                              if(count($udata) >= 100)
                                {   
                                    ...
                                    //$this->mls_model->update_record7($udata);
                                    $i = 0;
                                    unset($udata);
                                }  
                            }
                            ...
                            if(count($audata[$i]) == count($row))
                            {
                              if(count($audata) >= 100)
                                {   
                                    ...
                                    //$this->mls_model->update_record7($udata,$this->mls_staging_db);
                                    //$this->mls_model->update_record7($udata,$this->mls_master_db);
                                    $this->mls_model->update_record7($audata);
                                    $i = 0;
                                    unset($audata);
                                }  
                            }
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/retrieve_office_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    */
    ...
    public function retrieve_school_data()
    {
                ...
                foreach($property_data as $row)
                {
                    ...
                    if(empty($res))
                    {
                            ...
                            /*if(count($data) >= 100)
                            {
                                ...
                                $this->mls_model->insert_record8($data);
                                $i = 0;
                                unset($data);
                            }*/
                            $i++;
                    ...
                    else
                    {
                            ...
                            /*if(count($cdata) >= 100)
                            {
                                    ...
                                    $this->mls_model->update_record8($cdata);
                                    //$this->mls_model->update_record1($cdata,$this->mls_master_db);
                                    //pr($cdata); exit;
                                    $j = 0;
                                    unset($cdata);
                        ...
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($idata[$i]) == count($row))
                            {
                                if(count($idata) >= 100)
                                {
                                    ...
                                    //$this->mls_model->insert_record8($idata);
                                    $i = 0; 
                                    unset($idata);
                                }
                            }
                            ...
                            if(count($udata[$i]) == count($row))
                            {
                              if(count($udata) >= 100)
                                {   
                                    ...
                                    //$this->mls_model->update_record8($udata);
                                    $i = 0;
                                    unset($udata);
                                }  
                            }
                    ...
                    foreach ($adata as $row) 
                    {
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($aidata[$i]) == count($row))
                            {
                                if(count($aidata) >= 100)
                                {
                                    $this->mls_model->insert_record8($aidata);
                                    $i = 0; 
                                    unset($aidata);
                                }
                            }
                    ...
                    foreach ($acdata as $row) 
                    {
                        foreach ($row as $key=>$value) 
                        {
                            ...
                            if(count($audata[$i]) == count($row))
                            {
                              if(count($audata) >= 100)
                                {   
                                    $this->mls_model->update_record8($audata);
                                    $i = 0;
                                    unset($audata);
                                }  
                            }
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/retrieve_school_data/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    function import_office_map()
    {
        ...
        if(!empty($office_table_name))
        {
            ...
            else
            {
                ...
                $match = array('mls_id' => $mls_id);
                $res_data=$this->mls_model->select_records7($field,$match,'','=','','1','0');
                //pr($res_data);exit;
                //echo $this->db->last_query();
            ...
            if(!empty($result))
            {
                ...
                //$begin_date ='2015-06-27 07:52:31';
                ...
                */
                ...
                if(!empty($mapping_table_data))
                {
                    ...
                    echo 'I'.count($ins_master);
                    echo 'u'.count($smdata);
                    echo '<br>';
                    echo 'I-lw'.count($insert_mapdata);
                    echo 'u-lw'.count($cdata);
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/import_office_map/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    function import_member_map()
    {
        ...
        if(!empty($member_table_name))
        {
            ...
            else
            {
                ...
                $field = array('id','max(modified_date) as modified_date');
                $match = array('mls_id' => $mls_id);
                $res_data=$this->mls_model->select_records6($field,$match,'','=','','1','0');
                //pr($res_data);exit;
                //echo $this->db->last_query();
                $begin_date = $res_data[0]['modified_date'];
                $curr_date  = date('Y-m-d h:i:s');
            ...
            if(!empty($result))
            {
                ...
                $table_name=substr($member_table_name, 4);
                $this->db1 = $this->load->database($db['second'],true);
                //$begin_date ='2015-06-06 00:00:00';
                //$curr_date='2015-07-01 00:00:00';
                $query ='SELECT * FROM '.$table_name.' WHERE rc_ModifiedTime >="'.$begin_date.'" AND rc_ModifiedTime <="'.$curr_date.'"';
                $query = $this->db1->query($query);
                $mapping_table_data = $query->result_array();
                //pr($mapping_table_data);exit;
                ...
                if(!empty($mapping_table_data))
                {
                    ...
                    {
                        //Update data on staging and master database
                        //$match = array($mapp_id[1]=> $odata[$mapp_id[1]]);
                         $match = array('id'=> $odata['id']);
                        $staging_data   = $this->obj->select_records_common($this->mls_staging_db.'.'.$member_table_name,'',$match,'','=');    
                        //pr($staging_data);exit;
                        //Get data fields wise
                        foreach($fdata as $fld)
                        {
                    ...
                    //pr($ins_master);
                    echo 'Iinsert - '.count($ins_master);
                    echo 'Update - '.count($smdata);
                    echo '<br>';
                    echo 'I-lw '.count($insert_mapdata);
                    echo 'u-lw '.count($cdata);
                    //exit;
                        ...
                        foreach ($ins_master as $row) 
                        {
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($insdata[$i]) == count($row))
                                {
                                    ...
                                    {
                                        echo 'insert100<br>';
                                        $this->mls_model->insert_batch_common($insdata,$member_table_name,$this->mls_staging_db);
                                        $this->mls_model->insert_batch_common($insdata,$member_table_name,$this->mls_master_db);
                                        $i = 0; 
                                        unset($insdata);
                                    }
                                }
                            ...
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($usdata[$i]) == count($row))
                                {
                                  if(count($usdata) >= 100)
                                    {   
                                        echo 'update100<br>';
                                        $this->mls_model->update_batch_record($usdata,$member_table_name,$mapp_id[1],$this->mls_staging_db);
                                        $this->mls_model->update_batch_record($usdata,$member_table_name,$mapp_id[1],$this->mls_master_db);
                            ...
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($udata[$i]) == count($row))
                                {
                                  if(count($udata) >= 100)
                                    {   
                                        echo 'u100230<br>';
                                        $this->mls_model->update_batch_record($udata,$our_database_table,'member_mls_id');
                                        $i = 0;
                                        unset($udata);
                                    }  
                                }
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/import_member_map/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    function import_school_map()
    {
        ...
        if(!empty($school_table_name))
        {
            ...
            else
            {
                ...
                $field = array('id','max(modified_date) as modified_date');
                $match = array('mls_id' => $mls_id);
                $res_data=$this->mls_model->select_records8($field,$match,'','=','','1','0');
                //pr($res_data);exit;
                //echo $this->db->last_query();
                $begin_date = $res_data[0]['modified_date'];
                $curr_date  = date('Y-m-d h:i:s');
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/import_school_map/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    function import_area_community_map()
    {
        ...
        if(!empty($area_community_table_name))
        {
            ...
            else
            {
                ...
                $field = array('id','max(modified_date) as modified_date');
                $match = array('mls_id' => $mls_id);
                $res_data=$this->mls_model->select_records2($field,$match,'','=','','1','0');
                //pr($res_data);exit;
                //echo $this->db->last_query();
                $begin_date = $res_data[0]['modified_date'];
                $curr_date  = date('Y-m-d h:i:s');
            ...
            if(!empty($result))
            {
                ...
                $table_name=substr($area_community_table_name, 4);
                $this->db1 = $this->load->database($db['second'],true);
                //$begin_date ='2015-05-21 07:52:31';
                //$curr_date='2015-05-24 07:52:31';
                $query ='SELECT * FROM '.$table_name.' WHERE rc_ModifiedTime >="'.$begin_date.'" AND rc_ModifiedTime <="'.$curr_date.'"';
                ...
                if(!empty($mapping_table_data))
                {
                    foreach($mapping_table_data as $odata)
                    {
                        //Update data on staging and master database
                        $match = array('id'=> $odata['id']);
                        $staging_data   = $this->obj->select_records_common($this->mls_staging_db.'.'.$area_community_table_name,'',$match,'','=');    
                        //pr($staging_data);exit;
                        //Get data fields wise
                        foreach($fdata as $fld)
                        {
                        ...
                        if(empty($staging_data)) //Insert to master and staging db
                        {
                            /*if(count($insert_master) >= 100)
                            {
                                $this->mls_model->insert_batch_common($insert_master,$area_community_table_name,$this->mls_staging_db);
                                $this->mls_model->insert_batch_common($insert_master,$area_community_table_name,$this->mls_master_db);
                                $n = 0;
                                unset($insert_master);
                            }*/
                            $ins_master[$n]=$insert_master[$n];
                        ...
                        else
                        {
                            ...
                            /*if(count($smdata) >= 100)
                            {
                                $this->mls_model->update_batch_record($smdata,$area_community_table_name,'id',$this->mls_staging_db);
                                $this->mls_model->update_batch_record($smdata,$area_community_table_name,'id',$this->mls_master_db);
                        ...
                        foreach ($ins_master as $row) 
                        {
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($insdata[$i]) == count($row))
                                {
                                    ...
                                    {
                                        echo 'insert100<br>';
                                        $this->mls_model->insert_batch_common($insdata,$area_community_table_name,$this->mls_staging_db);
                                        $this->mls_model->insert_batch_common($insdata,$area_community_table_name,$this->mls_master_db);
                                        $i = 0; 
                                        unset($insdata);
                                    }
                                }
                            ...
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($usdata[$i]) == count($row))
                                {
                                  if(count($usdata) >= 100)
                                    {   
                                        echo 'update100<br>';
                                        $this->mls_model->update_batch_record($usdata,$area_community_table_name,'id',$this->mls_staging_db);
                                        $this->mls_model->update_batch_record($usdata,$area_community_table_name,'id',$this->mls_master_db);
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/import_area_community_map/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    function import_amenity_map()
    {
        ...
        if(!empty($amenity_table_name))
        {
            ...
            else
            {
                ...
                $field = array('id','max(modified_date) as modified_date');
                $match = array('mls_id' => $mls_id);
                $res_data=$this->mls_model->select_records1($field,$match,'','=','','1','0');
                //pr($res_data);exit;
                //echo $this->db->last_query();
                $begin_date = $res_data[0]['modified_date'];
                $curr_date  = date('Y-m-d h:i:s');
            ...
            if(!empty($result))
            {
                ...
                $table_name=substr($amenity_table_name, 4);
                $this->db1 = $this->load->database($db['second'],true);
                //$begin_date ='2015-05-21 07:52:31';
                //$curr_date='2015-05-24 07:52:31';
                $query ='SELECT * FROM '.$table_name.' WHERE rc_ModifiedTime >="'.$begin_date.'" AND rc_ModifiedTime <="'.$curr_date.'"';
                ...
                if(!empty($mapping_table_data))
                {
                    foreach($mapping_table_data as $odata)
                    {
                        //Update data on staging and master database
                        $match = array('id'=> $odata['id']);
                        $staging_data   = $this->obj->select_records_common($this->mls_staging_db.'.'.$amenity_table_name,'',$match,'','=');    
                        //pr($staging_data);exit;
                        //Get data fields wise
                        foreach($fdata as $fld)
                        {
                        ...
                        if(empty($staging_data)) //Insert to master and staging db
                        {
                            /*if(count($insert_master) >= 100)
                            {
                                $this->mls_model->insert_batch_common($insert_master,$amenity_table_name,$this->mls_staging_db);
                                $this->mls_model->insert_batch_common($insert_master,$amenity_table_name,$this->mls_master_db);
                                $n = 0;
                                unset($insert_master);
                            }*/
                            $ins_master[$n]=$insert_master[$n];
                        ...
                        else
                        {
                            ...
                            /*if(count($smdata) >= 100)
                            {
                                $this->mls_model->update_batch_record($smdata,$amenity_table_name,'id',$this->mls_staging_db);
                                $this->mls_model->update_batch_record($smdata,$amenity_table_name,'id',$this->mls_master_db);
                        ...
                        foreach ($ins_master as $row) 
                        {
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($insdata[$i]) == count($row))
                                {
                                    ...
                                    {
                                        echo 'insert100<br>';
                                        //$this->mls_model->insert_batch_common($insdata,$amenity_table_name,$this->mls_staging_db);
                                        //$this->mls_model->insert_batch_common($insdata,$amenity_table_name,$this->mls_master_db);
                                        $i = 0; 
                                        unset($insdata);
                                    }
                                }
                            ...
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($usdata[$i]) == count($row))
                                {
                                  if(count($usdata) >= 100)
                                    {   
                                        echo 'update100<br>';
                                        //$this->mls_model->update_batch_record($usdata,$amenity_table_name,'id',$this->mls_staging_db);
                                        //$this->mls_model->update_batch_record($usdata,$amenity_table_name,'id',$this->mls_master_db);
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/import_amenity_map/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    function import_prop_history_map()
    {
        ...
        if(!empty($property_history_table_name))
        {
            ...
            if(!empty($result))
            {
                ...
                $table_name=substr($property_history_table_name, 4);
                $this->db1 = $this->load->database($db['second'],true);
                //$begin_date ='2015-05-21 07:52:31';
                //$curr_date='2015-05-24 07:52:31';
                $query ='SELECT * FROM '.$table_name.' WHERE rc_ModifiedTime >="'.$begin_date.'" AND rc_ModifiedTime <="'.$curr_date.'"';
                ...
                if(!empty($mapping_table_data))
                {
                    foreach($mapping_table_data as $odata)
                    {
                        //Update data on staging and master database
                        $match = array('id'=> $odata['id']);
                        $staging_data   = $this->obj->select_records_common($this->mls_staging_db.'.'.$property_history_table_name,'',$match,'','=');    
                        //pr($staging_data);exit;
                        //Get data fields wise
                        foreach($fdata as $fld)
                        {
                        ...
                        if(empty($staging_data)) //Insert to master and staging db
                        {
                            /*if(count($insert_master) >= 100)
                            {
                                $this->mls_model->insert_batch_common($insert_master,$property_history_table_name,$this->mls_staging_db);
                                $this->mls_model->insert_batch_common($insert_master,$property_history_table_name,$this->mls_master_db);
                                $n = 0;
                                unset($insert_master);
                            }*/
                            $ins_master[$n]=$insert_master[$n];
                        ...
                        else
                        {
                            ...
                            /*if(count($smdata) >= 100)
                            {
                                $this->mls_model->update_batch_record($smdata,$property_history_table_name,'id',$this->mls_staging_db);
                                $this->mls_model->update_batch_record($smdata,$property_history_table_name,'id',$this->mls_master_db);
                        ...
                        foreach ($ins_master as $row) 
                        {
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($insdata[$i]) == count($row))
                                {
                                    ...
                                    {
                                        echo 'insert100<br>';
                                        $this->mls_model->insert_batch_common($insdata,$property_history_table_name,$this->mls_staging_db);
                                        $this->mls_model->insert_batch_common($insdata,$property_history_table_name,$this->mls_master_db);
                                        $i = 0; 
                                        unset($insdata);
                                    }
                                }
                            ...
                            foreach ($row as $key=>$value) 
                            {
                                ...
                                if(count($usdata[$i]) == count($row))
                                {
                                  if(count($usdata) >= 100)
                                    {   
                                        echo 'update100<br>';
                                        $this->mls_model->update_batch_record($usdata,$property_history_table_name,'id',$this->mls_staging_db);
                                        $this->mls_model->update_batch_record($usdata,$property_history_table_name,'id',$this->mls_master_db);
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/import_prop_history_map/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
    ...
    function import_image_map()
    {
        ...
        if(!empty($image_table_name))
        {
            ...
            else
            {
                ...
                $field = array('id','max(modified_date) as modified_date');
                $match = array('mls_id' => $mls_id);
                $res_data=$this->mls_model->select_records4($field,$match,'','=','','1','0');
                
                $begin_date = $res_data[0]['modified_date'];
                $curr_date  = date('Y-m-d h:i:s');
            }
            //$begin_date  = '2015-06-01 01:30:48';
            $match = array('mls_id'=>$mls_id);
            $result = $this->obj->select_records('',$match,'','=');
            ...
            if(!empty($result))
            {
                ...
                $table_name=substr($image_table_name, 4);
                $this->db1 = $this->load->database($db['second'],true);

                //$begin_date ='2015-06-05 07:52:31';
                //$curr_date='2015-05-24 07:52:31';
                $query ='SELECT * FROM '.$table_name.' WHERE rc_ModifiedTime >="'.$begin_date.'" AND rc_ModifiedTime <="'.$curr_date.'" order by '.$ln_numner[1].' asc';
                $query = $this->db1->query($query);
                $mapping_table_data = $query->result_array();
                //pr($mapping_table_data);exit;
                $first_mls_id = $mapping_table_data[0][$ln_numner[1]];
                ...
                if(!empty($mapping_table_data))
                {
                    foreach($mapping_table_data as $odata)
                    {
                        //Update data on staging and master database
                        $match = array('id'=> $odata['id']);
                        $staging_data   = $this->obj->select_records_common($this->mls_staging_db.'.'.$image_table_name,'',$match,'','=');    
                        //  echo $this->db->last_query();exit;
                        //pr($staging_data);exit;
                        //Get data fields wise
                        foreach($fdata as $fld)
                    ...
                    if(!empty($ins_master) && count($ins_master) > 0)
                    {
                        $this->mls_model->insert_batch_common($ins_master,$main_table,$this->mls_staging_db);
                        $this->mls_model->insert_batch_common($ins_master,$main_table,$this->mls_master_db);
                        unset($ins_master);   
                    }
                    // insert data when record less then 100
                    if(!empty($insert_mapdata) && count($insert_mapdata) > 0)
    ...
    */
    function import_property_map()
    {
        ...
        if(!empty($main_table))
        {
            ...
            */
            ...
            if(!empty($result))
            {
                ...
                $table_name=substr($main_table, 4);
                $this->db1 = $this->load->database($db['second'],true);
                //$begin_date ='2015-06-05 07:52:31';
                //$curr_date='2015-05-24 07:52:31';
                $query ='SELECT * FROM '.$table_name.' WHERE rc_ModifiedTime >="'.$begin_date.'" AND rc_ModifiedTime <="'.$curr_date.'"';
                ...
                { 
                    if(!empty($mapping_table_data))
                    {
                        foreach($mapping_table_data as $odata)
                        {
                            ...
                            //$match = array('id'=> 'id');
                            //$match = array('id'=> $odata['id']);
                            $staging_data   = $this->obj->select_records_common($this->mls_staging_db.'.'.$main_table,'',$match,'','=');    
                            //  echo $this->db->last_query();exit;
                            //pr($staging_data);exit;
                            //Get data fields wise
                            foreach($fdata as $fld)
                            ...
                            if(!empty($odata) && count($odata) > 0)              
                            {
                                ...
                                $HSN = '';$DRP = '';$STR = '';$SSUF = '';$DRS = '';$UNT = '';$CIT = '';$STA = '';$ZIP = '';
                                    ...
                                    if(!empty($res[0]['ST']) && ($res[0]['ST'] != $insert_mapdata[$i]['ST']))
                                    {$insert_mapdata[$i]['is_status_change'] = '1';}
                                    else
                                    {$insert_mapdata[$i]['is_status_change'] = '0';}
                                    $insert_mapdata[$i]['old_status'] = !empty($res[0]['ST'])?$res[0]['ST']:'';
                                //pr($insert_mapdata);exit;
                                if(empty($res)) //Insert data
                                {
                                    //pr($insert_mapdata);
                            ...
                            foreach ($ins_master as $row) 
                            {
                                foreach ($row as $key=>$value) 
                                {
                                    ...
                                    if(count($insdata[$i]) == count($row))
                                    {
                                        ...
                                        {
                                            echo 'insert100<br>';
                                            $this->mls_model->insert_batch_common($insdata,$main_table,$this->mls_staging_db);
                                            $this->mls_model->insert_batch_common($insdata,$main_table,$this->mls_master_db);
                                            $i = 0; 
                                            unset($insdata);
                                        }
                                    }
                                ...
                                foreach ($row as $key=>$value) 
                                {
                                    ...
                                    if(count($usdata[$i]) == count($row))
                                    {
                                      if(count($usdata) >= 100)
                                        {   
                                            echo 'update100<br>';
                                            $this->mls_model->update_batch_record($usdata,$main_table,$mapp_id[1],$this->mls_staging_db);
                                            $this->mls_model->update_batch_record($usdata,$main_table,$mapp_id[1],$this->mls_master_db);
        ...
        {    
            ...
            $output = shell_exec('crontab -l');
            //$cron_time=$minute.'-'.$hour.'-'.$day.'-'.$month;
            $url = base_url().'superadmin/mls_update/import_property_map/'.$mls_id.'/'.$cron_time;
            $minute  = $cron_field[0];
            $hour    = $cron_field[1];
            $day     = $cron_field[2];
            $month   = $cron_field[3];
