...
/*                            $data['searchtext'] = $searchsort_session['searchtext'];
		...
		$where1 = 'ipm.status = "1" AND by_superadmin != 2';
		if(!empty($searchtext))
		{
			//pr($searchtext);exit;
...
/*                            $data['searchtext'] = $searchsort_session['searchtext'];
		...
		if(!empty($searchtext))
		{
			//pr($searchtext);exit;
			$match=array('ipm.plan_name'=>$searchtext);
			$where1=array('ipm.status'=>$status_value);
    ...
    */
	...
	public function search_contact_ajax()
    {
		...
		$searchtext = stripslashes(mysql_real_escape_string($this->input->post('searchtext')));
    ...
    */
    public function insert_data()
    {
       ...
       //pr($cdata);exit;
		...
		if(!empty($interaction_contacts))
		{
			...
			foreach($interaction_contacts as $row)
			{
				if(!empty($row))
				{
					...
					$icdata['modified_date'] = $cdata['created_date'];
		...
		{
			redirect('superadmin/interaction/add_record/'.$interaction_plan_id);	
		}
		else
		{
    ...
    */
    public function update_data()
    {
							...
							foreach($interaction_list as $row1)
							{
								...
								if(!empty($row1['template_category']))
								{
									...
									else
									{
										...
										$category_details = $this->obj->getmultiple_tables_records($table,'','','','','','','','','id','desc','',$where);
										if(!empty($category_details))
										{
								...
								if(!empty($row1['template_name']))
								{
									...
									else
									{
										...
										$template_details = $this->obj->getmultiple_tables_records($table,'','','','','','','','','id','desc','',$where);
										if(!empty($template_details))
										{
		...
		if(!empty($submitbtn_action) && isset($submitbtn_action))
		{
			redirect('superadmin/interaction/'.$interaction_plan_id);	
		}
		else
		{
