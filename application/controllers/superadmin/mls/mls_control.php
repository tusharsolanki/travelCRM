...
{	
	$searchopt='';$searchtext='';$date1='';$date2='';$searchoption='';$perpage='';$mls_type='';$que='';
	$property_type='';$property_status='';$max_price='';$min_price='';
	...
	//property use only
	$mls_type = $this->input->post('mls_type');
	...
	if(!empty($mls_type))
	{
		$data['mls_type'] = $mls_type;
	} 
	else 
	{
		if(empty($allflag))
		{
			if(!empty($searchsort_session['mls_type'])) {
				$data['mls_type'] = $searchsort_session['mls_type'];
				$mls_type =  $data['mls_type'];
			}
		}
	}
	...
	else 
	{
		if(empty($allflag))
		{
			if(!empty($searchsort_session['property_type'])) {
				$data['property_type'] = $searchsort_session['property_type'];
	...
	else 
	{
		if(empty($allflag))
		{
			if(!empty($searchsort_session['property_status'])) {
				$data['property_status'] = $searchsort_session['property_status'];
	...
	else 
	{
		if(empty($allflag))
		{
			if(!empty($searchsort_session['min_price'])) {
				$data['min_price'] = $searchsort_session['min_price'];
				$min_price =  $data['min_price'];
				$data['max_price'] = $searchsort_session['max_price'];
...
//$uri_segment = $this->uri->segment(3);
	...
	{
		// MLS Amenity Tab
		$table = "mls_amenity_data mt";
		$fields = array('mm.mls_name','mt.*');
		$join_tables = array('mls_master as mm'=>'mm.id = mt.mls_id');
		$chk='mt.id != ""';
		if(!empty($searchtext) || !empty($mls_type)  || !empty($property_type) || !empty($property_status) || !empty($min_price) || !empty($max_price))
		{
			$que =' And ';
		}
		
		if(!empty($mls_type))
			$mstring= $que.' mm.id ='.$mls_type;
		if(!empty($property_type))
			$pstring= $que." mt.property_type = '".$property_type."'";
		
		$wherestring=$chk.$mstring.$pstring;
		//$group_by='mm.id, ipm.mls_id';
		if(!empty($searchtext))
		{
			$match=array('mls_name'=>$searchtext,'property_type'=>$searchtext,'code'=>$searchtext,'description'=>$searchtext,'value_code'=>$searchtext,'value_description'=>$searchtext);	
		...
		{
			$data['datalist']    = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config['per_page'], $uri_segment,$data['sortfield'],$data['sortby'],'',$wherestring);
	...
	elseif(!empty($tab) && $tab == 2)
	{
		...
		$table = "mls_area_community_data mt";
		$fields = array('mm.mls_name','mt.*');
		$join_tables = array('mls_master as mm'=>'mm.id = mt.mls_id');
		$chk='mt.id != ""';
		if(!empty($searchtext) || !empty($mls_type)  || !empty($property_type) || !empty($property_status) || !empty($min_price) || !empty($max_price))
		...
		$wherestring=$chk.$mstring;
		
		if(!empty($searchtext))
		{
			$match=array('mls_name'=>$searchtext,'area'=>$searchtext,'community'=>$searchtext);	
			$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$sortfield,$sortby,'',$wherestring);
			$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','','','','',$wherestring,'','1');
		}
		else
	...
	elseif(!empty($tab) && $tab == 3)
	{
		...
		if(!empty($searchtext))
		{
			$match=array('mls_name'=>$searchtext,'office_mls_id'=>$searchtext,'office_name'=>$searchtext,'street_address'=>$searchtext);	
			$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$sortfield,$sortby,'',$wherestring);
			$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','','','','',$wherestring,'','1');
		}
		else
	...
	elseif(!empty($tab) && $tab == 4)
	{
		...
		$table = "mls_property_history_data mt";
		$fields = array('mm.mls_name','mt.*');
		$join_tables = array('mls_master as mm'=>'mm.id = mt.mls_id');
		$chk='mt.id != ""';
		if(!empty($searchtext) || !empty($mls_type)  || !empty($property_type) || !empty($property_status) || !empty($min_price) || !empty($max_price))
		...
		if(!empty($searchtext))
		{
			$match=array('mls_name'=>$searchtext,'ml_number'=>$searchtext,'list_price'=>$searchtext);	
			$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$sortfield,$sortby,'',$wherestring);
			$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','','','','',$wherestring,'','1');
		}
		else
	...
	elseif(!empty($tab) && $tab == 5)
	{
		...
		if(!empty($searchtext))
		{
			$match=array('mls_name'=>$searchtext,'school_district_code'=>$searchtext,'school_district_description'=>$searchtext);	
			$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$sortfield,$sortby,'',$wherestring);
			$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','','','','',$wherestring,'','1');
		}
		else
	...
	{
		// MLS Member Tab
		$table = "mls_member_data mt";
		$fields = array('mm.mls_name,mt.member_mls_id,CONCAT_WS(" ",mt.first_name,mt.last_name) as name,mt.member_office_name');
		$join_tables = array('mls_master as mm'=>'mm.id = mt.mls_id');
		$chk='mt.id != ""';
		if(!empty($searchtext) || !empty($mls_type)  || !empty($property_type) || !empty($property_status) || !empty($min_price) || !empty($max_price))
		{
		...
		if(!empty($searchtext))
		{
			$match=array('mls_name'=>$searchtext,'member_mls_id'=>$searchtext,'CONCAT_WS(" ",first_name,last_name)'=>$searchtext,'member_office_name'=>$searchtext);	
			$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$sortfield,$sortby,'',$wherestring);
			$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','','','','',$wherestring,'','1');
		}
		else
	...
	elseif(!empty($tab) && $tab == 7)
	{
		...
		$table = "mls_property_list_master mt";
		$fields = array('mm.mls_name,mt.display_price,mt.ID,mt.LN,mt.PTYP,mt.LAG,mt.ST,mt.LP,mt.SP,mt.SSUF,mt.DRS,mt.full_address,mt.UD');
		$join_tables = array('mls_master as mm'=>'mm.id = mt.mls_id');
		//$fields = array('ID,LN,PTYP,LAG,ST,LP,SP,SSUF,DRS,UD');
		//echo $mls_type;exit;
		$chk='mt.id != ""';
		if(!empty($searchtext) || !empty($mls_type)  || !empty($property_type) || !empty($property_status) || !empty($min_price) || !empty($max_price))
		{
		...
		if(!empty($mls_type))
			$mstring= $que.' mm.id ='.$mls_type;
		if(!empty($property_type))
			$pstring= $que." mt.PTYP = '".$property_type."'";
		if(!empty($property_status))
			$sstring= $que." mt.ST = '".$property_status."'";
		// Min Price , Max Price
        if(!empty($min_price) && !empty($max_price)) 
            $price = $que.' (mt.display_price >='.str_replace(',','',$min_price).' AND mt.display_price <='.str_replace(',','',$max_price).')';
        else if(!empty($max_price) && empty($min_price))
            $price = $que.' mt.display_price >='.str_replace(',','',$max_price);
        else if(empty($max_price) && !empty($min_price))
            $price = $que.'mt.display_price >='.str_replace(',','',$min_price);
		
		$wherestring=$chk.$mstring.$pstring.$sstring.$price;
		if(!empty($searchtext))
		{	//$table='',$fields='',$join_tables='',$join_type='',$match_values = '',$condition ='', $compare_type = '', $num = '',$offset='',$orderby='',$sort='',$group_by='',$wherestring='',$where_in='',$totalrow='',$having='',$or_where=''
			$match=array('mls_name'=>$searchtext,'LN'=>$searchtext,'PTYP'=>$searchtext,'LAG'=>$searchtext,'ST'=>$searchtext,'LP'=>$searchtext,'SP'=>$searchtext,'SSUF'=>$searchtext,'DRS'=>$searchtext,'UD'=>$searchtext);	
			$data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$sortfield,$sortby,'',$wherestring);
			$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','','','','',$wherestring,'','1');
		}
		else
		{
           ...
           //Get mls lists
			...
			$fields = array('mm.*');
			$data['mls_type_data'] =$this->obj->getmultiple_tables_records($table,$fields);
			//pr($data['mls_type_data']);
	...
	$mls_amenity_sortsearchpage_data = array(
		...
		'tab' => !empty($tab)?$tab:'1',
		'mls_type' =>!empty($data['mls_type'])?$data['mls_type']:'',
		'property_type' =>!empty($data['property_type'])?$data['property_type']:'',
		'property_status' =>!empty($data['property_status'])?$data['property_status']:'',
...
{
	$listing_number = $this->uri->segment(4);
	...
	$config['base_url'] = site_url($this->user_type.'/'."mls/property_image_list/".$listing_number."/");
    $config['is_ajax_paging'] = TRUE; // default FALSE
    $config['paging_function'] = 'ajax_paging'; // Your jQuery paging
	if(!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
		$config['uri_segment'] = 0;
	...
	if(!empty($searchtext))
	{
		...
		$where=array('listing_number'=>$listing_number);
	...
	{
		$match=array('listing_number'=>$listing_number);
...
/*
@Description: Function to view property
@Author: Niral Patel
@Input: k
@Output: - 
@Date: 6-05-2015
