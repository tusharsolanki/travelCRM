				...
				if(!empty($csv_mapping_id))
				{
					...
					if(!empty($mls_fields))
					{
						foreach($mls_fields as $fields)
						{
							...
							{
								$data[$fields] = $line_of_text[$i][($this->input->post('slt_'.$fields)-1)];
							}
							if($this->input->post('slt_'.$fields) != '')
							{
								$template['contact_master_field'] = 'slt_'.$fields;
...
function add_new_field()
{
	...
	$this->obj->add_new_field($field_name,$field_type,$field_size,$last_field);
	return $field_name;
