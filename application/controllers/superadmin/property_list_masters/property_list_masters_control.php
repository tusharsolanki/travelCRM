...
{	
    ...
    */
    public function add_record()
    {
		...
		$table22 = "property_listing__status_master as plst";
		$fields22 = array('plst.*','lm.user_type');
		$join_tables22 = array('login_master as lm' => 'lm.id = plst.created_by');
		$group_by22='plst.id';
		$data['property_listing_status'] = $this->obj->getmultiple_tables_records($table22,$fields22,$join_tables22,'left','','','=','','','','',$group_by22,$match);
    ...
    */
    ...
    {
		$cdata['created_by'] = $this->superadmin_session['id'];
		$cdata['name'] = $this->input->post('property_status_type');
		$cdata['created_date'] = date('Y-m-d H:i:s');		
		$cdata['status'] = '1';
		$parent_id=$this->obj->insert_property_status_record($cdata);
		...
		// update already added record
		$udata['update'] = $this->input->post('property_status_update');
		$udata['idd'] = $this->input->post('property_status_idd');
		$update_name = $udata['update'];
		$update_id = $udata['idd'];
		for($u=0;$u<count($update_id);$u++)
		{
			...
			$rdata['modified_by'] = $this->superadmin_session['id'];
			$rdata['modified_date'] = date('Y-m-d H:i:s');
			$this->obj->update_property_status_record($rdata);   
		}	
    ...
    */
    ...
    {
        $cdata['id'] = $this->input->post('property_status_id');
		$cdata['name'] = $this->input->post('property_status_type');	
		$cdata['modified_by'] = $this->superadmin_session['id'];	
		$cdata['modified_date'] = date('Y-m-d H:i:s');
		$this->obj->update_property_status_record($cdata);
		$msg = $this->lang->line('common_edit_success_msg');
        $newdata = array('msg'  => $msg);
        $this->session->set_userdata('message_session', $newdata);	
    }
    ...
    */
	...
	function delete_property_status()
    {
		$id = $this->uri->segment(4);
        $this->obj->delete_property_status_record($id);
		$msg = $this->lang->line('common_delete_success_msg');
       	$newdata = array('msg'  => $msg);
       	$this->session->set_userdata('message_session', $newdata);	
        redirect('superadmin/'.$this->viewName.'/add_record');
