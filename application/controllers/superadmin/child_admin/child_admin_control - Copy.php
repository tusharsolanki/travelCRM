...
{	
    ...
    */
    ...
    {	
        ...
        $fields = array('caw.id,caw.status,CONCAT_WS(" ",caw.first_name,caw.last_name) as user_name,caw.email_id','caw.domain','lm.admin_name,caw.status,mm.mls_name');
    ...
    */
    ...
    function get_domain_id($edit_id)
    {
        ...
        if(!empty($domain_result) && !empty($domain_result[0]['db_name']))
        {
            ...
            $fields = array('id');
            $match = array('domain_name'=>$domain_result[0]['domain']);
    ...
    */
    ...
    function unpublish_record()
    {
        ...
        if(!empty($datalist[0]['id']))
        {
            ...
            $child_db = $datalist[0]['db_name'];
            $this->lead_users_model->update_status_record($icdata,$child_db);
        }
    ...
    */
    public function carousels()
    {
        ...
        if(!empty($searchtext))
        {
            ...
            $data['datalist'] = $this->carousels_master_model->select_records('',$match,'','like','',$config['per_page'],$uri_segment,$sortfield,$sortby,$where);
            $config['total_rows'] = $this->carousels_master_model->select_records('',$match,'','like','','','','','',$where,'','1');
        }
        else
        {
            $data['datalist'] = $this->carousels_master_model->select_records('',$where,'','=','',$config['per_page'],$uri_segment,$sortfield,$sortby);
            $config['total_rows'] = $this->carousels_master_model->select_records('',$where,'','=','','','','','','','','1');
        }
    ...
    */
    public function insert_carousels()
    {
        ...
        if(!empty($data['location_filter']))
        {
            ...
            $data['state'] = '';$data['city'] = '';$data['community_name'] = '';$data['zipcode']='';
        }
    ...
    */
    public function insert_footer()
    {
        ...
        $tab6_data['id'] = $id;
        // $tab6_data['mls_disclaimer'] = $this->input->post('mls_disclaimer');
        $tab6_data['copyright_statement'] = $this->input->post('copyright_statement');
        $tab6_data['modified_date'] = date('Y-m-d H:i:s');
        $tab6_data['modified_by'] = $this->superadmin_session['id'];
        $this->obj->update_record($tab6_data);
    ...
    public function fetch_agents($db_name,$domain_id)
    {
        ...
        foreach($login_master_data as $key => $value)
        {
            ...
            else if($value['user_type'] == 3 || $value['user_type'] == 4)
            {
                ...
                $user_data = $this->agent_team_model->select_records('user_master','',array('id'=>$value['user_id']),'','=','','','','','','',$db_name);
                $cdata['agent_name'] = $user_data[0]['first_name'].' '.$user_data[0]['last_name'];
                if(!empty($user_data[0]['contact_pic']))
                {
                    $cdata['agent_pic'] = $user_data[0]['contact_pic'];
                    copy($this->config->item('user_big_img_path').$user_data[0]['contact_pic'],$this->config->item('child_big_img_base_path').$cdata['agent_pic']);
                    copy($this->config->item('user_small_img_path').$user_data[0]['contact_pic'],$this->config->item('child_small_img_base_path').$cdata['agent_pic']);
                }
                $cdata['title'] = $user_data[0]['company_post'];
                $cdata['agent_description'] = $user_data[0]['notes'];
