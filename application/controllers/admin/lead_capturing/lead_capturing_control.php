...
{	
    ...
    */
    ...
    public function add_record()
    {
		...
		//Get all user
		$data['userlist'] = $this->obj->get_admin_agent_list();
    ...
    public function insert_data()
     {
		...
		$cdata['assign_user_id'] = $this->input->post('slt_user'); 
		//pr($cdata);exit;
		...
		$cdata1['file'] = $this->input->post('file_name');
		if($cdata1['file']!='')
			$cdata['file']=count($cdata1['file']);
		...
		if(!empty($contact_type[0]))
		{
				foreach($contact_type as $row)
				{
					...
					$lead_contact_type['created_by']=$this->admin_session['id'];
    ...
    */
    ...
    public function edit_record()
    {
		...
		//Get all user
		$cdata['userlist'] = $this->obj->get_admin_agent_list();
		
		//pr($cdata['userlist']);exit;
	...
	*/
	public function form_lead_list()
	{
            ...
            if(!empty($admin_data[0]['form_lead_last_seen']) && $admin_data[0]['form_lead_last_seen'] != '0000-00-00 00:00:00') {
                $data['current_date'] = $admin_data[0]['form_lead_last_seen'];
                $current_date = $admin_data[0]['form_lead_last_seen'];
            }
