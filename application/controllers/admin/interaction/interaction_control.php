...
{	
    ...
    */
    ...
    {	
		...
		$data['sortby']			= 'asc';
        $searchsort_session = $this->session->userdata('interaction_sortsearchpage_data');
		//pr($data['sortfield']).'1';
		...
		if(!isset($data['list'][0]['id']))
			redirect('admin/interaction_plans');
		...
		$data['msg'] = $this->message_session['msg'];
                ...
                $interaction_sortsearchpage_data = array(
                ...
                $this->session->set_userdata('interaction_sortsearchpage_data', $interaction_sortsearchpage_data);
    ...
    */
    ...
    {	
		...
		$data['sortby']			= 'desc';
                $searchsort_session = $this->session->userdata('iview_archive_sortsearchpage_data');
...
/*                            $data['searchtext'] = $searchsort_session['searchtext'];
		...
		$data['msg'] = $this->message_session['msg'];
                ...
                $iview_archive_sortsearchpage_data = array(
				'sortfield'  => !empty($data['sortfield'])?$data['sortfield']:'',
				'sortby' =>!empty($data['sortby'])?$data['sortby']:'',
				'searchtext' =>!empty($data['searchtext'])?$data['searchtext']:'',
				'perpage' => !empty($data['perpage'])?trim($data['perpage']):'10',
                ...
                $this->session->set_userdata('iview_archive_sortsearchpage_data', $iview_archive_sortsearchpage_data);
                $data['uri_segment'] = $uri_segment;
		if($this->input->post('result_type') == 'ajax')
		{
    ...
    */
    ...
    public function view_record()
    {
		...
		if(isset($result[0]['status']) && $result[0]['status'] == '0')
		{
			...
			redirect('admin/'.$this->viewName."/".$plan_id);
		}
...
//		pr($data['editRecord']);exit;
		...
		$catid = !empty($result[0]['template_category'])?$result[0]['template_category']:'';
		...
		switch(strtolower($plan_type[0]['name']))
		{
					case 'email':
					$match = array('template_category'=>$catid);
					$match1 = array("email_send_type"=>2);
					$match = array_merge($match,$match1);
					$fields = array('id','template_name','template_category');
					$data['template_list'] = $this->email_library_model->select_records($fields,$match,'','=','','','',$sortfield,$sortby);
					break;
					...
					case 'sms':
					$fields = array('id','template_name','template_category');
					$data['template_list'] = $this->sms_texts_model->select_records($fields,$match,'','=','','','',$sortfield,$sortby);
					break;
					case 'call':
					$fields = array('id','template_name','template_category');
					$data['template_list'] = $this->phonecall_script_model->select_records($fields,$match,'','=','','','',$sortfield,$sortby);
					//echo $this->db->last_query();
					break;
					case 'envelope':
					$fields = array('id','template_name','template_category');
					$data['template_list'] = $this->envelope_library_model->select_records($fields,$match,'','=','','','',$sortfield,$sortby);
					break;
					case 'label':
					$fields = array('id','template_name','template_category');
					$data['template_list'] = $this->label_library_model->select_records($fields,$match,'','=','','','',$sortfield,$sortby);
					break;
					case 'letter':
					$fields = array('id','template_name','template_category');
					$data['template_list'] = $this->letter_library_model->select_records($fields,$match,'','=','','','',$sortfield,$sortby);
					break;
				}
		//pr($data);exit;
    ...
    */
    ...
    public function add_record()
    {
		...
		elseif(isset($check_plan_status[0]['status']) && $check_plan_status[0]['status'] == '0')
		{
			...
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);
			redirect('admin/'.$this->viewName."/".$data['plan_id']);
		}
		...
		$table = "login_master as lm";
		$fields = array('lm.id,lm.admin_name,lm.email_id,lm.user_id,lm.status,lm.user_type,um.agent_id,CONCAT_WS(" ",um.first_name,um.middle_name,um.last_name) as user_name,um.status as user_status');
		$join_tables = array('user_master as um'=>'um.id = lm.user_id',
							 'user_right_transaction as urt'=>'urt.user_id = lm.id',
							 'module_master mm'=>'mm.id = urt.module_id',
							 );
		$group_by='lm.id';

		$where = "(lm.user_type = '2' OR (lm.user_type = '3' && um.status = '1' AND mm.module_unique_name = 'communications')) AND lm.status = '1'";
		
        $data['user_list'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','=','','','','',$group_by,$where);
		...
		if(($interaction_type == 6 || $interaction_type == 8) && !empty($template_id) && !empty($last_id))
		{
			...
			if(!empty($result[0]['id']))
			{
				...
				if($this->input->post('sign_include') == 1)
				{
					...
					$email_signature_data = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','is_default','asc','',$match);
					/*echo $this->db->last_query();
					pr($email_signature_data);exit;*/
...
//		pr($data['editRecord']);exit;
		...
		$assign_to = !empty($result[0]['assign_to'])?$result[0]['assign_to']:'0';
		$where = "((lm.user_type = '2' OR (lm.user_type = '3' && um.status = '1' AND mm.module_unique_name = 'communications')) AND lm.status = '1') OR lm.id = ".$assign_to;
        $data['user_list'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','=','','','','',$group_by,$where);
		...
		if(count($interaction_id) > 1)
		{
			...
			if(isset($last_key) && $interaction_id[$last_key] != $result[0]['id'])
				$data['next_interaction'] = $interaction_id[$current_key + 1];
			if(!empty($last_key) && !empty($current_key))
				$data['previous_interaction'] = $interaction_id[$current_key - 1];
		}
    ...
    */
    public function update_data()
    {
		...
		else if($this->input->post('rad_start_type') == '2')
		{
			 ...
			 $cdata['number_type']=$this->input->post('slt_nub_type_2');
			 $cdata['interaction_id']=$this->input->post('slt_interaction_stat_2');
			 $slt_interaction_stat_12=$this->input->post('slt_interaction_stat_2');
			...
			if($loop == 1)
			{
        		...
        		$this->session->set_userdata('message_session', $newdata);
				redirect('admin/'.$this->viewName."/".$plan_id."/".$pagingid);
			}
			//echo $j; exit;
		...
		if(!empty($interaction_details))
		{
			foreach($interaction_details as $row)
			{
				...
				$ctdata['modified_by'] = $this->admin_session['id'];
    ...
    */
    function delete_record()
    {
		...
		$msg = $this->lang->line('common_delete_success_msg');
       	$newdata = array('msg'  => $msg);
       	$this->session->set_userdata('message_session', $newdata);	
        redirect('admin/'.$this->viewName.'/'.$this->uri->segment(4));
    ...
    */
	...
	/*
    @Description: Function use to Active and Inactive contact in by admin
    @Author: Kaushik Valiya
    @Input: 
    @Output: - 
    @Date: 16-07-2014
    ...
    */
	...
	/*
    @Description: Function use to select template by admin
    @Author: Nishit Modi
    @Input: 
    @Output: - 
    @Date: 21-08-2014
    */
	...
	public function ajax_selecttemplate()
	{
		...
		if(!empty($id))
		{
			...
			if(!empty($cdata['plan_type'][0]['id']) && ($catid > 0 || !empty($selected)))
			{
				...
				if($catid > 0)
					$match = array('template_category'=>$catid);
				//echo $cdata['plan_type'][0]['name'];exit;
				switch(strtolower($cdata['plan_type'][0]['name']))
				{
					...
					$match = array_merge($match,$match1);
					$fields = array('id','template_name','template_category');
					$cdata['template_list'] = $this->email_library_model->select_records($fields,$match,'','=','','','',$sortfield,$sortby);
					break;
					case 'bomb bomb emails':
					$fields = array('id','template_name','template_category');
	...
	/*
		@Description: Function use to select category by admin
