...
{	
    ...
    */
    ...
    {	
		...
		$fields = array('email_template_master.*,mmlcm.category','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name');
		...
		if(!empty($searchtext) || !empty($selected_cat))
		{
			...
			if(!empty($selected_cat))
				$wherestring='email_send_type = 2 AND is_default =0 AND template_category = '.$selected_cat;
			else
				$wherestring='email_send_type = 2  AND is_default =0' ;
		...
		{
			$match = array('email_send_type' => 2,'is_default' => '0');
		...
		$table = "email_template_master";
		$fields = array('email_template_master.*,mmlcm.category,et.superadmin_publish_date as parent_superadmin_publish_date,et.publish_flag as parent_publish_flag');
		$join_tables = array('marketing_master_lib__category_master mmlcm'=>'mmlcm.id = email_template_master.template_category',''.$this->parent_db_name.'.email_template_master et'=>'et.id = email_template_master.superadmin_template_id');
		$group_by = 'email_template_master.id';
		...
		if(!empty($searchtext1) || !empty($selected_cat1))
		{
			...
			if(!empty($selected_cat1))
				$wherestring='email_template_master.email_send_type = 2 AND email_template_master.template_category = '.$selected_cat1.' AND email_template_master.is_default = 1';
			else
				$wherestring='email_template_master.email_send_type = 2 AND email_template_master.is_default = 1';
		...
		{
			$match = array('email_template_master.email_send_type' => 2,'email_template_master.is_default'=>'1');
			//$data['datalist'] = $this->obj->select_records('',$match,'','','',$config['per_page'],$uri_segment,$sortfield,$sortby);	
		...
		$match = array('email_send_type'=>'2','is_default'=>'1');
