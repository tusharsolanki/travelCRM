...
{	
    ...
    */
    ...
    {	
		...
		$fields = array('sms_text_template_master.*,mmlcm.category','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name');
		...
		if(!empty($searchtext) || !empty($selected_cat))
		{
			...
			if(!empty($selected_cat))
				$wherestring = 'sms_send_type = 2 AND sms_text_template_master.template_category = '.$selected_cat.' AND is_default = 0';
			else
				$wherestring = 'sms_send_type = 2 AND is_default = 0';	
		...
		{
				$match = array("sms_send_type" => 2, "is_default"=>'0');
		...
		$table = "sms_text_template_master";
		$fields = array('sms_text_template_master.*,mmlcm.category,et.superadmin_publish_date as parent_superadmin_publish_date,et.publish_flag as parent_publish_flag');
		$join_tables = array('marketing_master_lib__category_master mmlcm'=>'mmlcm.id = sms_text_template_master.template_category',''.$this->parent_db_name.'.sms_text_template_master et'=>'et.id = sms_text_template_master.superadmin_template_id');
		$group_by = 'sms_text_template_master.id';
		...
		if(!empty($searchtext1) || !empty($selected_cat1))
		{
			...
			if(!empty($selected_cat1))
				$wherestring='sms_text_template_master.sms_send_type = 2 AND sms_text_template_master.template_category = '.$selected_cat1.' AND sms_text_template_master.is_default = 1';
			else
    ...
    */
	...
	{
		$match = array('sms_send_type'=>'2', 'is_default'=>'1');
		$field=array('DISTINCT superadmin_template_id');
        $new_temp = $this->obj->select_records($field,$match,'','=','','','','id','desc','');
		if(!empty($new_temp))
		{
    ...
    */
	...
	public function ajax_delete_all()
	{
	 ...
	 /*
    @Description: Function for Unpublish Sms Texts Profile By Admin
    @Author: Mohit Trivedi
    ...
    */
	...
	/*
    @Description: Function for publish Sms Texts Profile By Admin
    @Author: Mohit Trivedi
    ...
    */
	...
	/*
    @Description: Function for  Sms Texts subcategory Profile By Admin
    @Author: Mohit Trivedi
