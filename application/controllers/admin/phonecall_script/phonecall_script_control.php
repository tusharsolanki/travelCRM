    ...
    */
    ...
    {	
		...
		if($main == 1)	
		{
			...
			$fields = array('phone_call_script_master.*,mmlcm.category','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name');
			...
			{
				if(!empty($searchtext))
					$match=array('phone_call_script_master.template_name'=>$searchtext,'phone_call_script_master.template_subject'=>$searchtext);
				else
					$match=array();
				...
				if(!empty($selected_cat))
					$wherestring = 'phone_call_script_master.template_category = '.$selected_cat.' AND is_default = 0';
				else
		...
		$selected_cat = $this->input->post('selected_cat');
        $data['sortfield1']		= 'phone_call_script_master.id';
        $data['sortby1']		= 'desc';
		...
		$table = "phone_call_script_master";
		$fields = array('phone_call_script_master.*,mmlcm.category,et.superadmin_publish_date as parent_superadmin_publish_date,et.publish_flag as parent_publish_flag');
		$join_tables = array('marketing_master_lib__category_master mmlcm'=>'mmlcm.id = phone_call_script_master.template_category',''.$this->parent_db_name.'.phone_call_script_master et'=>'et.id = phone_call_script_master.superadmin_template_id');
		$group_by = 'phone_call_script_master.id';

		if(!empty($searchtext1) || !empty($selected_cat1))
		{
			if(!empty($searchtext1))
				$match=array('phone_call_script_master.template_name'=>$searchtext1,'phone_call_script_master.template_subject'=>$searchtext1);
			else
				$match=array();
			//$data['datalist'] = $this->obj->select_records('',$match,'','like','',$config['per_page'],$uri_segment,$sortfield,$sortby);
			//$config['total_rows'] = count($this->obj->select_records('',$match,'','like',''));
			...
			if(!empty($selected_cat1))
				$wherestring='phone_call_script_master.template_category = '.$selected_cat1.' AND phone_call_script_master.is_default = 1';
			else
