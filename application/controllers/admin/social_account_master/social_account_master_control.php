...
{	
    ...
    */
    ...
    {	
		//check user right
		check_rights('social_account');
		//Get social credential
		$field = array('id','linkedin_access_token','linkedin_username','twitter_access_token','google_access_token','google_user_name','twitter_access_token_secret','twitter_username','fb_api_key','fb_secret_key','bombbomb_username');
    ...
    */
    ...
    public function add_record()
    {
		if($_REQUEST['action'] == 'login')
		{
			...
			//$data['fb_details']= $result;
			//pr($data['fb_details']);exit;
			$data['main_content'] = "admin/".$this->viewName."/facebookdata/actions";       
			$this->load->view("admin/include/template",$data);
		}
		else
		{
    ...
    */
    ...
    {
			$data['main_content'] = "admin/".$this->viewName."/login_with_linkedin/index";       
			$this->load->view("admin/include/template",$data);
    }
    public function add_linkedin()
    {
		$this->data['callback_url']         =   base_url().'admin/social_account_master/linkedin_insert_data';
...
//pr($oauth_data);exit;
		...
		if(!empty($contacts['person']))
		{
			...
			{ 
				...
				if(!empty($user_id))
				{
					...
					$match = array('linkedin_id'=>$cdata['linkedin_id'],'linkedin_user_id'=>$cdata['linkedin_user_id'],'created_by'=>$this->admin_session['id']);
    ...
    */
	function insert_linkedin_data()
	{
		...
		pr($xml_response);exit;
		$id=$this->admin_session['id'];
		$ldata=array('id'=>$id,'linkedin_access_token'=>$access_token,'linkedin_username'=>$linkedin_username,'linkedin_secret_access_token'=>$access_secret_token);
		$uid = $this->admin_model->update_user($ldata);	
	}
	/*
        @Description: Function Add New Social Account master details
    ...
    */
    ...
    public function add_twitter()
    {
			...
			$cusumer_key=$this->config->item('twitter_access_key');
			$cusumer_secret=$this->config->item('twitter_secret_key');
			 $cusumer_callback_url=$this->config->item('twitter_callback_url');
    ...
    */
    ...
    {	
            ...
            $this->session->set_userdata('twitter_old_session', $newdata);
			 redirect('admin/social_account_master/clearsessions');
			}
			...
			//echo '1'.'dsifsdhfsjd';exit;
			   redirect('admin/social_account_master/get_twitter_data');
		...
		if(!empty($followers_count) && $followers_count <= $total_follower)
		{
			...
			if(!empty($result))
			{
				foreach ($result->users as $follower) 
				{
					...
					$data1['created_by']=$this->admin_session['id'];
					$data1['created_date']=date('Y-m-d H:i:s');
					$data1['status']='1';
					
					$match=array('twitter_user_id'=>$data1['twitter_user_id'],'twitter_id'=>$data1['twitter_id'],'created_by'=>$this->admin_session['id']);
    ...
    */
	function import_google_contact()
	{
		...
		foreach ($xml as $title) 
		{
			...
			if(!empty($contact_name))
			{
				...
				$cdata['created_date'] = date('Y-m-d H:i:s');
				$cdata['status'] = '1';
				$match = array('google_user_id'=>$cdata['google_user_id']);
				$result = $this->obj1->select_records1('',$match,'','=','','','','id','desc','contact_master');
				if(empty($result))
				{	
					$contact_id = $this->contacts_model->insert_record($cdata);
	...
	/*
        @Description: Function for connect to bomb bomb
        @Author     : Niral Patel
        @Input      : Import contact
        @Output     : Connect to google
		@Date       : 26-1-2015
    */
	function bombbomb_connection()
	{
		$parent_db = $this->config->item('parent_db_name');
		$cdata['username']=$this->input->post('bomb_username');
		$cdata['password']=$this->input->post('bomb_password');
		//$IsValidLogin = file_get_contents("https://app.bombbomb.com/app/api/api.php?method=IsValidLogin&email=".urlencode($user_name)."&pw=".urlencode($pass));
                $url = "https://app.bombbomb.com/app/api/api.php?method=IsValidLogin";
                
                $IsValidLogin = $this->admin_model->bombbombapi_curl($url,$cdata);
		...
		{
			$password=$this->Common_function_model->encrypt_script($cdata['password']);
			
			//Insert bomb bomb credential
			$data['bombbomb_username']=!empty($cdata['username'])?trim($cdata['username']):'';
			$data['bombbomb_password']=!empty($password)?trim($password):'';
			$data['id']=$this->admin_session['id'];
			$uid = $this->admin_model->update_user($data);
			//Update parent db
			...
			$field = array('id','db_name','email_id');
			$admindata=array('db_name'=>$result[0]['db_name'],'email_id'=>$result[0]['email_id']);
			$admin_data = $this->admin_model->get_parent_login_details($field,$parent_db,$admindata);
			
			$admin_id=!empty($admin_data[0]['id'])?$admin_data[0]['id']:'';
			$data['id']=$admin_id;
			
			$uid = $this->admin_model->update_user($data,$parent_db);
			//echo $this->db->last_query();exit;
			$msg = $this->lang->line('common_connect_msg');
		}
		else
			$msg = $this->lang->line('invalid_us_pass_bombbomb');
		$newdata = array('msg'  => $msg,'connect'=>'BombBomb');
		$this->session->set_userdata('message_session', $newdata);
		redirect('admin/social_account_master');
	}
	/*
    ...
    */
			...
			{ 
				if(!empty($data_list['data'][$i]['id']))
				{
					...
					{    
						...
						$match = array('fb_id'=>$cdata['fb_id'],'created_by'=>$this->admin_session['id']);
        				$result = $this->obj1->select_records1('',$match,'','=','','','','id','desc','contact_master');
    ...
    public function insert_linkedin_data1()
    {
		//pr($this->admin_session);exit;
		$data_frnd=$this->input->post($frnd);
		$data_user=$this->input->post($user_array);
    ...
    */
	...
	/*
        @Description: Function for disconnect bombbomb
        @Author     : Niral Patel
        @Input      :
        @Output     : Disconnect bombbomb
        @Date       : 2-3-2015
    */
	function disconnect_bombbomb()
	{
		$parent_db=$this->config->item('parent_db_name');
		$data['bombbomb_username']='';
		$data['bombbomb_password']='';
		$data['id']=$this->admin_session['id'];
		$uid = $this->admin_model->update_user($data);
		...
		echo $uid = $this->admin_model->update_user($data,$parent_db);
		/*$table='login_master as l';
		$fields = array('l.id,l.admin_name,l.email_id');
		//$group_by='l.id';
		$where = "l.user_type = 3 or l.user_type = 4";
		$admin_user_list=$this->admin_model->getmultiple_tables_records($table,$fields,'','','','','','', '','l.id','asc','',$where);
		//pr($admin_user_list);exit;
		...
		if(!empty($admin_user_list))
		{
			...
			{
				$this->module_master_model->delete_module_subchild_array($row['id'],$deletecontactdata,'');
			}
		}*/
	}
