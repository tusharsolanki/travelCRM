...
{	
    ...
    */
    ...
    {	
		...
		$searchopt='';$searchtext='';$date1='';$date2='';$searchoption='';$perpage='';$wherestring='';
		...
		{
			$data['datalist'] =$this->mail_blast_model->getmultiple_tables_records($table,$fields,$join_tables,'left','','','',$config['per_page'], $uri_segment,$sortfield,$sortby,$group_by);
		//pr($data['datalist']);exit;
		$config['total_rows'] = $this->mail_blast_model->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,'','1');
		}
		$this->pagination->initialize($config);
		$data['pagination'] = $this->pagination->create_links();
    ...
    */
	...
	public function search_contact_ajax()
    {
		...
		//$sord_bytest = $this->input->post('sort_by');
		...
		$data['contact_list'] =$this->interaction_plans_model->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$data['sortfield'],$data['sortby'],$group_by,$where);
		
		//echo $this->db->last_query();
		$config['total_rows'] = $this->interaction_plans_model->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','','','',$group_by,$where,'','1');
    ...
    */
	...
	public function get_envelope()
	{
		...
		{
			$match = array('template_category' => $category);
	...
	public function mail_out_print()
	{
		...
		$data['main_content'] = "admin/".$this->viewName."/print";
        $this->load->view('admin/include/template', $data);
	}
	...
	public function generate_pdf()
	{
		...
		$pdata['sort_by'] 			= $this->input->post('post_category');
		...
		if(!empty($data))
		{
			$this->load->view("admin/mail_out/compare_pdf", $data);
		}
	}
