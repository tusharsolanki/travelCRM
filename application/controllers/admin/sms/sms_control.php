...
{	
    ...
    */
	...
	//	echo "hi"; exit;
		...
		$wherestring = 'scm.sms_type = "Campaign"';
		if(!empty($searchtext))
		{
...
;
    ...
    */
    ...
    public function insert_data()
    {
		...
		if($this->input->post('submitbtn1') || $this->input->post('submitbtn'))
		{
			...
			$data['sms_send_from'] = $this->admin_session['id'];
    ...
    */
    ...
    public function edit_record()
    {
		...
		$cdata['contact_list'] = $this->contact_type_master_model->select_records('','','','','',$config['per_page'],'0','id','desc');
		//echo $this->db->last_query();
		$config['total_rows'] = $this->contact_type_master_model->select_records('','','','','','','','id','desc','1');
		$this->pagination->initialize($config);
    ...
    */
	...
	public function ajax_templatedata()
	{
		...
		{
                    $match = array("template_category"=>$category_id,"sms_send_type"=>2);
    ...
    */
	...
	public function queued_list()
	{
		...
		$wherestring = "scm.sms_type = 'Intereaction_plan' AND scr.is_send = '0' AND ipm.status = '1' AND ipi.status='1'";
		$groupby = 'ipm.id';
		if(!empty($searchtext))
		{
    ...
    */
	...
	public function interaction_queued_list()
	{
		...
		$match=array('ipm.plan_name'=>$searchtext);
		$wherestring = "scm.sms_type = 'Intereaction_plan' AND scr.is_send = '0' AND ipm.status = '1'";
		$groupby = 'ipm.id';
		$data['interaction_plan'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'],$uri_segment,$sortfield,$sortby,$groupby,$wherestring);
		$config['total_rows'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','',$sortfield,$sortby,$groupby,$wherestring,'','1');
    ...
    */
	...
	public function interaction_plan_queued_list()
	{
		...
		$table ="sms_campaign_recepient_trans as scr";
		$fields = array('scr.sms_message,scr.contact_id','cm.first_name,cm.last_name','ipi.description,ipm.plan_name','scm.sms_send_date,scm.sms_type,scr.send_sms_date,scm.interaction_id,scr.id,scr.is_sms_exist,ipccp1.is_done,ipi.interaction_id as interaction_id1,scr.is_sms_exist,cpt.is_default,ipi.start_type as i_start_type');
		...
		$wherestring = "scm.sms_type = 'Intereaction_plan' AND ipm.id = ".$interaction_plan." AND scr.is_send = '0' AND ipi.status='1'";
		$groupby = 'scr.id';
		if(!empty($searchtext))
		{
    ...
    */
	...
	public function view_interaction_data()
	{
		$id = $this->uri->segment(4);
		$table ="sms_campaign_recepient_trans as scr";
		$fields = array('cpt.phone_no,scr.id,scr.sms_campaign_id,scr.sms_message,scr.contact_id,stm.template_name,CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name,mml1.category as category,scm.interaction_id');
    ...
    */
	...
	/*
		@Description: Function for Selected contacts type add the SMS to
		@Author: Sanjay Chabhadiya
		@Input: - contact_id
		@Output: - 
		@Date: 06-08-2014
