...
{	
    ...
    */
    ...
    {	
        ...
        $sel_agent = $this->input->post('sel_agent');
        ...
        // Search Criteria Sanjay Moghariya 11-07-2105
        if(!empty($reg_source))
        ...
        if(!empty($sel_agent))
            $data['search_agent'] = $sel_agent;
        else
        {
            if(!empty($searchsort_session['search_agent'])) {
                $data['search_agent'] = $searchsort_session['search_agent'];
                $sel_agent = $searchsort_session['search_agent'];
            }
        }
        // Search Criteria Sanjay Moghariya 11-07-2105
        ...
        if(!empty($current_date))
        {
            ...
            $leads_dashboard_sortsearchpage_data = array(
                ...
                'search_reg_source' => !empty($reg_source)?$reg_source:'0',
                'search_lead_type' => !empty($sel_lead_type)?$sel_lead_type:'0',
                'search_agent' => !empty($sel_agent)?$sel_agent:'0',
            );
	...
	$wherestring = '';
        ...
        if(!empty($sel_agent))
        {
            $wherestring .= 'uct.user_id ="'.$sel_agent.'" AND ';
        }
		...
		if($flag_category == 1)
		{
			if($searchtext == 'Inactive')
			{
                            ...
                            $wherestring .= 'cm.created_type = 6 AND cm.joomla_category != "Inactive Prospect" AND cm.joomla_category != "Bogus" AND ';
				//$wherestring = array('cm.created_type'=>'6','cm.joomla_category !='=>"'Inactive Prospect'",'cm.joomla_category !='=>"'Bogus'");
			}
			else
			{
		...
		else
		{
            ...
            {
                 $wherestring = 'cm.created_type = 6 AND cm.created_date > "'.date('Y-m-d H:i:s',strtotime($current_date1)).'"';
                 //$wherestring = array('cm.created_type'=>'6','cm.created_date > '=>date('Y-m-d H:i:s',strtotime($current_date1)));
                 //$wherestring = 'cm.created_type = 6 AND cm.joomla_category != "Inactive Prospect" AND cm.joomla_category != "Bogus"';
            }
            else
            {*/
                $wherestring .= 'cm.created_type = 6 AND ';
            //}
        ...
        $fields = array('id,domain_name,slug,login_id');
        $match = array('login_id'=>$this->admin_session['id'],'website_status'=>1);
        $data['sel_assigned_domain'] =$this->contacts_model->getmultiple_tables_records($table,$fields,'','','',$match,'=');
        
        // Agent listing
        $table ="login_master as lm";   
        $fields = array('um.id,lm.email_id,CONCAT_WS(" ",um.first_name ,um.middle_name ,um.last_name) as agent_name,lm.user_type');
        $join_tables = array('user_master as um'=>'lm.user_id = um.id');
        $match = array('lm.user_type'=>3);
        $data['sel_agent_list'] = $this->contacts_model->getmultiple_tables_records($table,$fields,$join_tables,'left','',$match,'=');
        ...
        $leads_dashboard_sortsearchpage_data = array(
            ...
            'search_agent' => !empty($sel_agent)?$sel_agent:'',
        );
    ...
    */
    public function view_record()
    {
		...
		$where = "um.status = '1' AND um.user_type = '3'";
		...
		$data['selected_contact_id'] = $id;
                ...
                $data['result_saved_searches'] = $this->saved_searches_model->select_records('',$match,'','=','',$config1['per_page'],$uri_segment1,'id','desc');
                ...
                $match = array('lw_admin_id'=>$id);
                $data['result_valuation_searched'] = $this->property_valuation_searches_model->select_records('',$match,'','=','',$config5['per_page'],$uri_segment5,'id','desc');
    ...
    public function view_record_index_val_searched()
    {
        ...
        */
        public function insert_saved_search_data()
        {
            ...
            //pr($cdata);exit;
            //$this->saved_searches_model->insert_record($cdata);	
            $msg = $this->lang->line('common_add_success_msg');
            $newdata = array('msg'  => $msg);
            $this->session->set_userdata('message_session', $newdata);
			...
			if(!empty($contact_id['contact_id']))
                redirect('admin/'.$this->viewName.'/view_record/'.$contact_id['contact_id'].'/3');
            else
                redirect('admin/'.$this->viewName);
        }
    ...
    */
    public function update_view()
    {
        ...
        $newdata = array('msg'  => $msg);
        $this->session->set_userdata('message_session', $newdata);
/*		redirect('admin/'.$this->viewName.'/view_record/'.$contact_id.'/'.($viewtab));*/
		 $data = array('selected_view' => $viewtab);
        $this->session->set_userdata('joomla_dbord_selected_view_session',$data);
		if($viewtab == 1)
		{
			redirect('admin/'.$this->viewName.'/view_record/'.$contact_id);
		}
		else if($viewtab == 4)
		{
			redirect('admin/'.$this->viewName.'/view_record/'.$contact_id);
