...
{	
    ...
    */
    ...
    {	
	    //check user right
		check_rights('label_library');
		$label_selected_view_session = $this->session->userdata('label_selected_view_session');
        if(!empty($label_selected_view_session['selected_view']))
            $selected_view = $label_selected_view_session['selected_view'];
        else
            $selected_view = '1';
		...
		if($main == 1)	
		{
			...
			$fields = array('label_template_master.*,mmlcm.category','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name');
			$join_tables = array('marketing_master_lib__category_master mmlcm'=>'mmlcm.id = label_template_master.template_category',
								 'login_master as lm'                          => 'lm.id = label_template_master.created_by',
			...
			if(!empty($searchtext) || !empty($selected_cat))
			{
				...
				if(!empty($selected_cat))
					$wherestring = 'template_category = '.$selected_cat.' AND is_default = 0';
				else
		...
		$selected_cat = $this->input->post('selected_cat');
        $data['sortfield1']		= 'label_template_master.id';
        $data['sortby1']		= 'desc';
        $searchsort_session1 = $this->session->userdata('def_label_library_sortsearchpage_data');
        ...
        $config1['base_url'] = site_url($this->user_type.'/'."label_library/");
		...
		$table = "label_template_master";
		$fields = array('label_template_master.*,mmlcm.category,et.superadmin_publish_date as parent_superadmin_publish_date,et.publish_flag as parent_publish_flag');
		$join_tables = array('marketing_master_lib__category_master mmlcm'=>'mmlcm.id = label_template_master.template_category',''.$this->parent_db_name.'.label_template_master et'=>'et.id = label_template_master.superadmin_template_id');
		$group_by = 'label_template_master.id';

		if(!empty($searchtext1) || !empty($selected_cat1))
		{
			if(!empty($searchtext1))
				$match=array('label_template_master.template_name'=>$searchtext1);
			else
				$match=array();
			//$data['datalist'] = $this->obj->select_records('',$match,'','like','',$config['per_page'],$uri_segment,$sortfield,$sortby);
			//$config['total_rows'] = count($this->obj->select_records('',$match,'','like',''));
			...
			if(!empty($selected_cat1))
				$wherestring='label_template_master.template_category = '.$selected_cat1.' AND label_template_master.is_default = 1';
			else
				$wherestring='label_template_master.is_default = 1';
        ...
        $def_label_library_sortsearchpage_data = array(
        ...
        $this->session->set_userdata('def_label_library_sortsearchpage_data', $def_label_library_sortsearchpage_data);
    ...
    */
    ...
    {
		 //check user right
		check_rights('label_library_add');
    ...
    */
    public function selectedview_session()
    {
        ...
        $sortsearchpage_data = array(
            'sortfield'  => 'label_template_master.id',
            'sortby' => 'desc',
            'searchtext' =>'',
            'perpage' => '',
            'uri_segment' => 0);
        if($selected_view == '1')
            $this->session->set_userdata('def_label_library_sortsearchpage_data', $sortsearchpage_data);
        else
            $this->session->set_userdata('label_library_sortsearchpage_data', $sortsearchpage_data);
        $data = array('selected_view' => $selected_view);
        $this->session->set_userdata('label_selected_view_session',$data);
    }
    /*
    ...
    */
    ...
    {
		 //check user right
		check_rights('label_library_edit');
    ...
    */
    ...
    {
		//check user right
		check_rights('label_library_delete');
        $id = $this->uri->segment(4);
