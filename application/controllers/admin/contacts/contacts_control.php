...
{	
    function __construct()
    {
        ...
        //$this->obj5 = $this->user_management_model;
		...
		if(array_key_exists('livewire_admin_session',$this->session->all_userdata()))
			echo "hi";
		exit;*/
    }
    ...
    */
    ...
    {
		/*$phone_data = '123-123-12345';
		//$phone_data = '-/*-/-/--/+/*-';
		//this.regex(v, /(?=(.*[0-9a-zA-Z]){1})^[0-9a-zA-Z!@#$%^&*\(\)_=|\\/\-\+"':;,.><[\]{}?\ ]{0,}$/);
		$regex = '/^-?([\+]{0,1})+([0-9])+$/';
		//$regex = '/(?=(.*[0-9a-zA-Z]){1})^[0-9a-zA-Z!@#$%^&*\(\)_=|\\/\-\+:;,.><[\]{}?\ ]{0,}$/';
		if(preg_match($regex, $phone_data)) {
			echo "hi";exit;
		}
		exit;*/
		//check user right
        ...
        $search_status='';$searchopt='';$searchtext='';$date1='';$date2='';$searchoption='';$perpage='';
        $searchtext = mysql_real_escape_string($this->input->post('searchtext'));
        $status_id=$this->input->post('status_id');
        //print_r($status_id);
        $sortfield = $this->input->post('sortfield');
        $sortby = $this->input->post('sortby');
        $search_status=$this->input->post('status_id');
        ...
        $table4 = "contact_listing_last_seen as cll";
        $fields4 = array('cll.*,lm.user_type');
        $join_tables4 = array('login_master as lm' => 'lm.id = cll.login_id');
        $group_by4='cll.id';
        $data['last_seen'] = $this->obj1->getmultiple_tables_records($table4,$fields4,$join_tables4,'left','','','=','','','','',$group_by4);
        ...
        if(!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
			...
			$config['uri_segment'] = 3;
			$uri_segment = $this->uri->segment(3);
			if($uri_segment == '' && !empty($searchsort_session['uri_segment']) && !isset($_POST['result_type']))
			{
				$config['cur_page'] = $searchsort_session['uri_segment'];
				$uri_segment = $searchsort_session['uri_segment'];
			}
			elseif($uri_segment < 0)
...
//pr($contacts_sortsearchpage_data);exit;
        ...
        if(!empty($searchtext) || is_array($status_id))
        {
			/*$fields = array('cm.id,cm.is_subscribe','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','cm.company_name','csm.name as contact_status','group_concat(DISTINCT ctm1.name ORDER BY ctm1.name separator \',\') as contact_type','cpt.phone_no','cet.email_address','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name','cm.created_by','cm.created_type');
            $join_tables = array(
                ...
                '(SELECT cttin.* FROM contact_tag_trans cttin GROUP BY cttin.contact_id) AS ctat'=>'ctat.contact_id = cm.id',
			...
			$fields = array('cm.id,cm.is_subscribe','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','cm.company_name','csm.name as contact_status','cpt.phone_no','cet.email_address','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name','cm.created_by','cm.created_type,cet.id as email_trans_id,cpt.id as phone_trans_id,cm.created_date');
            $join_tables = array(
				'contact__status_master as csm' => 'csm.id = cm.contact_status',
                '(SELECT cptin.* FROM contact_phone_trans cptin WHERE cptin.is_default = "1" GROUP BY cptin.contact_id) AS cpt'=>'cpt.contact_id = cm.id',
                '(SELECT cetin.* FROM contact_emails_trans cetin WHERE cetin.is_default = "1" GROUP BY cetin.contact_id) AS cet'=>'cet.contact_id = cm.id',
				'contact_tag_trans ctat'=>'ctat.contact_id = cm.id',
                //'(SELECT cttin.* FROM contact_tag_trans cttin GROUP BY cttin.contact_id) AS ctat'=>'ctat.contact_id = cm.id',
				'login_master as lm' => 'lm.id = cm.created_by',
				'user_master as um' => 'um.id = lm.user_id'
            );
        ...
        if(is_array($status_id))
        {
            ...
            $add_status = '';
                ...
                foreach($status_id as $row)
                 {
                    //echo "ST - ".$row;
                    if(!empty($row))
                        $add_status .= "cm.contact_status = " . $row." OR ";
                 }
                 $add_status = rtrim($add_status,' OR');

            $where .= $add_status.')';
        }
			//echo $where;exit;

			//$or_where = array('cm.created_date > '=>$searchtext);
        ...
        else
        {
			...
			// Contact type added in this query //
			
			//$fields = array('cm.id,cm.is_subscribe','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','cm.company_name','csm.name as contact_status','group_concat(DISTINCT ctm.name ORDER BY ctm.name separator \',\') as contact_type','cpt.phone_no','cet.email_address','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name','cm.created_by','cm.created_type');
			...
			///////////////////////////////////////
			 ...
			 $fields = array('cm.id,cm.is_subscribe','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','cm.company_name','csm.name as contact_status','cpt.phone_no','cet.email_address','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name','cm.created_by','cm.created_type,cet.id as email_trans_id,cpt.id as phone_trans_id,cm.created_date,cll.contact_last_seen');
			 ...
			 $join_tables = array(
                ...
                '(SELECT cetin.* FROM contact_emails_trans cetin WHERE cetin.is_default = "1" GROUP BY cetin.contact_id) AS cet'=>'cet.contact_id = cm.id',
                'login_master as lm' => 'lm.id = cm.created_by',
                'contact_listing_last_seen as cll'=>'cll.login_id = lm.id',
			...
			//$match=array('cm.created_date'=>'2014-11-13');
        ...
        $group_by3='csm.id';
        $data['status_type'] = $this->obj1->getmultiple_tables_records($table3,$fields3,$join_tables3,'left','','','=','','','','',$group_by3);
		...
		$data['user_list'] = $this->obj1->getmultiple_tables_records($table,$fields,$join_tables,'left','',$match,'=','','','','asc');
		...
		$searchsort_test_session = $this->session->userdata('contacts_sortsearchpage_data');
		
		//pr($searchsort_test_session);
    ...
    */
    public function add_record()
    {
		...
		$data['communication_plans'] = $this->obj1->select_records1('',$match,'','=','','','','plan_name','asc', 'interaction_plan_master');
		...
		$data['user_list'] = $this->obj1->getmultiple_tables_records($table,$fields,$join_tables,'left','',$match,'=','','','','asc');
		//echo $this->db->last_query();
        //$match=array('lm.status'=> '1');
		//$data['user_list'] = $this->obj1->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','=','','','','asc');
		
		$data['all_tag_trans_data'] = $this->obj->select_tag_record('','','1');
    ...
    */
    public function insert_data()
    {
		...
		if(!empty($cdata['contact_status']))
		{
			...
			$data_status['created_by'] = $this->admin_session['id'];
		...
		if(!empty($slt_user))
		{
			for($i=0;$i<count($slt_user);$i++)
			{
				$match = array('id'=>$slt_user[$i]);
		...
		{
			redirect('admin/'.$this->viewName.'/edit_record/'.$contact_id.'/'.($contacttab+1));
		}
    ...
    */
    public function edit_record()
    {
		...
		if(!empty($data['user_add_list']))
		{	
			foreach($data['user_add_list'] as $row)
				$user_add_list[] = $row['user_id'];
		}
		$data['user_add_list'] = $user_add_list;
		//pr($data['user_add_list']);exit;
		...
		$data['all_tag_trans_data'] = $this->obj->select_tag_record($id,'','1');//echo $this->db->last_query();exit;
		//pr($data['all_tag_trans_data']);exit;
		...
		/*$fields = array('is_buyer_tab,lead_dashboard_tab,market_watch_tab');
		$match1 = array("id"=>$this->admin_session['id']);*/
		$fields = array('is_buyer_tab,lead_dashboard_tab,market_watch_tab');
		$match1 = array("id"=>$this->admin_session['admin_id']);
		//$match1 = array("id"=>$this->admin_session['id'],"is_buyer_tab"=> "1");
		...
		if(count($result) == 0)
		{
			$msg = $this->lang->line('common_archive_error_msg');
			$newdata = array('msg'  => $msg);
			$this->session->set_userdata('message_session', $newdata);	
    ...
    */
    ...
    {	
		...
		if($contacttab == 1)
		{
			...
			//$slt_user = array();
			//pr($slt_user);exit;
			//$this->obj->delete_table_user_contact_record($contact_id);
			$olduser = array();
			$user_add_list = $this->obj->select_user_contact_trans_record($contact_id);
			if(!empty($user_add_list))
			{
				foreach($user_add_list as $row)
					$olduser[] = $row['user_id'];
			}
			//pr($slt_user);
			//pr($olduser);
			$deleteuser = array_diff($olduser,$slt_user);
			//pr($newuser);exit;
			//pr($user_id);
			if(!empty($deleteuser))
			{
				foreach($deleteuser as $row)
				{
					$deldata['contact_id'] = $contact_id;
					$deldata['user_id'] = $row;
					$this->obj->delete_table_user_contact_record('',$deldata);
				}
			}
			if(!empty($slt_user))
			{
				...
				for($i=0;$i<count($slt_user);$i++)
				{
					if(!in_array($slt_user[$i],$olduser))
					{
						$match = array('id'=>$slt_user[$i]);
						$user_data = $this->admin_model->get_user($field, $match,'','=');
						if(!empty($user_data[0]['agent_type']))
				...
				/*if(!empty($user_add_list))
				{
					...
					$dataresult=$this->obj->select_contact_conversation_trans_record1($contact_id,$slt_user);
					//pr($dataresult);exit;
					if(empty($dataresult))
					{
						$data_conver['contact_id']=$contact_id;
			...
			if(!empty($allsocialtype) && count($allsocialtype) > 0)
			{
				for($i=0;$i<count($allsocialtype);$i++)
				{
					if(trim($allsocialname[$i]) != "")
					{
						...
						/*elseif($csdata['profile_type'] == 3)
						{
							//https://www.linkedin.com/profile/view?id=357280002&trk=nav_responsive_tab_profile
							$linkedin_url = explode('id=',$allsocialname[$i]);
							if(!empty($linkedin_url) && count($linkedin_url) > 1)
							{
								if(!empty($linkedin_url[1]))
								{
									$linkedin_filnal_url = explode('&',$linkedin_url[1]);
									if(!empty($linkedin_filnal_url[0]) && is_numeric($linkedin_filnal_url[0]))
									{
										$idata['id'] = $contact_id;
										$idata['linkedin_message_id'] = $linkedin_filnal_url[0];
						...
						//$s_name = explode("/",$allsocialname[$i]);
						//$csdata['website_name'] = $s_name[3];
...
//end additional field			
		...
		if($submitvaltab2 == '3')
		{
				redirect('admin/'.$this->viewName.'/edit_record/'.$contact_id.'/'.($submitvaltab2-1).'#documenets');
		}
    ...
    */
	public function ajax_archive_all()
	{
		...
		{
			//$c_id = $array_data[0];
			//$pagingid = $this->obj->getemailpagingid($c_id);
			$searchsort_session = $this->session->userdata('contacts_sortsearchpage_data');
			if(!empty($searchsort_session['uri_segment']))
				$pagingid = $searchsort_session['uri_segment'];
			else
    ...
    @Date: 18-09-2014
    */
	function add_to_active_list()
    {
        $id = $this->uri->segment(4);
				
		$this->obj->add_to_list_record($id);
		$msg = $this->lang->line('common_archive_to_list_msg');
        $newdata = array('msg'  => $msg);
        $this->session->set_userdata('message_session', $newdata);	
		redirect('admin/'.$this->viewName.'/view_archive');
        //redirect('admin/'.$this->viewName.'/msg/'.$this->lang->line('common_publish_msg'));
    }
    ...
    */
	...
	public function view_archive()
    {
		...
		$fields = array('cm.id','CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name) as contact_name','cm.company_name','csm.name as contact_status','cpt.phone_no','cet.email_address','cm.created_by','lm.admin_name','CONCAT_WS(" ",um.first_name,um.last_name) as user_name','cm.created_type');
		$join_tables = array(
                    'contact__status_master as csm' => 'csm.id = cm.contact_status',
		...
		if(!empty($searchtext))
		{
                    $match=array('CONCAT_WS(" ",cm.first_name,cm.middle_name,cm.last_name)'=>$searchtext,'CONCAT_WS(" ",cm.first_name,cm.last_name)'=>$searchtext,'CONCAT_WS(" ",cm.spousefirst_name,cm.spousemiddle_name,cm.spouselast_name)'=>$searchtext,'CONCAT_WS(" ",cm.spousefirst_name,cm.spouselast_name)'=>$searchtext,'email_address'=>$searchtext,'phone_no'=>$searchtext,'tag'=>$searchtext,'csm.name'=>$searchtext,'cm.company_name'=>$searchtext);
                    /*$data['datalist'] = $this->obj->select_records('',$match,'','like','',$config['per_page'],$uri_segment,$sortfield,$sortby);
                    $config['total_rows'] = count($this->obj->select_records('',$match,'','like',''));*/

                    /////////////

                    $data['datalist'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like',$config['per_page'], $uri_segment,$data['sortfield'],$data['sortby'],$group_by);
    ...
    */
	...
	$join_tables = array(
		...
		if(!empty($where_in))
			...
			$where_in = '';
		$res =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left',$match,'','like','','',$data['sortfield'],'',$group_by,'',$where_in);
		$output='';
		$data1='';
		$data2='';
		//pr($res);exit;
    ...
    */
	...
	function merge_search_contacts()
	{
		...
		$merge_data = $this->session->userdata('merge_data');
		if(!empty($fields))
		{
			$this->session->set_userdata('merge_data',$fields);
			$merge_data = $this->session->userdata('merge_data');
		}
		elseif(!empty($merge_data))
		{
			$fields = $merge_data;
			//pr($fields);exit;
		}
		...
		if(!empty($fields) && count($fields) > 0 && !empty($fields[0]))
		{
    ...
    */
    ...
    {
//New Code for facebook chat history By Mohit Trivedi....
		if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'login')
		{
			...
			//pr(json_encode($conversation['data']));exit;
				...
				if(!empty($get_contact_data))
				{
					...
					$old_msg=$this->obj->select_contact_chat_message_list($contactid,$get_contact_data[0]['fb_id'],$user_fbid,$this->admin_session['id']);
						...
						if(!empty($conversation['data']))
						{
							...
							if($get_contact_data[0]['fb_id'] == $user_fbid)
							{
								...
								if(empty($conversation['data'][$i]['to']['data'][1]['id']))
								{
									...
									$sycdata['created_by']=$this->admin_session['id'];
									...
									for($j=0;$j<count($conversation['data'][$i]['comments']['data']);$j++)
									{
										...
										{		
											...
											$data_insert['created_by']=$this->admin_session['id'];
							...
							if(($contact_fb_id==$data2[$i]['participent1'] || $contact_fb_id==$data2[$i]['participent2'])&&($login_id==$data2[$i]['participent1'] || $login_id==$data2[$i]['participent2']) && ($data2[$i]['participent2'] != NULL))
							{
									...
									for($j=0; $j<(count($data2[$i])); $j++)
									{
										...
										if (strtotime($lastupdatedate) < strtotime($datetime))
										{
											...
											$data4['created_by']=$this->admin_session['id'];
		...
		{
                    $cform_tab = check_joomla_tab_setting($this->admin_session['id']);
                    if(!empty($cform_tab))
		...
		$createdby = $this->admin_session['id'];
		...
		//print_r($data['contact_invitation_trans']);exit;
		/*if(!empty($user_id))
		{
		...
		unset($match);
		$match = array();
        $data['email_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc','contact__email_type_master');
		$data['phone_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc','contact__phone_type_master');
		$data['address_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc','contact__address_type_master');
		$data['status_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc','contact__status_master');
		$data['profile_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc','contact__social_type_master');
		$data['contact_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc','contact__type_master');
		$data['document_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc', 'contact__document_type_master');
		$data['source_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc', 'contact__source_master');
		$data['field_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc', 'contact__additionalfield_master');
		
		$data['website_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','asc','contact__websitetype_master');
		
		$data['method_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','asc', 'contact__method_master');		
		$data['disposition_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc', 'contact__disposition_master');
		...
		//pr($data['conversations']);exit;
		/*$fields = array('cct.id','tm.task_name,tm.desc');
		$join_tables = array('task_master as tm'=>'tm.id = cct.task_id');
		$data['conversations'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','ctt.is_completed_task','',$group_by,$where);
		//pr($data['conversations']);exit;*/
				/////////////////////////////////////////////End Conversations/////////////////////////////////////////
	
		/////////////////////////////////////////// Communication Plan ////////////////////////////////
		$table = "interaction_plan_contact_communication_plan as ipcc";
		$fields = array('ipcc.id,ipcc.completed_by,lom1.admin_name as completed_by_name,ipcc.task_completed_date','ipcc.contact_id','ipcc.task_date','ipcc.is_done','ipcc.interaction_plan_interaction_id','ipm.plan_name as interaction_plan_name','iptm.name','ipim.description as interaction_name','ipm.id as interaction_plan_id,ipim.assign_to,lom.admin_name as assign_name','ipim.status','ipim.start_type','ipim.interaction_id','CONCAT_WS(" ",um.first_name,um.middle_name,um.last_name) as assign_name1','CONCAT_WS(" ",um1.first_name,um1.middle_name,um1.last_name) as completed_by_name1,ipim.interaction_type');
		$join_tables = array(
		...
		$group_by='ipcc.id';
		$where=array('ipcc.contact_id'=>$id,'ipcc.is_done'=>"'0'");
		...
		$table = "interaction_plan_contact_communication_plan as ipcc";
		$fields = array('ipcc.id,ipcc.completed_by,lom1.admin_name as completed_by_name,ipcc.task_completed_date','ipcc.contact_id','ipcc.task_date','ipcc.is_done','ipcc.interaction_plan_interaction_id','ipm.plan_name as interaction_plan_name','iptm.name','ipim.description as interaction_name','ipm.id as interaction_plan_id,ipim.assign_to,lom.admin_name as assign_name','ipim.status','ipim.start_type','ipim.interaction_id','CONCAT_WS(" ",um.first_name,um.middle_name,um.last_name) as assign_name1','CONCAT_WS(" ",um1.first_name,um1.middle_name,um1.last_name) as completed_by_name1,ipim.interaction_type','task_completed_date as created_date','"comm" AS communication_type');
		$join_tables = array(
							'interaction_plan__plan_type_master as iptm' => 'iptm.id = ipcc.interaction_type',
							'interaction_plan_master as ipm' => 'ipm.id = ipcc.interaction_plan_id',
							'interaction_plan_interaction_master as ipim'=>'ipim.id = ipcc.interaction_plan_interaction_id',
		...
		$group_by='ipcc.id';
		$where=array('ipcc.contact_id'=>$id,'ipcc.is_done'=>"'1'");
		$data['interation_plan_communication_plan_completed'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','special_case','',$group_by,$where);
		
		/* End */
		
		//pr($data['conversations']);
		...
		$group_by='ipcp.id';
		$where=array('contact_id'=>$id,'is_done'=>"'0'");
		$data['personale_touches'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','followup_date','asc',$group_by,$where);
		
		//pr($data['personale_touches']);exit;
		
		//echo $this->db->last_query();exit;
		
		/* Completed personal touches */
		
		$table = "interaction_plan_contact_personal_touches as ipcp";
		$fields = array('ipcp.id','ipcp.task','iptm.name','ipcp.followup_date','ipcp.is_done','ipcp.followup_date as created_date','"personal" AS personal_touch_type','lm_c.admin_name as created_by_admin','CONCAT_WS(" ",um_c.first_name,um_c.middle_name,um_c.last_name) as created_by_user');
		$join_tables = array(
								'interaction_plan__plan_type_master as iptm' => 'iptm.id = ipcp.interaction_type',
								'login_master as lm_c'=>'lm_c.id = ipcp.created_by',
								'user_master as um_c'=>'um_c.id = lm_c.user_id',
							);
		$group_by='ipcp.id';
		$where=array('contact_id'=>$id,'is_done'=>"'1'");
		$data['personale_touches_completed'] =$this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','followup_date','asc',$group_by,$where);
		
		/* END */
		
		// Merge conversation, completed communication, and completed personal touches
		
		$op = array_merge($data['conversations'],$data['interation_plan_communication_plan_completed'],$data['personale_touches_completed']);
		//pr($op);exit;
		
		//echo $op[0]['created_date'];exit;
		
		usort($op, function($a, $b) {
			/*echo $a['created_date']."---".$b['created_date']."<br>";
		...
		$data['conversations'] = $op;
		
		
		
		$data['interaction_type'] = $this->obj1->select_records1('',$match,'','=','','','','name','desc', 'interaction_plan__plan_type_master');


		////////////////////////////////////////////////////Get communication plan data //////////////////////////////////////////////
		$data['communication_plans'] = $this->obj1->select_records1('',$match,'','=','','','','description','desc', 'interaction_plan_master');
		
		
		// Interaction Plan list	
		//$plan_arr = $this->obj2->select_assigned_contact_intraction_plan($id);
		//$data['interaction_plan_list'] = $this->obj2->select_contact_intraction_plan($plan_arr);
		$data['interaction_plan_list'] = $this->obj2->select_contact_intraction_plan($id);
		$data['email_trans_data'] = $this->obj->select_email_trans_record($id);
		$data['phone_trans_data'] = $this->obj->select_phone_trans_record($id);
		$data['address_trans_data'] = $this->obj->select_address_trans_record($id);
		...
		$data['document_trans_data'] = $this->obj->select_document_trans_record($id);
                ...
                // Contact Register Sanjay Moghariya. 29-10-2104
                $config['per_page'] = '10';
		...
		{
			$match = array('linkedin_id'=>$data['editRecord'][0]['linkedin_id'],'created_by'=>$this->admin_session['id']);
		...
		/*if($data['editRecord'][0]['id']==$id && $data['editRecord'][0]['created_by']==$createdby && $data['editRecord'][0]['created_type']=='4')
		{
			$data['already_connected']='1';
		}*/
		...
		//if(($result[0]['joomla_user_id'] != 0) && (!empty($result[0]['joomla_user_id']) == !empty($result[0]['created_by'])))
//		exit;
		
                // Saved Searches Sanjay Moghariya. 29-10-2014
                $config1['per_page'] = '10';
                ...
                // Valaution contact form Sanjay Moghariya. 09-03-2015
                ...
                // End Valuation contact form SANJAY M. 09-03-2015
		...
		$match=array('contact_id'=>$id);
		$data['datalist'] = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','',$match,'=',$config6['per_page'], $uri_segment6,'','',$group_by);
		...
		//////// Check Buyer Right or Not 
		$match1 = array("id"=>$this->admin_session['admin_id'],"is_buyer_tab"=> "1");
		$data['right_buyer'] = $this->obj1->select_records1('',$match1,'','=','','','','id','desc','login_master');
		//pr($data['right_buyer']);exit;
		//////End Buyer Tab
    ...
    */
	function property_listing()
	{
		...
		$this->pagination->initialize($config);
		$data['pagination6'] = $this->pagination->create_links();
		$this->load->view('admin/'.$this->viewName.'/property_list',$data);	
	}
    /*
    ...
    */
    public function view_record_index_fav()
    {
        ...
        if(!empty($searchtext))
        {
            ...
            $data['searchtext2'] = stripslashes($searchtext);
        } else {
            if(empty($allflag))
            {
                if(!empty($searchsort_session['searchtext'])) {
    ...
    */
    public function view_record_index_prop_view()
    {
        ...
        if(!empty($searchtext))
        {
            $data['searchtext3'] = stripslashes($searchtext);
        } else {
            if(empty($allflag))
            {
                if(!empty($searchsort_session['searchtext'])) {
    ...
    */
    public function view_record_index_lastlog()
    {
        ...
        {
            //$searchtext = $this->input->post('searchtext');
            $data['searchtext4'] = stripslashes($searchtext);
        } else {
            if(empty($allflag))
            {
                if(!empty($searchsort_session['searchtext5'])) {
                   
				   $searchtext =  mysql_real_escape_string($searchsort_session['searchtext5']);
		...
		if(!empty($user_id))
		{
			for($j=0;$j<count($user_id);$j++)
			{
				for($i=0;$i<count($array_data);$i++)
				{
					...
					$match=array('contact_id'=>$check_contact_id,'user_id'=>$user_id[$j]);
					$contact_check = $this->obj1->select_records1('',$match,'','=','','','','','asc', 'user_contact_trans');
					if(empty($contact_check))
					{
						$match = array('id'=>$user_id[$j]);
						$user_data = $this->admin_model->get_user($field, $match,'','=');
						if(!empty($user_data[0]['agent_type']))
							$data['agent_type'] = $user_data[0]['agent_type'];
						...
						$data_conver['contact_id']=$check_contact_id;
						$data_conver['created_by'] = $this->admin_session['id'];
						$data_conver['assign_to'] = $user_id[$j]; 
					...
					/*else
					{
							$this->obj->delete_table_user_contact_record($array_data[$i]);
							$cudata['contact_id'] =$array_data[$i];
							$cudata['user_id'] = $user_id[$j];   
							$cudata['created_by'] = $this->admin_session['id'];
							$cudata['created_date'] = date('Y-m-d H:i:s');		
							$cudata['status'] = '1';
							$this->obj->insert_user_contact_trans_record($cudata);
							unset($cudata);
							$msg=0;
							$dataresult=$this->obj->select_contact_conversation_trans_record1($array_data[$i],$user_id[$j]);
							if(empty($dataresult))
							{
								$data_conver['contact_id']=$array_data[$i];
								$data_conver['created_by'] = $this->admin_session['id'];
								$data_conver['assign_to'] = $user_id[$j];
								$data_conver['log_type'] = '4';// Log Type 4 -Re-assign in User.
								$data_conver['created_date'] = date('Y-m-d H:i:s');
    ...
    */
	...
	public function insert_merge_data()
	{
		...
		redirect('admin/'.$this->viewName.'/merge_search_contacts');
		
	}
	/*
    @Description: This Function interaction_id_done to Unsubscription/Subscription.
    @Author: Kaushik  Valiya
    @Input: Contact Id to Unsubscription/Subscription Contact Interaction Table.
    @Output: Unsubscription/Subscription of Personal Touches
    @Date: 26-08-2014
	...
	*/
	...
	{			
		...
		/*if(!empty($interaction_type) && $interaction_type == '6' && !empty($data['id']))
			$this->send_mail($data['id']);
		elseif(!empty($interaction_type) && $interaction_type == '3' && !empty($data['id']))
	...
	function insert_last_action_communication_plan()
	{
		...
		$this->obj->update_interaction_plan_interaction_transtrans_record($data);
		$cdata['contact_id'] = $this->input->post('hid_contact_id');
		$cdata['interaction_plan_id'] = $this->input->post('hid_current_plan_id');
		$cdata['is_completed'] = '1';
		
		$this->obj->update_interaction_plan_contacts_trans($cdata);
		...
		elseif($interaction_plan_start_type == 3)
		{
			...
			if(!empty($contact_id))
			{
				...
				{
					redirect("admin/contacts/view_record/".$contact_id."/6#myTab2");
					exit;
				}
	...
	/*
    @Description: This Function Subscription List
    @Author: Kaushik  Valiya
    @Input: Contact Id to Unsubscription/Subscription Contact Personal Touches Table.
    @Output: Unsubscription/Subscription of Personal Touches
    @Date: 26-08-2014
	...
	*/
	...
	/*
    @Description: This Function Insert Personal Touches List
    @Author: Kaushik  Valiya
    @Input: Contact Id to Insert Contact Personal Touches Table.
    @Output: Insert of Personal Touches
    @Date: 26-08-2014
	...
	*/
	...
	{			
		...
		$data['created_date'] = date('Y-m-d H:i:s');
		$data['status'] = '1';
                if(!empty($data['task']) && !empty($data['followup_date']))
		...
		if($from_page == 1)
			redirect("admin/leads_dashboard/view_record/".$data['contact_id']);
		else if($from_page == 2)
			redirect("admin/leads_dashboard/");	
		else if($from_page == 3)
			redirect("admin/dashboard/telephone_task/");
		else
			redirect("admin/contacts/view_record/".$data['contact_id']."/5#myTab2");
	...
	*/
	...
	function insert_conversations()
	{
		...
		$data['status'] = '1';
                ...
                if($from_page == 1 || $from_page == 2 || $from_page == 3)
		{
                    $data['interaction_type'] = $this->input->post('sl_interaction_type');
                    $data['plan_name']= 'Call';
		}
		...
		if($from_page == 1)
			...
			redirect("admin/joomla_property_cron/assigned_contact_list/");
		else
			redirect("admin/contacts/view_record/".$data['contact_id']."/4#myTab2");
	}
   ...
   */
	...
	public function fb_conversation()
    {
        ...
        */
        public function selectedview_session()
        {
            ...
            else if($selected_view == '109') // Valuation searched
        ...
        public function view_property_contact_popup()
	{
        ...
        */
        public function edit_valuation_searched()
        {
            ...
            $data['editRecord'] = $result;
            $data['main_content'] = "admin/".$this->viewName."/add_valuation_searched";
            $this->load->view('admin/include/template', $data);
        }
        ...
        */
        public function update_valuation_searched_data()
        {
            ...
            if(!empty($contact_id['contact_id']))
                redirect('admin/'.$this->viewName.'/view_record/'.$contact_id['contact_id']);
            else
                redirect('admin/'.$this->viewName);
            //redirect(base_url('admin/'.$this->viewName.'/'.$pagingid));
        ...
        */
        ...
        {
            $tab_result = check_joomla_tab_setting($this->admin_session['admin_id']);
            ...
            $data['main_content'] = "admin/".$this->viewName."/add_saved_searches";
            $this->load->view('admin/include/template', $data);
        }
        ...
        */
        public function insert_saved_search_data()
        {
            ...
            //$cdata['lw_admin_id'] = $this->admin_session['id'];
	...
	function send_twitter_message()
	{
		//pr($_POST);exit;
		 ...
		 //echo strlen($email_message); exit;
		 ...
		 if(!empty($platform) && $platform == '2')
		 {
			...
			if(isset($submit_direct) && !empty($submit_direct))
			{
				...
				if(!empty($error))
				{
					 ...
					 redirect('admin/contacts/view_record/'.$contactid.'/7#myTab2');
				}
				else
				{
			...
			if(isset($submit_twit) && !empty($submit_twit))
			{
				...
				$params4 = array();
				$params4['screen_name'] = $screen_name;
				$params4['status'] =  "@$screen_name"." ".$email_message;
		 ...
		 if(!empty($linkedin_access_token))
		 {
			 if(!empty($contactid))
			 {
				...
				if(!empty($email))
				{
					...
					$data['user_id']=$this->admin_session['id'];
					$data['create_date']= date('Y-m-d H:i:s');
					$this->contacts_model->insert_linkedin_invitation($data);
					redirect('admin/contacts/view_record/'.$contactid.'/7#myTab1');
				}
			}
		 }
		 else
	...
	public function send_mail($communication_id='')
	{	
		...
		$join_tables = array('interaction_plan_interaction_master as ipim'=>'ipim.id = ipccp.interaction_plan_interaction_id',
							 'email_campaign_master ecm'=>'ecm.interaction_id = ipim.id',
							 'email_campaign_recepient_trans as ecr'=>'ecm.id = ecr.email_campaign_id AND ecr.contact_id=ipccp.contact_id',
							 'contact_emails_trans cet'=>'cet.contact_id = ecr.contact_id',
							 'contact_master cm'=>'cm.id = ecr.contact_id'
							 );
			
		$group_by = 'cet.contact_id';
		$wherestring = "cet.is_default = '1' AND cm.is_subscribe = '0' AND ecr.is_send = '0' AND ipccp.id = ".$id;
		...
		if(count($result) > 0)
		{
			...
			$campaign_data = $this->email_campaign_master_model->select_records('',$match,'','=');
			...
			$attachment = $this->email_campaign_master_model->select_email_campaign_attachments($result[0]['email_campaign_id']);
			...
			else
			{
				if(!empty($result[0]['email_address']))
				{
					...
					if($result[0]['is_unsubscribe'] == '1'){
						...
						$headers = str_replace('{(my_unsubscribe_link)}',$message1,$headers);
					}
	...
	public function send_sms($communication_id='')
	{
		...
		$join_tables = array('interaction_plan_interaction_master as ipim'=>'ipim.id = ipccp.interaction_plan_interaction_id',
							 'sms_campaign_master scm'=>'scm.interaction_id = ipim.id',
							 'sms_campaign_recepient_trans as scr'=>'scr.sms_campaign_id = scm.id AND scr.contact_id=ipccp.contact_id',
							 'contact_phone_trans cpt'=>'cpt.contact_id = scr.contact_id',
							 'contact_master cm'=>'cm.id = scr.contact_id'
							 );
			
		$group_by = 'cpt.contact_id';
		$wherestring = "cpt.is_default = '1' AND scr.is_send = '0' AND ipccp.id = ".$id;
		$result = $this->obj->getmultiple_tables_records($table,$fields,$join_tables,'left','','','','','','','',$group_by,$wherestring);
		/*echo $this->db->last_query();
		pr($result); exit;*/
		if(count($udata) > 0)
	...
	*/
	...
	{	
		...
		if($from_page == 1)
			redirect("admin/leads_dashboard/view_record/".$contact_id);
		else if($from_page == 2)
			redirect("admin/leads_dashboard/");
		else if($from_page == 3)
			redirect("admin/dashboard/telephone_task/");
		else
			redirect("admin/contacts/view_record/".$contact_id."/5#myTab2");
	...
	function change_conversations()
	{
		...
		$this->load->view('admin/leads_dashboard/history_details',$data);
	}
